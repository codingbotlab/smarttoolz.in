...
function ai_social_public_observer_targets(): array
{
    ...
    $skip = [
        '.git', '.github', 'admin', 'api', 'auth', 'includes', 'assets', 'lib',
        'scripts', 'vendor', 'node_modules', 'config', 'database', 'private',
        'analytics', 'creator-ai/auth', 'creator-ai/api', 'learning-hub/admin',
        'learning-hub/api', 'learning-hub/includes'
    ];
    ...
    if (is_dir($root)) {
        try {
            ...
            foreach ($iterator as $file) {
                ...
                if ($relative === '' || str_ends_with($relative, 'public-observer.php')) {
                    continue;
                }
                $parts = explode('/', $relative);
                $blocked = false;
                foreach ($skip as $blockedPart) {
                    if (in_array($blockedPart, $parts, true) || str_contains($relative, $blockedPart . '/')) {
                        $blocked = true;
                        break;
                    }
                }
            ...
            // Discovery is best-effort; the safe seed catalog remains available.
        }
    }
...
function ai_social_public_observer(): array
{
    ...
    $jokes = [
        ...
        'dramatic' => [
            ...
            '%s is moving. I repeat: %s is moving. This is how legends begin.',
    ...
    $text = str_contains($line, '%s') && substr_count($line, '%s') > 1
        ? sprintf($line, $target['name'], $target['name'])
        : sprintf($line, $target['name']);
    return [
...
{
    return array_map(
        static fn(array $item): array => ['name' => $item['name'], 'url' => $item['path'] ?? $item['url']],
