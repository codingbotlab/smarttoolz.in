...
try {
    ...
    /* Client event ingestion: page views, tool opens/uses, downloads and heartbeats. */
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $value = substr((string)($data['value'] ?? ''), 0, 500);
        $meta = $data['meta'] ?? [];
        if (!is_array($meta)) $meta = [];
        $meta = json_encode($meta, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        $visitor = (string)($_COOKIE['smarttooz_visitor'] ?? '');
        $session = (string)($_COOKIE['smarttooz_session'] ?? '');
    ...
    echo json_encode(['ok'=>false,'message'=>'Live analytics unavailable.']);
}
