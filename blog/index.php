      ...
      <div class="blog-grid">
        <?php foreach ($posts as $post): ?>
          <article class="blog-card">
            <a class="blog-thumb" href="<?= htmlspecialchars($post['url'], ENT_QUOTES, 'UTF-8') ?>" aria-label="Read <?= htmlspecialchars($post['title'], ENT_QUOTES, 'UTF-8') ?>">
              <img src="<?= htmlspecialchars($post['thumbnail'], ENT_QUOTES, 'UTF-8') ?>" width="1200" height="630" loading="lazy" alt="<?= htmlspecialchars($post['title'], ENT_QUOTES, 'UTF-8') ?>">
            </a>
            <div class="blog-card-content"><div class="blog-card-top"><span>GUIDE</span><span>SmartToolz</span></div><h3><a href="<?= htmlspecialchars($post['url'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($post['title'], ENT_QUOTES, 'UTF-8') ?></a></h3><p>Read this practical SmartToolz guide for useful tips and straightforward ways to handle everyday digital tasks.</p><a class="blog-read" href="<?= htmlspecialchars($post['url'], ENT_QUOTES, 'UTF-8') ?>">Read article →</a></div>
          </article>
        <?php endforeach; ?>
      </div>
    <?php else: ?>
