...
$canonicalPath=(string)(parse_url((string)($_SERVER['REQUEST_URI']??'/'),PHP_URL_PATH)??'/');
$canonicalPath='/' . ltrim($canonicalPath,'/');
if($canonicalPath!=='/' && substr($canonicalPath,-1)!=='/' && $isToolPage){$canonicalPath.='/';}
...
$seoTitle=$isToolPage
    ...
    : ($isAllTools ? 'All Free Online Tools | SmartToolz' : ($isHome ? 'SmartToolz — Free Online Tools for Images, PDF, Text & More' : ucwords(str_replace(['-','_','.'],' ',pathinfo($script,PATHINFO_FILENAME))).' | SmartToolz'));
$seoDescription=$isToolPage
    ...
    : ($isAllTools ? 'Browse free SmartToolz online tools for images, PDFs, text, developers, calculators and everyday tasks.' : 'SmartToolz provides free, fast and simple online tools for images, PDFs, text, developers and everyday tasks.');
...
$cssHtml='';
if(is_file($cssFile)){$cssHtml='<style id="smarttoolz-css">'.file_get_contents($cssFile).'</style>';}
...
ob_start(function(string $html) use ($seoHead,$headerHtml,$toolIntroHtml,$cssHtml,$layoutCss): string {
    if(stripos($html,'</head>')!==false){$headInsert=$seoHead.$cssHtml.$layoutCss;$html=preg_replace('~</head>~i',$headInsert.'</head>',$html,1)??$html;}
