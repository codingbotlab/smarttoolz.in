...
if($canonicalPath!=='/' && ($isToolPage||$isCategoryPage) && substr($canonicalPath,-1)!=='/'){$canonicalPath.='/';}
$canonicalUrl='https://smarttoolz.in'.$canonicalPath;

$seoTitle=$isToolPage
    ? 'Free Online '.$toolTitle.' | SmartToolz'
    : ($isCategoryPage ? $categoryTitle.' — Free Online Tools | SmartToolz' : ($isAllTools ? 'All Free Online Tools | SmartToolz' : ($isHome||$isLegacyHome ? 'SmartToolz — Free Online Tools for Images, PDF, Text & More' : ucwords(str_replace(['-','_','.'],' ',pathinfo($script,PATHINFO_FILENAME))).' | SmartToolz')));
$seoDescription=$isToolPage
    ...
    : ($isCategoryPage ? 'Browse free '.$categoryTitle.' on SmartToolz. Simple online tools that work in your browser without signup.' : ($isAllTools ? 'Browse free SmartToolz online tools for images, PDFs, text, developers, calculators and everyday tasks.' : 'SmartToolz provides free, fast and simple online tools for images, PDFs, text, developers and everyday tasks.'));
...
if($isToolPage){
    $schema=json_encode([
        '@context'=>'https://schema.org','@type'=>'WebApplication','name'=>$toolTitle.' - SmartToolz','url'=>$canonicalUrl,
        'description'=>$seoDescription,'applicationCategory'=>'UtilitiesApplication','operatingSystem'=>'Any','offers'=>['@type'=>'Offer','price'=>'0','priceCurrency'=>'USD']
...
ob_start(function(string $html) use ($seoHead,$seoTitle,$headerHtml,$toolIntroHtml,$cssHtml,$layoutCss): string {
    if(stripos($html,'<head')!==false){
        ...
        if(stripos($html,'</title>')!==false && stripos($html,'<title>')!==false && str_contains($html,'/tools/')){$html=preg_replace('~<title>.*?</title>~is','<title>'.htmlspecialchars($seoTitle,ENT_QUOTES,'UTF-8').'</title>',$html,1)??$html;}
        $html=preg_replace('~</head>~i',$seoHead.$cssHtml.$layoutCss.'</head>',$html,1)??$html;
    }
    if(stripos($html,'<body')!==false){$html=preg_replace('~(<body[^>]*>)~i','$1'.$headerHtml.$toolIntroHtml,$html,1)??$html;}
    return $html;
});
