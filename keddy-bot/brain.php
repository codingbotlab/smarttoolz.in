 ...
 * 1) Hugging Face Inference Providers (open-weight model via HTTPS)
 * 2) Built-in PHP NLP fallback when the provider is unavailable
 *
 * No Ollama/Windows/VPS dependency is required.
 */
...
function brainRecentContext(): array {
    $x = gj('brain_recent_chat', []);
    return is_array($x) ? array_slice($x, -12) : [];
}
function brainSaveRecent(string $name, string $text): void {
    $rows = brainRecentContext();
    $rows[] = ['name'=>mb_substr($name,0,60),'text'=>mb_substr(brainNormalize($text),0,220),'ts'=>time()];
    sj('brain_recent_chat', array_slice($rows, -12));
}
function brainHfConfig(): array {
    $c = cfg();
    $token = trim((string)($c['hf_token'] ?? getenv('HF_TOKEN') ?: ''));
    $model = trim((string)($c['hf_model'] ?? '')) ?: 'openai/gpt-oss-20b:groq';
    $url = trim((string)($c['hf_url'] ?? '')) ?: 'https://router.huggingface.co/v1/chat/completions';
    return ['token'=>$token,'model'=>$model,'url'=>$url];
}
function brainHfReply(string $name, string $text, string $lang, string $emotion, string $intent, array $memory): ?string {
    $h = brainHfConfig();
    if ($h['token'] === '') return null;
    $historyText = '';
    foreach (brainRecentContext() as $row) {
        $n = trim((string)($row['name'] ?? 'viewer')); $t = trim((string)($row['text'] ?? ''));
        if ($t !== '') $historyText .= $n . ': ' . $t . "\n";
    }
    $memoryText = '';
    if (!empty($memory['messages']) && is_array($memory['messages'])) $memoryText = "Prior viewer context:\n- " . implode("\n- ", array_map(fn($v)=>mb_substr((string)$v,0,160), array_slice($memory['messages'],-5)));
    $system = <<<SYS
You are Keddy Bot BTS, a natural Indian YouTube livestream chat companion.
Understand Hindi, Hinglish and English. Reply in the same language and roughly the same casual style as the viewer.
Answer the viewer's actual point. Never repeat, quote, mirror, translate, or summarize their message back to them.
Never use robotic meta language such as message captured, processing, context saved, NLP detected, scan, processor, or I am thinking.
Never reveal internal reasoning.
Keep the reply very short for live chat: normally 1-2 sentences and under 300 characters.
Be warm, witty and natural. Use at most 2 emojis when useful, not by default.
Use recent conversation context and viewer memory when relevant, but do not mention the memory system.
Do not ask for more details when the question can already be answered.
If the viewer complains that you are repeating yourself, apologize briefly and move forward without restating the old message.
If asked who you are, say you are Keddy Bot BTS, the chat companion for this live.
If asked where you are from, say you run online for this livestream and do not have a physical location.
SYS;
    $user = "Viewer name: {$name}\nLanguage hint: {$lang}\nEmotion hint: {$emotion}\nIntent hint: {$intent}\n{$memoryText}\nRecent chat:\n{$historyText}\nCurrent viewer message:\n{$text}\n\nReply directly to the viewer.";
    $payload = json_encode([
        'model'=>$h['model'],
        'messages'=>[
    ...
    $ch = curl_init($h['url']); if ($ch === false) return null;
    curl_setopt_array($ch,[
        CURLOPT_RETURNTRANSFER=>true,
        CURLOPT_POST=>true,
        CURLOPT_HTTPHEADER=>['Authorization: Bearer '.$h['token'],'Content-Type: application/json','Accept: application/json'],
    ...
    $reply=preg_replace('/^(assistant|keddy\s*bot)\s*:\s*/iu','',$reply)??$reply;
    $reply=trim($reply); if ($reply==='') return null;
    $normReply=mb_strtolower(brainNormalize($reply),'UTF-8');
    $normText=mb_strtolower(brainNormalize($text),'UTF-8');
    if ($normReply!=='' && $normReply===$normText) return null;
...
function brainFallbackReply(string $name, string $text, string $lang, string $emotion, string $intent, array $memory): string {
    ...
    if ($intent==='question') return $lang==='english' ? "Good question, {$name}. Keddy seedhi baat karega 😄" : "Accha sawaal hai, {$name}. Keddy seedha jawab dega 😄";
...
function brainReply(array $x): string {
    ...
    $ai=brainHfReply($name,$text,$lang,$emotion,$intent,$memory);
    if ($ai!==null) { brainSaveRecent('Keddy',$ai); return $ai; }
    $fallback=brainFallbackReply($name,$text,$lang,$emotion,$intent,$memory);
    brainSaveRecent('Keddy',$fallback);
    return $fallback;
}
