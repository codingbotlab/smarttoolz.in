...
$error='';$notice='';
if(isset($_GET['connected'])) $notice='YouTube connected successfully.';
if(isset($_GET['wrong_channel'])) $error='Keddy is connected as "'.(gv('oauth_channel_title')?:'another YouTube channel').'". Reconnect and select the "Keddy Bot BTS" channel/account in Google.';
if($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        ...
        elseif($a==='stop') stopS();
        elseif($a==='next') nextS();
        elseif($a==='reconnect') { sv('oauth',''); sv('oauth_channel_id',''); sv('oauth_channel_title',''); sv('chat_token',''); header('Location: oauth.php'); exit; }
        elseif($a==='save'){
...
$connected=!!gv('oauth');
$connectedChannel=gv('oauth_channel_title');
$ss=session();$tasks=tasks();
...
<?php if($connected):?><div class="identity">Connected as: <strong><?=h($connectedChannel?:'Unknown YouTube channel')?></strong><?php if($connectedChannel!==KEDDY_BOT_CHANNEL_NAME):?> <span class="muted">• Please use Keddy Bot BTS</span><?php endif;?></div><?php endif;?>
...
<script>
const keddyToken=<?=json_encode($heartbeatToken)?>;const keddyActive=<?=$ss['active']?'true':'false'?>;
if(keddyActive){const tick=()=>fetch('cron.php?heartbeat=1&token='+encodeURIComponent(keddyToken),{cache:'no-store'}).catch(()=>{});tick();setInterval(tick,120000);}
</script></body></html>
