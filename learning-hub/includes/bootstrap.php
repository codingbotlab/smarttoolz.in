...
function lh_install(): void {
    ...
    $sql=[
      ...
      "CREATE TABLE IF NOT EXISTS learning_enrollments(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,user_id BIGINT UNSIGNED NOT NULL,course_id BIGINT UNSIGNED NOT NULL,status VARCHAR(30) NOT NULL DEFAULT 'enrolled',progress_percent TINYINT UNSIGNED NOT NULL DEFAULT 0,enrolled_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,last_accessed_at TIMESTAMP NULL DEFAULT NULL,completed_at TIMESTAMP NULL DEFAULT NULL,UNIQUE KEY user_course(user_id,course_id),INDEX(user_id,enrolled_at),INDEX(course_id,status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4",
