 ...
 * Only same-host HTTP(S) page links are queued; external CDN/OAuth URLs
 * are never converted into fake smarttoolz.in paths.
...
function fetch_url(string $url,int $timeout):array{
    ...
    $ch=curl_init($url);
    curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_FOLLOWLOCATION=>false,CURLOPT_MAXREDIRS=>0,CURLOPT_CONNECTTIMEOUT=>5,CURLOPT_TIMEOUT=>$timeout,CURLOPT_USERAGENT=>'SmartToolz-Live-Crawler/4.0']);
...
function queue_links(string $pageUrl,string $html,string $host,array &$queue,array &$queued,array &$seen,int $maxPages):void{
    ...
    foreach($m[1]??[] as $href){
        ...
        $queued[$next]=true;$queue[]=$next;if(count($queue)>=$maxPages*2)break;
    }
}
function seed_sources(string $host,array &$queue,array &$queued,int $timeout):void{
    foreach(['https://smarttoolz.in/sitemap.xml','https://smarttoolz.in/robots.txt','https://smarttoolz.in/smart-toolz/sitemap.xml'] as $source){
        try{[,,$body]=fetch_url($source,$timeout);}catch(Throwable){continue;}
        ...
        if(preg_match_all('/<loc>\s*(.*?)\s*<\/loc>/is',$body,$locs))foreach($locs[1] as $loc){$loc=trim(html_entity_decode($loc));if(crawlable($loc,$host)&&!isset($queued[$loc])){$queued[$loc]=true;$queue[]=$loc;}}
    }
}
...
try{
    ...
    while($queue&&count($checked)<$maxPages){
        ...
        if(!$url||isset($seen[$url])||!crawlable($url,$host))continue;$seen[$url]=true;
        try{
    ...
    write_report($reportFile,$report);echo json_encode($report,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE).PHP_EOL;exit($errors?1:0);
}catch(Throwable $e){
