...
function go(string $tab): never { header('Location: /smart-toolz/admin/?tab=' . rawurlencode($tab)); exit; }
try {
  if ($action === 'ad_save') {
    ...
    $id=(int)($_POST['id']??0); $role=(string)($_POST['role']??'user'); if(!in_array($role,['user','admin','superadmin'],true))$role='user';
    $q=$db->prepare('SELECT email FROM creator_users WHERE id=? LIMIT 1');$q->execute([$id]);$email=(string)$q->fetchColumn(); if(strtolower($email)===strtolower(SMARTTOOLZ_ADMIN_EMAIL))$role='admin';
    $q=$db->prepare('UPDATE creator_users SET role=? WHERE id=?');$q->execute([$role,$id]);adminAudit('change_user_role','creator_users',(string)$id,$email.' => '.$role);
  } elseif ($action === 'user_credits') {
    $id=(int)($_POST['id']??0);$credits=max(0,(int)($_POST['credits']??0));$daily=max(0,(int)($_POST['daily_credits']??0));
    $q=$db->prepare('UPDATE creator_users SET credits=?,daily_credits=? WHERE id=?');$q->execute([$credits,$daily,$id]);adminAudit('update_user_credits','creator_users',(string)$id,'credits='.$credits.',daily='.$daily);
  }
