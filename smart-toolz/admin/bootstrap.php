...
function adminInstall(): void
{
    ...
    foreach (['users', 'creator_users'] as $table) {
        if (adminTableExists($db, $table) && !adminColumnExists($db, $table, 'role')) {
            $db->exec("ALTER TABLE `{$table}` ADD COLUMN role VARCHAR(20) NOT NULL DEFAULT 'user'");
        }
    }
    ...
    // Requested permanent admin account. This only changes role, never password/credits.
    foreach (['users', 'creator_users'] as $table) {
        if (adminTableExists($db, $table) && adminColumnExists($db, $table, 'role')) {
...
function requireAdmin(): array
{
    ...
    if (!$user || !in_array((string)($user['role'] ?? 'user'), ['admin','superadmin'], true)) {
        if (empty($_SESSION['user_id'])) {
            header('Location: /creator-ai/auth/google-login.php');
        } else {
...
{
    if (empty($_SESSION['smarttoolz_admin_csrf'])) {
        $_SESSION['smarttoolz_admin_csrf'] = bin2hex(random_bytes(32));
    }
...
function verifyAdminCsrf(): void
{
    ...
    if (!hash_equals(adminCsrf(), $token)) {
        ...
        exit('Invalid security token. Please reload the admin panel.');
    }
}
...
function adminAudit(string $action, ?string $targetType = null, ?string $targetId = null, ?string $details = null): void
{
    try {
        ...
        error_log('SmartToolz admin audit: ' . $e->getMessage());
    }
}
