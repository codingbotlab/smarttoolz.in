...
/* Registry URLs are relative to the site root. Normalize them exactly once. */
$toolPath='/'.ltrim((string)$toolPath,'/');
if (str_starts_with($toolPath,'/smart-toolz/smart-toolz/')) {
    $toolPath=substr($toolPath,strlen('/smart-toolz/'));
...
?>
<a href="<?= htmlspecialchars($toolPath,ENT_QUOTES,'UTF-8') ?>" class="sidebar-tool <?= $active?'active':'' ?>"><span class="sidebar-icon"><?= htmlspecialchars($icon,ENT_QUOTES,'UTF-8') ?></span><span class="sidebar-name"><?= htmlspecialchars($name,ENT_QUOTES,'UTF-8') ?></span><?php if($active): ?><span class="sidebar-active">●</span><?php endif; ?></a>
<?php endforeach; ?>
</div>
</aside>
<style>
