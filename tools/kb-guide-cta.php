...
// The Knowledge Base is outside smart-toolz/tools and stores every guide in
// one shared guide.php registry. Read that registry; do not assume that a
// directory or article.php exists for a tool.
...
// Extract only the actual entries from $guides = [ 'tool-slug' => $G(...), ... ].
$guideSlugs = [];
if (preg_match_all("~['\"]([a-z0-9]+(?:-[a-z0-9]+)*)['\"]\\s*=>\\s*\\$G\\s*\\(~i", $guideSource, $matches)) {
    $guideSlugs = array_values(array_unique($matches[1]));
}

// Never create a guessed/fake guide URL.
if (!in_array($slug, $guideSlugs, true)) return;

$url = '/knowledge-base/guide.php?tool=' . rawurlencode($slug);
