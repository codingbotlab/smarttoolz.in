 ...
 * Custom SmartToolz auth pages; WordPress default login remains untouched.
 */
...
function smarttoolz_video_auth_post_redirect( $route, $error ) {
    $url = smarttoolz_video_route_url( $route );
    wp_safe_redirect( add_query_arg( 'stv_auth_error', $error, $url ) );
    exit;
}
...
function smarttoolz_video_process_auth_forms() {
    ...
    if ( 'login' === $action && 'login' === $route ) {
        ...
        wp_set_auth_cookie( $signed->ID, $remember );
    ...
    if ( 'register' === $action && 'signup' === $route ) {
        ...
        if ( strlen( $password ) < 8 ) { smarttoolz_video_auth_post_redirect( 'signup', 'weak_password' ); }
        ...
        wp_safe_redirect( smarttoolz_video_route_url( 'channel', $username ) );
        exit;
    }
}
...
function smarttoolz_video_google_callback() {
    ...
    wp_safe_redirect( wp_validate_redirect( $redirect_to, smarttoolz_video_route_url( 'home' ) ) );
    exit;
}
add_action( 'init', 'smarttoolz_video_google_callback', 1 );

function smarttoolz_video_find_google_user( $google_id, $email ) {
    $users = get_users( array( 'meta_key' => '_smarttoolz_google_id', 'meta_value' => $google_id, 'number' => 1, 'fields' => 'ids' ) );
...
function smarttoolz_video_auth_error_message() {
    ...
    $messages = array(
        ...
        'invalid_form' => 'Please try the form again.',
        'missing_login' => 'Enter your email/username and password.',
        'invalid_login' => 'Incorrect email/username or password.',
        'missing_register' => 'Please fill in all registration fields.',
        'invalid_username' => 'Username must be at least 3 characters and use letters, numbers, dots, hyphens or underscores.',
        'username_taken' => 'That username is already taken.',
        'email_taken' => 'An account with that email already exists. Please sign in.',
...
function smarttoolz_video_render_login() {
    ...
    $route = get_query_var( 'smarttoolz_video_route', 'login' );
    if ( 'signup' === $route ) { return smarttoolz_video_render_register(); }
    $redirect_to = wp_get_referer() ? wp_get_referer() : smarttoolz_video_route_url( 'home' );
    $google_url = smarttoolz_video_google_login_url( $redirect_to );
    ...
    <section class="stv-auth"><div class="stv-auth__card">
        ...
        <form method="post" class="stv-auth-form" action="<?php echo esc_url( smarttoolz_video_route_url( 'login' ) ); ?>">
            ...
            <label>Email or username<input type="text" name="stv_login_identifier" autocomplete="username" required></label>
            <label>Password<input type="password" name="stv_login_password" autocomplete="current-password" required></label>
            <label class="stv-check"><input type="checkbox" name="stv_login_remember" value="1"> Remember me</label>
            <button class="stv-auth-submit" type="submit">Sign in</button>
        </form>
        ...
        <div class="stv-auth-switch">Don't have an account? <a href="<?php echo esc_url( smarttoolz_video_route_url( 'signup' ) ); ?>">Create account</a></div>
        <div class="stv-auth__footer">SmartToolz account • WordPress default login is not used.</div>
    </div></section>
    <?php
}
...
function smarttoolz_video_render_register() {
    ...
    ?>
    <section class="stv-auth"><div class="stv-auth__card">
        <span class="stv-auth__mark">▶</span><h1>Create your SmartToolz account</h1><p>Join SmartToolz, create your own channel and start sharing videos.</p>
        <?php if ( $error ) : ?><div class="stv-notice stv-notice--error"><?php echo esc_html( $error ); ?></div><?php endif; ?>
        <form method="post" class="stv-auth-form" action="<?php echo esc_url( smarttoolz_video_route_url( 'signup' ) ); ?>">
            ...
            <label>Full name<input type="text" name="stv_register_name" autocomplete="name" required></label>
            <label>Username / channel handle<input type="text" name="stv_register_username" autocomplete="username" pattern="[A-Za-z0-9_.-]{3,}" required><small>Your channel URL will use @username.</small></label>
            <label>Email<input type="email" name="stv_register_email" autocomplete="email" required></label>
            <label>Password<input type="password" name="stv_register_password" autocomplete="new-password" minlength="8" required></label>
            <label>Confirm password<input type="password" name="stv_register_confirm" autocomplete="new-password" minlength="8" required></label>
            <button class="stv-auth-submit" type="submit">Create account</button>
        </form>
        <div class="stv-divider"><span>or</span></div>
        <?php $google_url = smarttoolz_video_google_login_url( smarttoolz_video_route_url( 'home' ) ); if ( $google_url ) : ?><a class="stv-google-button" href="<?php echo esc_url( $google_url ); ?>"><span class="stv-google-button__g">G</span><span>Sign up with Google</span></a><?php endif; ?>
...
function smarttoolz_video_auth_settings_page() {
    ...
    $s = smarttoolz_video_auth_settings();
    ?>
    <div class="wrap"><h1>Google Login</h1><p>Configure Google OAuth for the custom SmartToolz login and registration pages. WordPress <code>/wp-login.php</code> is not modified.</p>
    ...
    <tr><th>Enable Google Login</th><td><label><input type="checkbox" name="smarttoolz_video_auth_settings[google_enabled]" value="1" <?php checked( $s['google_enabled'], 1 ); ?>> Allow Google sign-in/sign-up</label></td></tr>
    <tr><th>Google Client ID</th><td><input type="text" class="regular-text" name="smarttoolz_video_auth_settings[google_client_id]" value="<?php echo esc_attr( $s['google_client_id'] ); ?>" autocomplete="off"></td></tr>
    <tr><th>Google Client Secret</th><td><input type="password" class="regular-text" name="smarttoolz_video_auth_settings[google_client_secret]" value="" autocomplete="new-password"><p class="description">Leave blank to keep the existing secret.</p></td></tr>
    <tr><th>Authorized redirect URI</th><td><code><?php echo esc_html( smarttoolz_video_auth_redirect_uri() ); ?></code></td></tr></table><?php submit_button( 'Save Google Login Settings' ); ?></form>
    <hr><h2>Status</h2><p><strong><?php echo smarttoolz_video_google_ready() ? 'Configured and ready' : 'Not configured'; ?></strong></p>
    </div><?php
}
