...
function smarttoolz_auth_assets() {
    ...
    wp_enqueue_style(
        ...
        array(),
        SMARTTOOLZ_VIDEO_VERSION . '.auth3'
    );
}
...
function smarttoolz_auth_google_button_markup() {
    ...
    window.smarttoolzGoogleCredential = function(response) {
        ...
        .then(function(r){ return r.json(); }).then(function(result){
            if (result && result.success && result.data && result.data.redirect) window.location.href = result.data.redirect;
            else alert((result && result.data && result.data.message) || 'Google sign-in failed.');
...
function smarttoolz_google_verify_id_token( $token ) {
    ...
    foreach ( $keys['keys'] as $key ) {
        if ( isset( $key['kid'], $key['x509c'][0] ) && $key['kid'] === $header['kid'] ) { $cert = $key['x509c'][0]; break; }
    }
    ...
    $valid = 1 === openssl_verify( $parts[0] . '.' . $parts[1], $signature, $public_key, OPENSSL_ALGO_SHA256 );
...
function smarttoolz_google_unique_username( $email, $sub ) {
    ...
    $candidate = $base; $i = 1;
    while ( username_exists( $candidate ) ) { $candidate = $base . $i; $i++; if ( $i > 9999 ) { $candidate = 'google' . substr( preg_replace( '/[^a-zA-Z0-9]/', '', $sub ), 0, 16 ); break; } }
    return $candidate;
}
...
function smarttoolz_google_login_ajax() {
    ...
    $email = sanitize_email( $claims['email'] ); $sub = sanitize_text_field( $claims['sub'] );
    ...
    if ( ! $user ) {
        ...
        if ( $existing_id ) { $user = get_user_by( 'id', $existing_id ); update_user_meta( $user->ID, '_smarttoolz_google_sub', $sub ); }
            ...
            if ( is_wp_error( $user_id ) ) wp_send_json_error( array( 'message' => 'Could not create your SmartToolz account.' ), 500 );
