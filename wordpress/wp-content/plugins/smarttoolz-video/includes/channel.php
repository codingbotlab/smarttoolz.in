...
function smarttoolz_video_channel_render( $route ) {
    ...
    <section class="stv-channel">
        ...
        <nav class="stv-channel__tabs" aria-label="Channel navigation">
            <?php foreach ( $tabs as $key => $tab ) : ?><a class="stv-channel__tab <?php echo $route === $key ? 'stv-channel__tab--active' : ''; ?>" href="<?php echo esc_url( $tab[1] ); ?>"><?php echo esc_html( $tab[0] ); ?></a><?php endforeach; ?>
        </nav>
        <div class="stv-channel__section">
            <?php if ( 'channel' === $route ) : ?>
                <?php if ( $profile['description'] ) : ?><p><?php echo esc_html( $profile['description'] ); ?></p><?php endif; ?>
                <?php if ( ! empty( $profile['links'] ) ) : ?><div class="stv-channel__links"><?php foreach ( $profile['links'] as $link ) : ?><a class="stv-channel__link" href="<?php echo esc_url( $link['url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html( $link['title'] ); ?></a><?php endforeach; ?></div><?php endif; ?>
                <?php if ( $profile['contact'] ) : ?><div class="stv-channel__contact">Business contact: <?php echo esc_html( $profile['contact'] ); ?></div><?php endif; ?>
                <h2 style="margin-top:28px;">For you</h2>
            ...
            <?php if ( 'channel' !== $route && 'channel-live' !== $route && 'channel-shorts' !== $route ) : ?>
...
function smarttoolz_video_channel_template_redirect() {
    $r = get_query_var( 'smarttoolz_video_route', '' );
    if ( ! in_array( $r, array( 'channel', 'channel-videos', 'channel-shorts', 'channel-live' ), true ) ) { return; }
