...
function smarttoolz_video_route_slug( $key ) {
    ...
    return isset( $settings[ $key ]['slug'] ) ? $settings[ $key ]['slug'] : $defaults[ $key ]['slug'];
}

function smarttoolz_video_route_enabled( $key ) {
    $settings = smarttoolz_video_route_settings();
    $defaults = smarttoolz_video_route_defaults();
    return isset( $settings[ $key ]['enabled'] ) ? ! empty( $settings[ $key ]['enabled'] ) : ! empty( $defaults[ $key ]['enabled'] );
}
...
function smarttoolz_video_sanitize_route_settings( $input ) {
    ...
    foreach ( $defaults as $key => $default ) {
        ...
        $out[ $key ] = array(
            'label' => $default['label'],
            'slug' => $slug,
            'enabled' => empty( $input[ $key ]['enabled'] ) ? 0 : 1,
        );
    }

    // The platform root must remain available so the custom app has a stable entry point.
    $out['home']['enabled'] = 1;
    return $out;
}
...
function smarttoolz_video_routing_settings_page() {
    ...
    if ( isset( $_POST['stv_routing_action'], $_POST['stv_routing_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['stv_routing_nonce'] ) ), 'stv_routing_save' ) ) {
        ...
        if ( 'reset' === $action ) {
            update_option( 'smarttoolz_video_route_settings', $defaults, false );
            delete_option( 'rewrite_rules' );
            $settings = $defaults;
            $notice = '<div class="notice notice-success is-dismissible"><p>SmartToolz URL mappings were reset to defaults.</p></div>';
        } elseif ( 'save' === $action ) {
            $raw = isset( $_POST['stv_routes'] ) ? wp_unslash( $_POST['stv_routes'] ) : array();
            $settings = smarttoolz_video_sanitize_route_settings( $raw );
            update_option( 'smarttoolz_video_route_settings', $settings, false );
            delete_option( 'rewrite_rules' );
            $notice = '<div class="notice notice-success is-dismissible"><p>URL mappings and routing settings saved. New routes are active after WordPress rebuilds its rewrite rules.</p></div>';
        }
    }
    ?>
    <div class="wrap">
        <h1>URL Mapping &amp; Routing</h1>
        <p>Control SmartToolz Video URLs from here. This does not modify WordPress core or the default <code>/wp-login.php</code> page.</p>
        <?php echo $notice; ?>
        <div style="max-width:1100px;background:#fff;border:1px solid #dcdcde;border-radius:10px;padding:18px;margin-top:16px;">
            <form method="post">
                <?php wp_nonce_field( 'stv_routing_save', 'stv_routing_nonce' ); ?>
                <input type="hidden" name="stv_routing_action" value="save">
                <table class="widefat striped">
                    <thead><tr><th style="width:110px;">Enabled</th><th>Route</th><th style="width:260px;">URL Segment</th><th>Preview</th></tr></thead>
                    <tbody>
                    <?php foreach ( $defaults as $key => $default ) :
                        $row = isset( $settings[ $key ] ) ? $settings[ $key ] : $default;
                        $preview = smarttoolz_video_routing_preview_path( $key, $settings );
                    ?>
                        <tr>
                            <td><label><input type="checkbox" name="stv_routes[<?php echo esc_attr( $key ); ?>][enabled]" value="1" <?php checked( ! empty( $row['enabled'] ) ); ?> <?php disabled( 'home', $key ); ?>> <strong><?php echo 'home' === $key ? 'Always' : 'On'; ?></strong></label></td>
                            <td><strong><?php echo esc_html( $default['label'] ); ?></strong><br><code><?php echo esc_html( $key ); ?></code></td>
                            <td><input type="text" class="regular-text" name="stv_routes[<?php echo esc_attr( $key ); ?>][slug]" value="<?php echo esc_attr( $row['slug'] ); ?>" pattern="[A-Za-z0-9_-]+" <?php disabled( 'home', $key ); ?>></td>
                            <td><code><?php echo esc_html( home_url( '/' . $preview . ( 'watch' === $key ? '/123/' : '/' ) ) ); ?></code></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
                <p class="description" style="margin-top:12px;">Use URL-safe words only. Nested routes automatically follow their parent mapping. Example: changing Channel to <code>creator</code> makes Channel Videos become <code>/video/creator/videos/</code>.</p>
                <?php submit_button( 'Save URL & Routing Settings', 'primary', 'submit', false ); ?>
            </form>
            <form method="post" style="display:inline-block;margin-left:8px;">
                <?php wp_nonce_field( 'stv_routing_save', 'stv_routing_nonce' ); ?>
                <input type="hidden" name="stv_routing_action" value="reset">
                <?php submit_button( 'Reset Defaults', 'secondary', 'submit', false, array( 'onclick' => "return confirm('Reset all SmartToolz URL mappings to defaults?');" ) ); ?>
            </form>
        </div>
    </div>
    <?php
...
function smarttoolz_video_routing_preview_path( $key, $settings = null ) {
    ...
    if ( 'watch' === $key ) { return trim( $settings['home']['slug'] . '/' . $settings['watch']['slug'], '/' ); }

    $parents = array(
        'channel-videos' => 'channel', 'channel-shorts' => 'channel', 'channel-live' => 'channel',
        'edit-profile' => 'account', 'comments' => 'account', 'badges' => 'account', 'podcasts' => 'account', 'privacy' => 'account',
    );
    $parent = isset( $parents[ $key ] ) ? $parents[ $key ] : 'home';
    $parent_path = smarttoolz_video_routing_preview_path( $parent, $settings );
    return trim( $parent_path . '/' . $settings[ $key ]['slug'], '/' );
}
