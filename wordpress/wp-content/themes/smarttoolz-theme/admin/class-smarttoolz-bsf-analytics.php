<?php
/**
 * SmartToolz BSF Analytics class helps to connect BSF Analytics.
 *
 * @package smarttoolz.
 */
...
/**
 * SmartToolz BSF Analytics class.
 *
 * @since 4.10.0
 */
class SmartToolz_BSF_Analytics {
	/**
	 * Instance object.
	 *
	...
	public function __construct() {
		...
		*/
		if ( ! class_exists( 'BSF_Analytics_Loader' ) ) {
			require_once SMARTTOOLZ_THEME_DIR . 'inc/lib/bsf-analytics/class-bsf-analytics-loader.php';
		}
		if ( ! class_exists( 'BSF_Analytics_Events' ) ) {
			require_once SMARTTOOLZ_THEME_DIR . 'inc/lib/bsf-analytics/class-bsf-analytics-events.php';
		}
		self::$events = new \BSF_Analytics_Events( 'smarttoolz' );

		add_action( 'init', array( $this, 'init_bsf_analytics' ), 5 );
		add_filter( 'bsf_core_stats', array( $this, 'add_smarttoolz_analytics_data' ) );

		// Track SmartToolz customizer publish events for kpi and one-time event tracking.
		add_action( 'smarttoolz_customizer_save', array( $this, 'maybe_save_customizer_published_timestamp' ) );

		// Track theme version updates.
		add_action( 'smarttoolz_theme_update_after', array( $this, 'track_theme_updated' ) );

		// Track onboarding completion/skip in real-time via One Onboarding hooks.
		add_action( 'one_onboarding_completion_smarttoolz', array( $this, 'track_onboarding_completed' ) );
		add_action( 'one_onboarding_state_saved_smarttoolz', array( $this, 'track_onboarding_skipped' ) );

		// Track admin settings toggles.
		add_action( 'update_option_smarttoolz_admin_settings', array( $this, 'track_admin_settings_changes' ), 10, 2 );

		// Track learn chapter progress.
		add_action( 'smarttoolz_learn_progress_saved', array( $this, 'track_learn_chapter_progress' ) );
	...
	public function init_bsf_analytics() {
		// Bail early if BSF_Analytics_Loader::get_instance is not callable and if SmartToolz white labelling is enabled.
		if ( ! is_callable( '\BSF_Analytics_Loader::get_instance' ) || smarttoolz_is_white_labelled() ) {
			return;
		}
		...
		$bsf_analytics->set_entity(
			array(
				'smarttoolz' => array(
					'product_name'        => 'SmartToolz',
					'path'                => SMARTTOOLZ_THEME_DIR . 'inc/lib/bsf-analytics',
					...
					'deactivation_survey' => apply_filters(
						'smarttoolz_deactivation_survey_data',
						array(
							// Kept it for future reference.
							// array(
							// 'id'                => 'deactivation-survey-smarttoolz',
							// 'popup_logo'        => SMARTTOOLZ_THEME_URI . 'inc/assets/images/smarttoolz-logo.svg',
							// 'plugin_slug'       => 'smarttoolz',
							// 'popup_title'       => __( 'Quick Feedback', 'smarttoolz' ),
							// 'support_url'       => 'https://wpsmarttoolz.com/contact/',
							// 'popup_description' => __( 'If you have a moment, please share why you are deactivating SmartToolz:', 'smarttoolz' ),
							// 'show_on_screens'   => array( 'themes' ),
							// 'plugin_version'    => SMARTTOOLZ_THEME_VERSION,
	...
	public static function get_default_reasons() {
		return array(
			'temporary_deactivation' => array(
				'label'           => esc_html__( 'This is a temporary deactivation for testing.', 'smarttoolz' ),
				'placeholder'     => esc_html__( 'How can we assist you?', 'smarttoolz' ),
				'show_cta'        => 'false',
				'accept_feedback' => 'false',
			),
			'theme_not_working'      => array(
				'label'           => esc_html__( 'The theme isn\'t working properly.', 'smarttoolz' ),
				'placeholder'     => esc_html__( 'Please tell us more about what went wrong?', 'smarttoolz' ),
				'show_cta'        => 'true',
				'accept_feedback' => 'true',
			),
			'found_better_theme'     => array(
				'label'           => esc_html__( 'I found a better alternative theme.', 'smarttoolz' ),
				'placeholder'     => esc_html__( 'Could you please specify which theme?', 'smarttoolz' ),
				'show_cta'        => 'false',
				'accept_feedback' => 'true',
			),
			'missing_a_feature'      => array(
				'label'           => esc_html__( 'It\'s missing a specific feature.', 'smarttoolz' ),
				'placeholder'     => esc_html__( 'Please tell us more about the feature.', 'smarttoolz' ),
				'show_cta'        => 'false',
				'accept_feedback' => 'true',
			),
			'other'                  => array(
				'label'           => esc_html__( 'Other', 'smarttoolz' ),
				'placeholder'     => esc_html__( 'Please tell us more details.', 'smarttoolz' ),
				'show_cta'        => 'false',
				'accept_feedback' => 'true',
			),
		);
	...
	/**
	 * Callback function to add SmartToolz specific analytics data.
	 *
	 ...
	 * @return array
	 */
	public function add_smarttoolz_analytics_data( $stats_data ) {
		if ( ! isset( $stats_data['plugin_data']['smarttoolz'] ) ) {
			$stats_data['plugin_data']['smarttoolz'] = array();
		}

		$bsf_internal_referrer    = get_option( 'bsf_product_referers', array() );
		$admin_dashboard_settings = get_option( 'smarttoolz_admin_settings', array() );
		$is_hf_builder_active     = class_exists( 'SmartToolz_Builder_Helper' ) ? SmartToolz_Builder_Helper::$is_header_footer_builder_active : true;

		$smarttoolz_stats = array(
			'free_version'                 => SMARTTOOLZ_THEME_VERSION,
			'site_language'                => get_locale(),
			'numeric_values'               => array(),
			'boolean_values'               => array(
				'pro_active'             => defined( 'SMARTTOOLZ_EXT_VER' ),
				'smarttoolz_sites_active'     => is_plugin_active( 'smarttoolz-sites/smarttoolz-sites.php' ),
				'smarttoolz_pro_sites_active' => is_plugin_active( 'smarttoolz-pro-sites/smarttoolz-pro-sites.php' ),
				'is_using_dark_palette'  => SmartToolz_Global_Palette::is_dark_palette(),
			),
			'internal_referrer'            => empty( $bsf_internal_referrer['smarttoolz'] ) ? '' : $bsf_internal_referrer['smarttoolz'],
		...
		self::add_smarttoolz_onboarding_analytics_data( $smarttoolz_stats );

		// Add learn progress analytics data.
		self::add_learn_progress_analytics_data( $smarttoolz_stats );

		// Add KPI tracking data.
		self::add_kpi_tracking_data( $smarttoolz_stats );

		$stats_data['plugin_data']['smarttoolz'] = array_merge_recursive( $stats_data['plugin_data']['smarttoolz'], $smarttoolz_stats );
		...
		self::add_events_tracking_data( $stats_data['plugin_data']['smarttoolz'] );

		return $stats_data;
	}

	/**
	 * Add SmartToolz onboarding analytics data.
	 *
	 * @param array $smarttoolz_stats Reference to the smarttoolz stats data.
	 *
	 * @since 4.11.12
	 * @return array
	 */
	public static function add_smarttoolz_onboarding_analytics_data( &$smarttoolz_stats ) {
		// Get onboarding analytics data from option.
		/** @psalm-suppress UndefinedClass */
		$option_name     = is_callable( '\One_Onboarding\Core\Register::get_option_name' )
			? \One_Onboarding\Core\Register::get_option_name( 'smarttoolz' )
			: 'smarttoolz_onboarding';
		...
		if ( isset( $onboarding_data['screens'] ) && is_array( $onboarding_data['screens'] ) ) {
			...
			$smarttoolz_stats['onboarding_skipped_screens'] = $skipped_screens;
		}

		// Process pro features.
		if ( isset( $onboarding_data['pro_features'] ) && is_array( $onboarding_data['pro_features'] ) ) {
			$smarttoolz_stats['onboarding_selected_pro_features'] = $onboarding_data['pro_features'];
		}

		// Process selected starter templates builder.
		$smarttoolz_stats['onboarding_selected_st_builder'] = isset( $onboarding_data['starter_templates_builder'] ) ? $onboarding_data['starter_templates_builder'] : '';

		// Process selected addons
		if ( isset( $onboarding_data['selected_addons'] ) && is_array( $onboarding_data['selected_addons'] ) ) {
			$smarttoolz_stats['onboarding_selected_addons'] = $onboarding_data['selected_addons'];
		}

		// Onboarding Completion Status.
		$smarttoolz_stats['boolean_values']['onboarding_completed'] = isset( $onboarding_data['completion_screen'] ) && ! empty( $onboarding_data['completion_screen'] );
		if ( $smarttoolz_stats['boolean_values']['onboarding_completed'] ) {
			$smarttoolz_stats['onboarding_completion_screen'] = isset( $onboarding_data['completion_screen'] ) ? $onboarding_data['completion_screen'] : '';
		}

		// Onboarding Exit Status.
		if ( isset( $onboarding_data['exited_early'] ) ) {
			$smarttoolz_stats['boolean_values']['onboarding_exited_early'] = (bool) $onboarding_data['exited_early'];
		}
	}

	/**
	 * Add SmartToolz learn progress analytics data.
	 *
	 * This function retrieves the smarttoolz_learn_progress user meta from ALL users
	 ...
	 * @return void
	 */
	public static function add_learn_progress_analytics_data( &$smarttoolz_stats ) {
		global $wpdb;
		...
		$chapters = SmartToolz_Learn::get_chapters_structure();
		...
		// Add to smarttoolz stats if we have completed chapters.
		if ( ! empty( $completed_chapters ) ) {
			$smarttoolz_stats['learn_chapters_completed'] = array_values( array_unique( $completed_chapters ) );
		}
	}
	 ...
	 *
	 * This function checks if SmartToolz customizer settings were modified during the customizer save event.
	 * If so, it records the current timestamp in the '_smarttoolz_customizer_published_timestamps' option for KPI tracking
	...
	public function maybe_save_customizer_published_timestamp() {
		...
		// Bail if customizer manager not available or no SmartToolz customizer settings modified.
		if ( ! $wp_customize || ! self::has_smarttoolz_customizer_settings_modified( $wp_customize ) ) {
			return;
		}

		$timestamps = get_option( '_smarttoolz_customizer_published_timestamps', array() );
		if ( ! is_array( $timestamps ) ) {
			$timestamps = array();
		}

		$timestamps[] = time();
		update_option( '_smarttoolz_customizer_published_timestamps', $timestamps, false );

		// Track first customizer publish as a one-time event.
		self::$events->track( 'first_customizer_published', SMARTTOOLZ_THEME_VERSION );
	}

	/**
	 * Check if any SmartToolz-specific settings were modified in the customizer.
	 *
	 * @param WP_Customize_Manager $wp_customize The customizer manager instance.
	 *
	 * @since 4.12.2
	 * @return bool True if SmartToolz customizer settings were modified, false otherwise.
	 */
	public static function has_smarttoolz_customizer_settings_modified( $wp_customize ) {
		$posted_values = $wp_customize->unsanitized_post_values();

		// Check if any setting key starts with 'smarttoolz-' to identify SmartToolz customizer settings.
		foreach ( $posted_values as $setting_id => $setting_value ) {
			if ( strpos( $setting_id, 'smarttoolz-' ) === 0 ) {
				return true;
			}
		}
	 ...
	 * @return void
	 */
	public static function add_kpi_tracking_data( &$smarttoolz_stats ) {
		$timestamps = get_option( '_smarttoolz_customizer_published_timestamps', array() );
		...
		if ( ! empty( $kpi_data ) ) {
			$smarttoolz_stats['kpi_records'] = $kpi_data;
		}
		...
		if ( ! empty( $timestamps_to_cleanup ) ) {
			...
			update_option( '_smarttoolz_customizer_published_timestamps', array_values( $remaining_timestamps ), false );
		}
	}
	 ...
	 * @return void
	 */
	private static function add_events_tracking_data( &$smarttoolz_stats ) {
		...
		$source        = ! empty( $bsf_referrers['smarttoolz'] ) ? $bsf_referrers['smarttoolz'] : 'self';
		...
		if ( ! isset( $smarttoolz_stats['events_record'] ) ) {
			$smarttoolz_stats['events_record'] = array();
		}

		// Flush pending events into the payload.
		$existing = isset( $smarttoolz_stats['events_record'] ) ? $smarttoolz_stats['events_record'] : array();
		$flushed  = self::$events->flush_pending();

		if ( ! empty( $existing ) || ! empty( $flushed ) ) {
			$smarttoolz_stats['events_record'] = array_merge( $existing, $flushed );
		}
	}
	 ...
	 *
	 * Fired by `one_onboarding_completion_smarttoolz` hook when user finishes onboarding.
	 *
	...
	public function track_onboarding_completed( $completion_data ) {
		...
		self::$events->track( 'onboarding_completed', SMARTTOOLZ_THEME_VERSION, $properties, true );
	}

	/**
	 * Track onboarding skipped event.
	 *
	 * Fired by `one_onboarding_state_saved_smarttoolz` hook when user exits onboarding early.
	...
	public function track_onboarding_skipped( $state_data ) {
		...
		self::$events->track( 'onboarding_skipped', SMARTTOOLZ_THEME_VERSION, $properties, true );
	}

	/**
	 * Track admin settings changes for learn tab, local fonts, and abilities toggles.
	 *
	 * Fired by `update_option_smarttoolz_admin_settings` hook. Re-trackable since
	 ...
	 *
	 * Fires on `smarttoolz_learn_progress_saved`. Compares chapter completion state
	...
	public function track_learn_chapter_progress( $saved_progress ) {
		if ( empty( $saved_progress ) || ! class_exists( 'SmartToolz_Learn' ) ) {
			return;
		}
	...
	/**
	 * Track use of an SmartToolz ability as a re-trackable event.
	 *
	 * Hooked to wp_after_execute_ability, which fires past the permission check and
	 * carries the result. SmartToolz abilities return an SmartToolz_Abilities_Response array
	 * rather than a WP_Error, so soft failures are visible here as 'success' => false.
	 *
	 * @param string $ability_name Ability name, e.g. 'smarttoolz/get-font-body'.
	...
	public function track_ability_used( $ability_name, $input, $result ) {
		// This action fires for every registered ability, not only SmartToolz's.
		if ( 0 !== strpos( $ability_name, 'smarttoolz/' ) ) {
			return;
		}
	...
	/**
	 * Get days since SmartToolz was installed.
	 *
	 * @since 4.12.7
	 * @return int Number of days since install.
	 */
	private static function get_days_since_install() {
		$install_time = get_site_option( 'smarttoolz_usage_installed_time', 0 );
		if ( $install_time > 0 ) {
...
/**
 * Initiates the SmartToolz_BSF_Analytics class instance.
 */
SmartToolz_BSF_Analytics::get_instance();
