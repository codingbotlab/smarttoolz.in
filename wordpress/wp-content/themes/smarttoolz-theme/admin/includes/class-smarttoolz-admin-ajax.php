...
class SmartToolz_Admin_Ajax {
	...
	public function __construct() {
		add_action(
			...
			function() {
				$this->errors = array(
					'permission' => esc_html__( 'Sorry, you are not allowed to do this operation.', 'smarttoolz' ),
					'nonce'      => esc_html__( 'Nonce validation failed', 'smarttoolz' ),
					'default'    => esc_html__( 'Sorry, something went wrong.', 'smarttoolz' ),
					'invalid'    => esc_html__( 'No post data found!', 'smarttoolz' ),
				);
			}
		);

		add_action( 'wp_ajax_ast_disable_pro_notices', array( $this, 'disable_smarttoolz_pro_notices' ) );
		add_action( 'wp_ajax_smarttoolz_recommended_plugin_install', array( $this, 'required_plugin_install' ) );
		add_action( 'wp_ajax_ast_migrate_to_builder', array( $this, 'migrate_to_builder' ) );
		add_action( 'wp_ajax_smarttoolz_update_admin_setting', array( $this, 'smarttoolz_update_admin_setting' ) );
		add_action( 'wp_ajax_smarttoolz_analytics_optin_status', array( $this, 'smarttoolz_analytics_optin_status' ) );
		add_action( 'wp_ajax_smarttoolz_recommended_plugin_activate', array( $this, 'required_plugin_activate' ) );
		add_action( 'wp_ajax_smarttoolz_recommended_plugin_deactivate', array( $this, 'required_plugin_deactivate' ) );
		add_action( 'wp_ajax_smarttoolz_load_google_fonts', array( $this, 'load_google_fonts' ) );
	}
	 ...
	 * @since 4.0.0
	 */
	public function smarttoolz_admin_settings_typewise() {
		return apply_filters(
			'smarttoolz_admin_settings_datatypes',
			array(
	...
	public function disable_smarttoolz_pro_notices() {
		 ...
		 * Nonce verification.
		 */
		if ( ! check_ajax_referer( 'smarttoolz_update_admin_setting', 'security', false ) ) {
	 ...
	 * @since 4.0.0
	 */
	public function smarttoolz_update_admin_setting() {
		...
		$get_bool_settings = $this->smarttoolz_admin_settings_typewise();
		/** @psalm-suppress PossiblyInvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		if ( isset( $get_bool_settings[ $sub_option_key ] ) ) {
			...
			SmartToolz_API_Init::update_admin_settings_option( $sub_option_key, $sub_option_value );
		}
		// @codingStandardsIgnoreEnd

		$response_data = array(
			'message' => esc_html__( 'Successfully saved data!', 'smarttoolz' ),
		);
	...
	/**
	 * SmartToolz Analytics Opt-in.
	 *
	 * @return void
	 * @since 4.10.0
	 */
	public function smarttoolz_analytics_optin_status() {
		$response_data = array( 'message' => $this->get_error_msg( 'permission' ) );
		...
		update_site_option( 'smarttoolz_usage_optin', $opt_in );
	 ...
	 *
	 * This function is responsible for installing and saving required plugins for the SmartToolz theme.
	...
	public function required_plugin_install() {
		...
		if ( empty( $plugin_slug ) ) {
			wp_send_json_error( array( 'message' => __( 'Plugin slug is missing.', 'smarttoolz' ) ) );
		}
			...
			static function () use ( $plugin_slug ) {
				...
				foreach ( $all_plugins as $plugin_file => $_ ) {
					if ( is_callable( 'BSF_UTM_Analytics::update_referer' ) && strpos( $plugin_file, $plugin_slug . '/' ) === 0 ) {
						...
						BSF_UTM_Analytics::update_referer( 'smarttoolz', $plugin_slug );
						return;
					}
				}
			}
		...
		if ( function_exists( 'wp_ajax_install_plugin' ) ) {
			...
			wp_send_json_error( array( 'message' => __( 'Plugin installation function not found.', 'smarttoolz' ) ) );
		}
	}
	...
	public function required_plugin_activate() {
		 ...
		 * Nonce verification.
		 */
		if ( ! check_ajax_referer( 'smarttoolz_plugin_manager_nonce', 'security', false ) ) {
			$response_data = array( 'message' => $this->get_error_msg( 'nonce' ) );
			wp_send_json_error( $response_data );
		}
		...
		if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! sanitize_text_field( wp_unslash( $_POST['init'] ) ) ) {
			...
			wp_send_json_error(
				array(
					'success' => false,
					'message' => esc_html__( 'No plugin specified', 'smarttoolz' ),
				)
			);
		}
		...
		if ( 'surecart/surecart.php' === $plugin_init ) {
			update_option( 'surecart_source', 'smarttoolz', false );
		}

		wp_send_json_success(
			array(
				'success' => true,
				'message' => esc_html__( 'Plugin Successfully Activated', 'smarttoolz' ),
			)
		);
	}
	...
	public function required_plugin_deactivate() {
		...
		wp_send_json_success(
			array(
				'success' => true,
				'message' => esc_html__( 'Plugin Successfully Deactivated', 'smarttoolz' ),
			)
		);
	}
	...
	public function load_google_fonts() {
		...
		$custom_fonts = SmartToolz_Font_Families::get_custom_fonts();
...
SmartToolz_Admin_Ajax::get_instance();
