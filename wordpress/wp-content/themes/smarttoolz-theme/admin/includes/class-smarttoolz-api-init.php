 ...
 * @since 4.1.0
 */
class SmartToolz_API_Init extends WP_REST_Controller {
	/**
	 * Instance
	 *
	 * @var null $instance
	 ...
	 * @var string
	 */
	protected $namespace = 'smarttoolz/v1';
	 ...
	 * @since 4.0.0
	 */
	private static $option_name = 'smarttoolz_admin_settings';

	/**
	 * Admin settings dataset
	 *
	 * @var array $smarttoolz_admin_settings Settings array.
	 * @since 4.0.0
	 */
	private static $smarttoolz_admin_settings = array();
	...
	public function __construct() {
		self::$smarttoolz_admin_settings = get_option( self::$option_name, array() );
	...
	public function get_admin_settings( $request ) {
		$db_option = get_option( 'smarttoolz_admin_settings', array() );

		$defaults = apply_filters(
			'smarttoolz_dashboard_rest_options',
			array(
				...
				'use_old_header_footer' => smarttoolz_get_option( 'is-header-footer-builder', false ),
				'use_upgrade_notices'   => smarttoolz_showcase_upgrade_notices(),
				'analytics_enabled'     => get_option( 'smarttoolz_usage_optin', 'no' ) === 'yes',
	...
	public function get_learn_chapters( $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		if ( ! is_callable( 'SmartToolz_Learn::get_learn_chapters' ) ) {
			return array();
		}

		// Use SmartToolz_Learn helper to get chapters with progress.
		return SmartToolz_Learn::get_learn_chapters();
	}
	...
	public function save_learn_progress( $request ) {
		...
		// Save to user meta.
		update_user_meta( $user_id, 'smarttoolz_learn_progress', $saved_progress );
		 ...
		 * @since 4.12.7
		 */
		do_action( 'smarttoolz_learn_progress_saved', $saved_progress );

		return new WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Progress saved successfully.', 'smarttoolz' ),
			),
			200
		);
	}
	...
	public function get_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			return new WP_Error( 'smarttoolz_rest_cannot_view', esc_html__( 'Sorry, you cannot list resources.', 'smarttoolz' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public static function get_admin_settings_option( $key, $default = false ) {
		return isset( self::$smarttoolz_admin_settings[ $key ] ) ? self::$smarttoolz_admin_settings[ $key ] : $default;
	}
	...
	public static function update_admin_settings_option( $key, $value ) {
		$smarttoolz_admin_updated_settings         = get_option( self::$option_name, array() );
		$smarttoolz_admin_updated_settings[ $key ] = $value;
		update_option( self::$option_name, $smarttoolz_admin_updated_settings );
		self::$smarttoolz_admin_settings[ $key ] = $value;
	}
}

SmartToolz_API_Init::get_instance();
