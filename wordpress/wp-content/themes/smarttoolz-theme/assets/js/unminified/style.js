 ...
 */
var smarttoolzGetParents = function ( elem, selector ) {
 ...
 * @return {Array}           The parent elements.
 */
var smarttoolzToggleClass = function ( el, className ) {
 ...
 */
var smarttoolzTriggerEvent = function smarttoolzTriggerEvent( el, typeArg ) {
	var customEventInit =
 ...
 */
 smarttoolzSmoothScroll = function smarttoolzSmoothScroll( e, top ) {
	e.preventDefault();
	window.scrollTo({
		top: top,
		left: 0,
...
( function() {
	...
	var updateHeaderBreakPoint = function () {
		...
		var break_point = smarttoolz.break_point,
			...
			for (var i = 0; i < headerWrap.length; i++) {
				...
				if (headerWrap[i].tagName == 'DIV' && headerWrap[i].classList.contains('main-header-bar-wrap')) {
					if (ww > break_point) {
						...
						smarttoolzTriggerEvent(document.body, "smarttoolz-header-responsive-enabled");

					} else {

						document.body.classList.add("ast-header-break-point");
						document.body.classList.remove("ast-desktop");
						smarttoolzTriggerEvent(document.body, "smarttoolz-header-responsive-disabled")
					}
				}
			}
		}
	...
	SmartToolzToggleSubMenu = function() {
		...
		if (parent_li.classList.contains('menu-item-has-children') ) {
			smarttoolzToggleClass(parent_li, 'ast-submenu-expanded');
	...
	SmartToolzNavigationMenu = function( parentList ) {
		console.warn( 'SmartToolzNavigationMenu() function has been deprecated since version 1.6.5 or above of SmartToolz Theme and will be removed in the future.' );
	};

	SmartToolzToggleMenu = function( smarttoolz_menu_toggle ) {
		console.warn('SmartToolzToggleMenu() function has been deprecated since version 1.6.5 or above of SmartToolz Theme and will be removed in the future. Use SmartToolzToggleSubMenu() instead.');

		// Add Eventlisteners for Submenu.
		if (smarttoolz_menu_toggle.length > 0) {
			for (var i = 0; i < smarttoolz_menu_toggle.length; i++) {
				smarttoolz_menu_toggle[i].removeEventListener('click', SmartToolzToggleSubMenu);
				smarttoolz_menu_toggle[i].addEventListener('click', SmartToolzToggleSubMenu, false);
			}
		}
	};

	SmartToolzToggleSetup = function () {
		...
		if (menu_toggle_all.length > 0) {
			...
			for (var i = 0; i < menu_toggle_all.length; i++) {
				...
				if ( ! menu_click_listeners_nav[i] ) {
					...
					menu_toggle_all[i].removeEventListener('click', smarttoolzNavMenuToggle);
					menu_toggle_all[i].addEventListener('click', smarttoolzNavMenuToggle, false);
				}

				if ('undefined' !== typeof __main_header_all[i]) {

					if (document.querySelector("header.site-header").classList.contains("ast-menu-toggle-link")) {
						var smarttoolz_menu_toggle = __main_header_all[i].querySelectorAll('.ast-header-break-point .main-header-menu .menu-item-has-children > .menu-link, .ast-header-break-point ul.main-header-menu .ast-menu-toggle');
					} else {
						var smarttoolz_menu_toggle = __main_header_all[i].querySelectorAll('ul.main-header-menu .ast-menu-toggle');
					}

					// Add Eventlisteners for Submenu.
					if (smarttoolz_menu_toggle.length > 0) {
						for (var j = 0; j < smarttoolz_menu_toggle.length; j++) {
							smarttoolz_menu_toggle[j].removeEventListener('click', SmartToolzToggleSubMenu);
							smarttoolz_menu_toggle[j].addEventListener('click', SmartToolzToggleSubMenu, false);
						}
					}
	...
	smarttoolzNavMenuToggle = function ( event ) {
		...
		if ( menu_class.indexOf('main-header-menu-toggle') !== -1 ) {
			smarttoolzToggleClass(__main_header_all[event_index], 'toggle-on');
			smarttoolzToggleClass(menu_toggle_all[event_index], 'toggled');
	...
	document.body.addEventListener("smarttoolz-header-responsive-enabled", function () {
	...
	/* Search Script */
	var SearchIcons = document.getElementsByClassName( 'smarttoolz-search-icon' );
	...
	SearchInputs.forEach(input => {
		input.addEventListener('focus', function (e) {
			var sibling = this.parentNode.parentNode.parentNode.querySelector( '.ast-search-menu-icon' );
			smarttoolzToggleClass( sibling, 'ast-dropdown-active' );
		});
    ...
    function toggleClose( )
    {
        ...
        if( self && ! self.classList.contains('smarttoolz-search-icon') ) {
            ...
            if( link.indexOf( hash ) !== -1 ) {
                ...
                if ( document.body.classList.contains('ast-header-break-point') && ! ( document.querySelector("header.site-header").classList.contains("ast-menu-toggle-link") && link_parent.classList.contains("menu-item-has-children") ) ) {
					...
					smarttoolzTriggerEvent( document.querySelector('body'), 'smarttoolzMenuHashLinkClicked' );
                } else {
	...
	if ( smarttoolz.is_scroll_to_id ) {
		...
		const scrollToIDHandler = ( e, hash = null ) => {
			...
			if (siteHeader) {
				...
				} else if ( typeof smarttoolzAddon !== 'undefined' && ! ( Number( smarttoolzAddon.sticky_hide_on_scroll ) && ! document?.querySelector( '.ast-header-sticked' ) ) ) {
					const fixedHeader = document.querySelector( '#ast-fixed-header' );
					if ( fixedHeader ) {
						offset = fixedHeader?.clientHeight;
						if ( Number( smarttoolzAddon?.header_main_shrink ) ) {
				...
				if (href) {
					...
					if (scrollId) {
						...
						if ( typeof smarttoolzAddon !== 'undefined' && Number( smarttoolzAddon.sticky_hide_on_scroll ) && window?.scrollY  < elementOffsetTop ) {
							offset = 0;
						}

						const scrollOffsetTop = elementOffsetTop - offset;
						if( scrollOffsetTop ) {
							smarttoolzSmoothScroll( e, scrollOffsetTop );
						}
					}
				}
			}
			...
			for (let link of hashLinks) {
				if (window.location.href.split('#')[0] === link.url) {
					...
					if (scrollId) {
						...
						if (scrollOffsetTop) {
							smarttoolzSmoothScroll(event, scrollOffsetTop);
						}
					}
				}
			}
	 ...
	 * @since x.x.x
	 */
	if ( smarttoolz.is_scroll_to_top ) {
