...
define( 'SMARTTOOLZ_VERSION', '3.0.0' );
...
function smarttoolz_customize_register( $wp_customize ) {
    ...
    foreach ( $settings as $id => $setting ) {
        $wp_customize->add_setting( $id, array( 'default' => $setting[0], 'sanitize_callback' => $setting[1], 'transport' => 'refresh' ) );
    }
...
function smarttoolz_breadcrumbs() {
    ...
    if ( is_category() ) {
        ...
        echo '<span>' . esc_html( get_the_author_meta( 'display_name' ) ) . '</span>';
    } elseif ( is_single() ) {
...
function smarttoolz_reading_progress() {
    if ( is_singular( 'post' ) && get_theme_mod( 'smarttoolz_show_reading_progress', 1 ) ) {
        echo '<div class="st-reading-progress" aria-hidden="true"><span></span></div>';
    }
}
...
function smarttoolz_schema_meta() {
    ...
    if ( is_single() ) {
        $schema['author'] = array( '@type' => 'Person', 'name' => get_the_author() );
    }
