...
define( 'SMARTTOOLZ_VERSION', '2.3.0' );
...
function smarttoolz_widgets() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'smarttoolz' ), 'id' => 'sidebar-1',
        'description' => __( 'Widgets shown beside posts and pages.', 'smarttoolz' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s st-card">', 'after_widget' => '</section>',
        'before_title' => '<h2 class="widget-title">', 'after_title' => '</h2>',
    ) );
    register_sidebar( array(
        'name' => __( 'Footer Area', 'smarttoolz' ), 'id' => 'footer-1',
...
function smarttoolz_fallback_menu() {
    echo '<ul class="st-nav-list"><li><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'smarttoolz' ) . '</a></li></ul>';
}
...
function smarttoolz_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_setting( 'smarttoolz_dark_mode', array( 'default' => '0', 'sanitize_callback' => 'sanitize_text_field' ) );
    $wp_customize->add_control( 'smarttoolz_dark_mode', array( 'label' => __( 'Enable Dark Mode Button', 'smarttoolz' ), 'section' => 'smarttoolz_theme_options', 'type' => 'checkbox' ) );
}
...
function smarttoolz_back_to_top() {
    echo '<button type="button" class="st-back-top" aria-label="' . esc_attr__( 'Back to top', 'smarttoolz' ) . '">↑</button>';
}
