...
/**
 * Class SmartToolz_Get_Postmeta
 */
class SmartToolz_Get_Postmeta extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/get-post-meta';
		$this->label       = __( 'Get Post-Specific Meta Settings (Individual Posts/Pages)', 'smarttoolz' );
		$this->description = __( 'Retrieves SmartToolz meta settings for ONE SPECIFIC post or page by post title or ID. Shows current overrides for that individual post only. IMPORTANT: Use this ONLY when the user asks about a specific post/page name (like "Sample Page", "Hello World", "About") or post ID. This is NOT for global settings.', 'smarttoolz' );
	...
	public function execute( $args ) {
		...
		if ( ! $post_id ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'Please provide either post_id or post_title to identify the post.', 'smarttoolz' )
			);
		}
		...
		$meta_boxes = SmartToolz_Meta_Boxes::get_instance();
		...
		return SmartToolz_Abilities_Response::success(
			...
			sprintf( __( 'Retrieved post meta settings for "%1$s" (ID: %2$d)', 'smarttoolz' ), $post->post_title, $post_id ),
			array(
				'post_id'    => $post_id,
...
SmartToolz_Get_Postmeta::register();
