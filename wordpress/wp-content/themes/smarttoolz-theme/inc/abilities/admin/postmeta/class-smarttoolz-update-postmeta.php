...
/**
 * Class SmartToolz_Update_Postmeta
 */
class SmartToolz_Update_Postmeta extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-post-meta';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update Post-Specific Meta Settings (Individual Posts/Pages)', 'smarttoolz' );
		$this->description = __( 'Updates SmartToolz meta settings for ONE SPECIFIC post or page by post title or ID. This overrides global settings for that individual post only. IMPORTANT: Use this ONLY when the user specifies a specific post/page name (like "Sample Page", "Hello World", "About") or post ID. This is NOT for global settings - use single page/post update tools for global settings instead.', 'smarttoolz' );
	...
	public function execute( $args ) {
		...
		if ( isset( $args['post_id'] ) ) {
			...
			if ( ! $post instanceof \WP_Post ) {
				return SmartToolz_Abilities_Response::error(
					__( 'Post not found.', 'smarttoolz' ),
					/* translators: %s: post title */
					sprintf( __( 'Could not find a post or page with the title "%s".', 'smarttoolz' ), $post_title )
				);
			}
		...
		if ( ! $post_id ) {
			return SmartToolz_Abilities_Response::error(
				__( 'No post specified.', 'smarttoolz' ),
				__( 'Please provide either post_id or post_title to identify the post to update.', 'smarttoolz' )
			);
		}
		...
		if ( ! $post ) {
			return SmartToolz_Abilities_Response::error(
				...
				sprintf( __( 'Post with ID %d does not exist.', 'smarttoolz' ), $post_id )
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'Permission denied.', 'smarttoolz' ),
				/* translators: %s: post title */
				sprintf( __( 'You do not have permission to edit "%s".', 'smarttoolz' ), $post->post_title )
			);
		}
		...
		if ( ! in_array( $key, $valid_keys, true ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'Invalid meta key.', 'smarttoolz' ),
				/* translators: 1: meta key, 2: list of valid keys */
				sprintf( __( 'The meta key "%1$s" is not valid. Valid keys are: %2$s', 'smarttoolz' ), $key, implode( ', ', $valid_keys ) )
			);
		}
		...
		return SmartToolz_Abilities_Response::success(
			sprintf(
				...
				__( 'Updated "%1$s" to "%2$s" for "%3$s" (ID: %4$d)', 'smarttoolz' ),
				$key_label,
				$value_label,
				$post->post_title,
				$post_id
...
SmartToolz_Update_Postmeta::register();
