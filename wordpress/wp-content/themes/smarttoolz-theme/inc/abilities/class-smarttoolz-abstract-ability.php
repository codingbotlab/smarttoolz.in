 ...
 *
 * Base class for all SmartToolz abilities.
 *
 * @package SmartToolz
...
/**
 * Abstract Class SmartToolz_Abstract_Ability
 */
abstract class SmartToolz_Abstract_Ability {
	/**
	 * Ability ID (e.g. 'smarttoolz/get-font-body').
	 *
	 * @var string
	 */
	protected $id = '';
	...
	public function handle_execute( $args ) {
		try {
			...
			return SmartToolz_Abilities_Response::error( sprintf( __( 'An unexpected error occurred: %s', 'smarttoolz' ), $e->getMessage() ) );
		} catch ( Error $e ) {
			/* translators: %s: error message */
			return SmartToolz_Abilities_Response::error( sprintf( __( 'A system error occurred: %s', 'smarttoolz' ), $e->getMessage() ) );
		}
		...
		if ( 'write' === $this->get_tool_type() && ! empty( $result['success'] ) ) {
			 ...
			 */
			do_action( 'smarttoolz_ability_settings_updated', $this->get_id(), $args );
		}
	 ...
	 * in the SmartToolz_Abilities_Response::success() format.
	 *
	...
	public function get_show_in_rest() {
		 ...
		 * @param string $ability_id       The ability ID (e.g. 'smarttoolz/get-font-body').
		 * @param self   $ability_instance The ability instance.
		 * @since 4.12.6
		 */
		return apply_filters( 'smarttoolz_ability_show_in_rest', true, $this->id, $this );
	}
	 ...
	 * and MCP type. The public flag is filterable via 'smarttoolz_ability_mcp_public'.
	 *
	 * @return array{public: bool, type: string}
	 */
	public function get_mcp() {
		/**
		 * Filter whether an SmartToolz ability is publicly exposed via MCP.
		 *
		 * @since 4.12.6
		 *
		 ...
		 * @param self   $ability_instance The ability instance.
		 */
		$is_public = apply_filters( 'smarttoolz_ability_mcp_public', true, $this->id, $this );
	...
	public static function register() {
		...
		if ( 'write' === $instance->get_tool_type() && ! SmartToolz_API_Init::get_admin_settings_option( 'enable_edit_abilities', true ) ) {
			return;
		}
