...
/**
 * Class SmartToolz_Update_Footer_Builder
 */
class SmartToolz_Update_Footer_Builder extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-footer-builder';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Footer Builder Settings', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme footer builder configuration for desktop and mobile layouts. Allows positioning components like buttons, HTML blocks, widgets, social icons, footer menu, and copyright in above footer, primary footer, and below footer sections with column-based placements. Also configures column counts and layout widths for each footer section.', 'smarttoolz' );
		...
		$this->meta = array(
			...
			'constraints' => array(
				'dependencies' => array(
					array(
						'tool'             => 'smarttoolz/get-footer-builder',
						'description_hint' => 'Get current footer layout and available_components before updating',
					),
				),
				'usage_hints'  => array(
					'SMART_MERGE_WORKFLOW' => 'Uses SMART MERGING that auto-detects operation type. Call smarttoolz/get-footer-builder FIRST to see current layout. ADD operations (new components) append to existing. REMOVE operations (subset of current) keep only specified. CLEAR operations (empty array) remove all. NO need to manually reconstruct arrays for simple add/remove.',
	...
	public function execute( $args ) {
		...
		// Check if smarttoolz_get_option function exists (required for proper data retrieval).
		if ( ! function_exists( 'smarttoolz_get_option' ) || ! function_exists( 'smarttoolz_update_option' ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'SmartToolz theme helper functions not available.', 'smarttoolz' ),
				__( 'The smarttoolz_get_option and smarttoolz_update_option functions are required but not found.', 'smarttoolz' )
			);
		}
		...
		if ( isset( $args['desktop'] ) && is_array( $args['desktop'] ) ) {
			// Get current desktop structure from database using SmartToolz's helper function.
			$current_desktop_raw = smarttoolz_get_option( 'footer-desktop-items', array() );
			...
			// Update the option using SmartToolz's update method to ensure proper storage.
			smarttoolz_update_option( 'footer-desktop-items', $new_desktop );
			$updated           = true;
			$update_messages[] = 'Desktop footer layout updated';
		}

		if ( isset( $args['mobile'] ) && is_array( $args['mobile'] ) ) {
			$current_mobile_raw = smarttoolz_get_option( 'footer-mobile-items', array() );
			...
			smarttoolz_update_option( 'footer-mobile-items', $new_mobile );
			$updated           = true;
		...
		if ( isset( $args['above_footer_columns'] ) ) {
			...
			if ( $columns < 1 || $columns > 5 ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid above_footer_columns: %d.', 'smarttoolz' ), $columns ),
					__( 'Value must be between 1 and 5.', 'smarttoolz' )
				);
			}
			smarttoolz_update_option( 'hba-footer-column', (string) $columns );
			$updated = true;
			/* translators: %d: column count */
			$update_messages[] = sprintf( __( 'Above footer columns set to %d', 'smarttoolz' ), $columns );
		}
		...
		if ( isset( $args['primary_footer_columns'] ) ) {
			...
			if ( $columns < 1 || $columns > 5 ) {
				return SmartToolz_Abilities_Response::error(
					/* translators: %d: column count */
					sprintf( __( 'Invalid primary_footer_columns: %d.', 'smarttoolz' ), $columns ),
					__( 'Value must be between 1 and 5.', 'smarttoolz' )
				);
			}
			smarttoolz_update_option( 'hb-footer-column', (string) $columns );
			$updated = true;
			/* translators: %d: column count */
			$update_messages[] = sprintf( __( 'Primary footer columns set to %d', 'smarttoolz' ), $columns );
		}
		...
		if ( isset( $args['below_footer_columns'] ) ) {
			...
			if ( $columns < 1 || $columns > 5 ) {
				return SmartToolz_Abilities_Response::error(
					/* translators: %d: column count */
					sprintf( __( 'Invalid below_footer_columns: %d.', 'smarttoolz' ), $columns ),
					__( 'Value must be between 1 and 5.', 'smarttoolz' )
				);
			}
			smarttoolz_update_option( 'hbb-footer-column', (string) $columns );
			$updated = true;
			/* translators: %d: column count */
			$update_messages[] = sprintf( __( 'Below footer columns set to %d', 'smarttoolz' ), $columns );
		}
		...
		if ( isset( $args['above_footer_layout'] ) && ! empty( $args['above_footer_layout'] ) ) {
			...
			if ( ! in_array( $layout, $valid_layouts, true ) ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid above_footer_layout: %s.', 'smarttoolz' ), $layout ),
					__( 'Valid options: full, content', 'smarttoolz' )
				);
			}
			smarttoolz_update_option( 'hba-footer-layout', $layout );
			$updated = true;
			/* translators: %s: layout value */
			$update_messages[] = sprintf( __( 'Above footer layout set to %s', 'smarttoolz' ), $layout );
		}
		...
		if ( isset( $args['primary_footer_layout'] ) && ! empty( $args['primary_footer_layout'] ) ) {
			...
			if ( ! in_array( $layout, $valid_layouts, true ) ) {
				return SmartToolz_Abilities_Response::error(
					/* translators: %s: layout value */
					sprintf( __( 'Invalid primary_footer_layout: %s.', 'smarttoolz' ), $layout ),
					__( 'Valid options: full, content', 'smarttoolz' )
				);
			}
			smarttoolz_update_option( 'hb-footer-layout', $layout );
			$updated = true;
			/* translators: %s: layout value */
			$update_messages[] = sprintf( __( 'Primary footer layout set to %s', 'smarttoolz' ), $layout );
		}
		...
		if ( isset( $args['below_footer_layout'] ) && ! empty( $args['below_footer_layout'] ) ) {
			...
			if ( ! in_array( $layout, $valid_layouts, true ) ) {
				return SmartToolz_Abilities_Response::error(
					/* translators: %s: layout value */
					sprintf( __( 'Invalid below_footer_layout: %s.', 'smarttoolz' ), $layout ),
					__( 'Valid options: full, content', 'smarttoolz' )
				);
			}
			smarttoolz_update_option( 'hbb-footer-layout', $layout );
			$updated = true;
			/* translators: %s: layout value */
			$update_messages[] = sprintf( __( 'Below footer layout set to %s', 'smarttoolz' ), $layout );
		}
	...
	/**
	 * Normalize component ID to match SmartToolz's expected format for footer components.
	 *
...
SmartToolz_Update_Footer_Builder::register();
