...
/**
 * Class SmartToolz_Get_Breadcrumb
 */
class SmartToolz_Get_Breadcrumb extends SmartToolz_Abstract_Ability {
	/**
	...
	public function configure() {
		$this->id          = 'smarttoolz/get-breadcrumb';
		$this->label       = __( 'Get SmartToolz Breadcrumb Settings', 'smarttoolz' );
		$this->description = __( 'Retrieves the current SmartToolz theme breadcrumb settings including position, alignment, display settings, separator, typography, colors, and spacing.', 'smarttoolz' );
	...
	public function execute( $args ) {
		...
		$position           = smarttoolz_get_option( 'breadcrumb-position', 'none' );
		$alignment          = smarttoolz_get_option( 'breadcrumb-alignment', 'left' );
		$separator_selector = smarttoolz_get_option( 'breadcrumb-separator-selector', '\003E' );
		$separator_custom   = smarttoolz_get_option( 'breadcrumb-separator', '' );

		$enable_on = array(
			'home_page'   => smarttoolz_get_option( 'breadcrumb-disable-home-page', '1' ) === '1',
			'blog_page'   => smarttoolz_get_option( 'breadcrumb-disable-blog-posts-page', '1' ) === '1',
			'search'      => smarttoolz_get_option( 'breadcrumb-disable-search', '1' ) === '1',
			'archive'     => smarttoolz_get_option( 'breadcrumb-disable-archive', '1' ) === '1',
			'single_page' => smarttoolz_get_option( 'breadcrumb-disable-single-page', '1' ) === '1',
			'single_post' => smarttoolz_get_option( 'breadcrumb-disable-single-post', '1' ) === '1',
			'singular'    => smarttoolz_get_option( 'breadcrumb-disable-singular', '1' ) === '1',
			'404_page'    => smarttoolz_get_option( 'breadcrumb-disable-404-page', '1' ) === '1',
		);

		$typography = array(
			'font_family' => smarttoolz_get_option( 'breadcrumb-font-family', 'inherit' ),
			'font_weight' => smarttoolz_get_option( 'breadcrumb-font-weight', 'inherit' ),
			'font_size'   => smarttoolz_get_option( 'breadcrumb-font-size', array() ),
			'font_extras' => smarttoolz_get_option( 'breadcrumb-font-extras', array() ),
		);

		$colors = array(
			'background'  => smarttoolz_get_option( 'breadcrumb-bg-color', array() ),
			'text'        => smarttoolz_get_option( 'breadcrumb-active-color-responsive', array() ),
			'separator'   => smarttoolz_get_option( 'breadcrumb-separator-color', array() ),
			'link_normal' => smarttoolz_get_option( 'breadcrumb-text-color-responsive', array() ),
			'link_hover'  => smarttoolz_get_option( 'breadcrumb-hover-color-responsive', array() ),
		);

		$spacing = smarttoolz_get_option( 'breadcrumb-spacing', array() );

		$position_labels = array(
			'none'                                 => 'None',
			'smarttoolz_header_primary_container_after' => 'Inside',
			'smarttoolz_header_after'                   => 'After',
			'smarttoolz_entry_top'                      => 'Before Title',
		);
		...
		return SmartToolz_Abilities_Response::success(
			__( 'Retrieved breadcrumb settings successfully. IMPORTANT: When presenting any colors to the user, you MUST show visual color previews using this markdown format if valid color code: ![#HEXCODE](https://via.placeholder.com/15/HEXCODE/000000?text=+) `#HEXCODE` - Replace HEXCODE with the actual hex color value.', 'smarttoolz' ),
			array(
				'position'             => $position,
...
SmartToolz_Get_Breadcrumb::register();
