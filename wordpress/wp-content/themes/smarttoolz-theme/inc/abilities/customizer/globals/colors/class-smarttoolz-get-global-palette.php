...
/**
 * Class SmartToolz_Get_Global_Palette
 */
class SmartToolz_Get_Global_Palette extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/get-global-palette';
		$this->label       = __( 'Get SmartToolz Global Palette', 'smarttoolz' );
		$this->description = __( 'Retrieves the current active SmartToolz theme global color palette (palette_1 through palette_4) with its color values. Also returns all available color presets (Oak, Viola, Cedar, Willow, Lily, Rose, Sage, Flare, Maple, Birch, Dark) that can be applied to any palette.', 'smarttoolz' );
	...
	public function execute( $args ) {
		...
		return SmartToolz_Abilities_Response::success(
			...
			sprintf(
				__( 'Retrieved active global palette: %1$s (currently using %2$d colors). Also retrieved %3$d available color presets: %4$s.', 'smarttoolz' ),
				$palette_id,
...
SmartToolz_Get_Global_Palette::register();
