...
/**
 * Class SmartToolz_Update_Background_Colors
 */
class SmartToolz_Update_Background_Colors extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-color-background';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Background Colors', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme background colors including site background and content background. Supports solid colors, gradients, and images.', 'smarttoolz' );
	...
	public function execute( $args ) {
		if ( ! defined( 'SMARTTOOLZ_THEME_SETTINGS' ) ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'Option functions are not available.', 'smarttoolz' )
			);
		}

		if ( empty( $args['site_background'] ) && empty( $args['content_background'] ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'No background specified.', 'smarttoolz' ),
				__( 'Please provide either site_background or content_background to update.', 'smarttoolz' )
			);
		}
		...
		if ( ! empty( $args['site_background'] ) ) {
			$current_site_bg = smarttoolz_get_option( 'site-layout-outside-bg-obj-responsive' );
		...
		if ( ! empty( $args['content_background'] ) ) {
			$current_content_bg = smarttoolz_get_option( 'content-bg-obj-responsive' );
		...
		return SmartToolz_Abilities_Response::success(
			...
			sprintf( __( 'Background colors updated successfully: %s', 'smarttoolz' ), implode( ', ', $updated ) ),
			array(
				'updated'            => $updated,
...
SmartToolz_Update_Background_Colors::register();
