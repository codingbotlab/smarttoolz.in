...
/**
 * Class SmartToolz_Update_Global_Palette
 */
class SmartToolz_Update_Global_Palette extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-global-palette';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Global Palette', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme global color palette. You can apply a preset (Oak, Lily, Viola, Cedar, Willow, Rose, Sage, Flare, Maple, Birch, Dark) or set individual colors. Colors should be in hex format (e.g., #046bd2).', 'smarttoolz' );
	...
	public function execute( $args ) {
		$palette_data = get_option( 'smarttoolz-color-palettes', array() );

		if ( empty( $palette_data ) && ! is_array( $palette_data ) ) {
			if ( class_exists( 'SmartToolz_Global_Palette' ) ) {
				$palette_data = SmartToolz_Global_Palette::get_default_color_palette();
			} else {
				return SmartToolz_Abilities_Response::error(
					__( 'SmartToolz Global Palette is not available.', 'smarttoolz' ),
					__( 'Please ensure SmartToolz theme is active.', 'smarttoolz' )
				);
			}
		}
		...
		if ( ! in_array( $palette_id, array( 'palette_1', 'palette_2', 'palette_3', 'palette_4' ), true ) ) {
			return SmartToolz_Abilities_Response::error(
				...
				sprintf( __( 'Invalid palette ID: %s.', 'smarttoolz' ), $palette_id ),
				__( 'Must be one of: palette_1, palette_2, palette_3, palette_4', 'smarttoolz' )
			);
		}
		...
		if ( isset( $args['preset'] ) && ! empty( $args['preset'] ) ) {
			...
			if ( function_exists( 'smarttoolz_get_palette_presets' ) ) {
				...
				if ( $preset_key && isset( $presets[ $preset_key ] ) ) {
					...
					return SmartToolz_Abilities_Response::error(
						...
						sprintf( __( 'Invalid preset name: %s.', 'smarttoolz' ), $preset_name ),
						__( 'Available presets: Oak, Lily, Viola, Cedar, Willow, Rose, Sage, Flare, Maple, Birch, Dark', 'smarttoolz' )
					);
				}
			} else {
				return SmartToolz_Abilities_Response::error(
					__( 'Preset function not available.', 'smarttoolz' ),
					__( 'Please ensure SmartToolz theme is active and up to date.', 'smarttoolz' )
				);
			}
		}
		...
		if ( empty( $applied_preset ) && ( ! isset( $args['colors'] ) || empty( $args['colors'] ) ) ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'Please provide either a preset name or color values to update.', 'smarttoolz' )
			);
		}
		...
		update_option( 'smarttoolz-color-palettes', $palette_data );
		...
		if ( $is_current_palette && function_exists( 'smarttoolz_get_option' ) && defined( 'SMARTTOOLZ_THEME_SETTINGS' ) ) {
			$global_palette = smarttoolz_get_option( 'global-color-palette', array( 'palette' => array() ) );
		...
		$message = ! empty( $applied_preset )
			/* translators: %s: preset name */
			? sprintf( __( 'Global palette updated successfully with %s preset.', 'smarttoolz' ), $applied_preset )
			: __( 'Global palette updated successfully.', 'smarttoolz' );
...
SmartToolz_Update_Global_Palette::register();
