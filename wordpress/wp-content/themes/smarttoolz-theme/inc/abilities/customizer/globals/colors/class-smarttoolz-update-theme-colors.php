...
/**
 * Class SmartToolz_Update_Theme_Colors
 */
class SmartToolz_Update_Theme_Colors extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-theme-color';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Theme Colors', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme global colors including accent color, link colors, heading colors, body text color, and border color. These are the core colors that define your theme\'s color scheme.', 'smarttoolz' );
	...
	public function execute( $args ) {
		// Map of input args to SmartToolz settings keys.
		...
		if ( ! $has_updates ) {
			return SmartToolz_Abilities_Response::error(
				__( 'No colors specified.', 'smarttoolz' ),
				__( 'Please provide at least one color to update.', 'smarttoolz' )
			);
		}
		...
		foreach ( $color_settings as $arg_key => $settings_key ) {
			if ( ! empty( $args[ $arg_key ] ) ) {
				...
				smarttoolz_update_option( $settings_key, $color );
		...
		if ( empty( $updated ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'Invalid color values.', 'smarttoolz' ),
				__( 'Please provide valid hex color values (e.g., #046bd2).', 'smarttoolz' )
			);
		}
		...
		$current_colors = array();
		foreach ( $color_settings as $arg_key => $settings_key ) {
			$current_colors[ $arg_key ] = smarttoolz_get_option( $settings_key );
		}

		return SmartToolz_Abilities_Response::success(
			/* translators: %s: comma-separated list of updated colors */
			sprintf( __( 'Theme colors updated successfully: %s', 'smarttoolz' ), implode( ', ', $updated ) ),
			array(
				'updated'        => $updated,
...
SmartToolz_Update_Theme_Colors::register();
