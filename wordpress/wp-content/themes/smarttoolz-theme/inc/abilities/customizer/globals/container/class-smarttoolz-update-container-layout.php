...
/**
 * Class SmartToolz_Update_Container_Layout
 */
class SmartToolz_Update_Container_Layout extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-container-layout';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Container Layout', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme container layout and style settings. You can set container layout (normal, narrow, or full width) and container style (boxed or unboxed). Container style applies only when layout is normal or narrow.', 'smarttoolz' );
		...
		return array(
			...
			'properties' => array(
				'container_layout' => array(
					'type'        => 'string',
					'description' => __( 'The container layout to apply. Options: "normal-width-container" (Normal width), "narrow-width-container" (Narrow width), "full-width-container" (Full width / Stretched).', 'smarttoolz' ),
					'enum'        => array( 'normal-width-container', 'narrow-width-container', 'full-width-container' ),
				),
				'container_style'  => array(
					'type'        => 'string',
					'description' => __( 'The container style to apply. Options: "boxed" (Boxed style with padding), "unboxed" (Unboxed/Plain style). Note: This only applies when layout is normal or narrow.', 'smarttoolz' ),
		...
		if ( isset( $args['container_layout'] ) && ! empty( $args['container_layout'] ) ) {
			...
			if ( ! in_array( $container_layout, $valid_layouts, true ) ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid container layout: %s.', 'smarttoolz' ), $container_layout ),
					__( 'Valid options: normal-width-container, narrow-width-container, full-width-container', 'smarttoolz' )
				);
			}
			...
			$update_messages[] = sprintf( __( 'Container layout set to %s', 'smarttoolz' ), $layout_labels[ $container_layout ] );
		}
		...
		if ( isset( $args['container_style'] ) && ! empty( $args['container_style'] ) ) {
			...
			if ( ! in_array( $container_style, $valid_styles, true ) ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid container style: %s.', 'smarttoolz' ), $container_style ),
			...
			$update_messages[] = sprintf( __( 'Container style set to %s', 'smarttoolz' ), $style_labels[ $container_style ] );
		}

		if ( ! $updated ) {
			return SmartToolz_Abilities_Response::error(
				__( 'No changes specified.', 'smarttoolz' ),
				__( 'Please provide either container_layout or container_style to update.', 'smarttoolz' )
			);
		}
...
SmartToolz_Update_Container_Layout::register();
