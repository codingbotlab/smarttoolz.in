...
/**
 * Class SmartToolz_Update_Heading_Font
 *
 * Parameterized ability that registers an update-font ability
 * for a specific heading level (h1 through h6).
 */
class SmartToolz_Update_Heading_Font extends SmartToolz_Abstract_Ability {
	/**
	 * Heading level (1-6).
	 *
	 * @var int
	...
	public function configure() {
		$this->id          = 'smarttoolz/update-font-h' . $this->level;
		$this->category    = 'smarttoolz';
		$this->label       = /* translators: %d: heading level */ sprintf( __( 'Update SmartToolz H%d Font', 'smarttoolz' ), $this->level );
		$this->description = /* translators: %d: heading level */ sprintf( __( 'Updates the SmartToolz theme H%d heading font family, weight, size, and other typography settings. IMPORTANT: When user specifies size like "32px" or "2rem", you must separate the numeric value from the unit: font_size.desktop = 32 (number), font_size.desktop-unit = "px" (string).', 'smarttoolz' ), $this->level );
	...
	public function execute( $args ) {
		...
		if ( isset( $args['font_family'] ) ) {
			...
			smarttoolz_update_option( 'font-family-' . $tag, $formatted_font );
		}

		if ( isset( $args['font_weight'] ) ) {
			smarttoolz_update_option( 'font-weight-' . $tag, sanitize_text_field( $args['font_weight'] ) );
		}

		if ( isset( $args['font_size'] ) && is_array( $args['font_size'] ) ) {
			$existing = smarttoolz_get_option( 'font-size-' . $tag, array() );
			$merged   = array_merge( $existing, $args['font_size'] );
			smarttoolz_update_option( 'font-size-' . $tag, SmartToolz_Abilities_Helper::sanitize_responsive_typo( $merged ) );
		}

		SmartToolz_Abilities_Helper::update_font_extras( $args, $tag . '-font-extras' );

		return SmartToolz_Abilities_Response::success(
			/* translators: %s: heading tag */
			sprintf( __( '%s font settings updated successfully.', 'smarttoolz' ), strtoupper( $tag ) ),
			array(
...
SmartToolz_Update_Heading_Font::register_all();
