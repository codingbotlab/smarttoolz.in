...
/**
 * Class SmartToolz_Update_Headings_Font
 */
class SmartToolz_Update_Headings_Font extends SmartToolz_Abstract_Ability {
	/**
	 * Configure the ability.
	 */
	public function configure() {
		$this->id          = 'smarttoolz/update-font-heading';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Headings Font', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme headings font family, weight, and other typography settings for all headings.', 'smarttoolz' );
	...
	public function execute( $args ) {
		if ( isset( $args['font_family'] ) ) {
			...
			smarttoolz_update_option( 'headings-font-family', $formatted_font );
		}

		if ( isset( $args['font_weight'] ) ) {
			smarttoolz_update_option( 'headings-font-weight', sanitize_text_field( $args['font_weight'] ) );
		}

		SmartToolz_Abilities_Helper::update_font_extras( $args, 'headings-font-extras' );

		return SmartToolz_Abilities_Response::success(
			__( 'Headings font settings updated successfully.', 'smarttoolz' ),
			array(
				'font_family' => smarttoolz_get_option( 'headings-font-family', '' ),
				'font_weight' => smarttoolz_get_option( 'headings-font-weight', '' ),
				'font_extras' => smarttoolz_get_option( 'headings-font-extras', array() ),
			)
		);
	}
}

SmartToolz_Update_Headings_Font::register();
