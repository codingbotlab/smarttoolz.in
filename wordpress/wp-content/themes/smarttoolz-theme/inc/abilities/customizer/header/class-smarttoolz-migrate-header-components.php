 ...
 *
 * Fixes incorrect component IDs in the header builder by normalizing them to SmartToolz's expected format.
...
/**
 * Class SmartToolz_Migrate_Header_Components
 */
class SmartToolz_Migrate_Header_Components extends SmartToolz_Abstract_Ability {
	/**
	...
	public function configure() {
		$this->id          = 'smarttoolz/update-header-component';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Migrate SmartToolz Header Component IDs', 'smarttoolz' );
		$this->description = __( 'Scans and fixes incorrect header builder component IDs. Automatically converts variations like menu_secondary to menu-2, social_1 to social-icons-1, etc. Use this when header components are not showing in customizer/frontend due to incorrect IDs.', 'smarttoolz' );
		...
		$this->meta = array(
			...
			'constraints' => array(
				'usage_hints' => array(
					...
					'what_it_fixes'  => 'Normalizes all component IDs to SmartToolz format: menu-1, menu-2, social-icons-1, button-1, html-1, widget-1, etc.',
	...
	public function execute( $args ) {
		...
		if ( 0 === $total_fixes ) {
			return SmartToolz_Abilities_Response::success(
				__( 'No component IDs need migration. All component IDs are already correct.', 'smarttoolz' ),
				array(
					'fixes_needed' => 0,
					'dry_run'      => $dry_run,
				)
...
SmartToolz_Migrate_Header_Components::register();
