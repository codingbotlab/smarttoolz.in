...
/**
 * Class SmartToolz_Update_Header_Builder_Design
 */
class SmartToolz_Update_Header_Builder_Design extends SmartToolz_Abstract_Ability {
	/**
	...
	public function configure() {
		$this->id          = 'smarttoolz/update-header-builder-design';
		$this->label       = __( 'Update SmartToolz Header Builder Design Options', 'smarttoolz' );
		$this->description = __( 'Updates design options for SmartToolz header builder sections (above, primary, below) including background, border color, border size, padding and margin.', 'smarttoolz' );
		...
		$this->meta = array(
			...
			'constraints' => array(
				'dependencies' => array(
					array(
						'tool'             => 'smarttoolz/get-header-builder-design',
	...
	public function execute( $args ) {
		...
		if ( ! isset( $args['section'] ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'Section is required.', 'smarttoolz' ),
				__( 'Please provide a section: above, primary, or below.', 'smarttoolz' )
			);
		}
		...
		if ( ! in_array( $section, $valid_sections, true ) ) {
			return SmartToolz_Abilities_Response::error(
				__( 'Invalid section provided.', 'smarttoolz' ),
				__( 'Section must be one of: above, primary, below.', 'smarttoolz' )
			);
		}
		...
		if ( isset( $args['background'] ) && is_array( $args['background'] ) ) {
			...
			smarttoolz_update_option( $prefix . '-bg-obj-responsive', $sanitized_bg );
			$updated           = true;
		...
		if ( isset( $args['border_color'] ) ) {
			...
			smarttoolz_update_option( $prefix . '-bottom-border-color', $sanitized_color );
			$updated           = true;
		...
		if ( isset( $args['border_size'] ) ) {
			...
			smarttoolz_update_option( $prefix . '-separator', $size );
			$updated           = true;
		...
		if ( isset( $args['padding'] ) && is_array( $args['padding'] ) ) {
			...
			smarttoolz_update_option( $section_id . '-padding', $sanitized_padding );
			$updated           = true;
		...
		if ( isset( $args['margin'] ) && is_array( $args['margin'] ) ) {
			...
			smarttoolz_update_option( $section_id . '-margin', $sanitized_margin );
			$updated           = true;
		...
		if ( ! $updated ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'Please provide at least one design option to update.', 'smarttoolz' )
			);
		}

		/* translators: 1: section name, 2: list of updates */
		$message = sprintf(
			__( '%1$s header design updated: %2$s.', 'smarttoolz' ),
			ucfirst( $section ),
...
SmartToolz_Update_Header_Builder_Design::register();
