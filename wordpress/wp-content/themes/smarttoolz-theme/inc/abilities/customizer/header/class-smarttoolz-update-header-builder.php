...
/**
 * Class SmartToolz_Update_Header_Builder
 */
class SmartToolz_Update_Header_Builder extends SmartToolz_Abstract_Ability {
	/**
	...
	public function configure() {
		$this->id          = 'smarttoolz/update-header-builder';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Header Builder Settings', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme header builder configuration for desktop and mobile layouts. Allows positioning components like buttons, HTML blocks, menus, widgets, social icons, and search in popup (mobile drawer), above header, primary header, and below header sections with specific zone placements.', 'smarttoolz' );
	...
	public function execute( $args ) {
		...
		if ( ! function_exists( 'smarttoolz_get_option' ) ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'The smarttoolz_get_option function is required but not found.', 'smarttoolz' )
			);
		}
		...
		if ( isset( $args['desktop'] ) && is_array( $args['desktop'] ) ) {
			$current_desktop_raw = smarttoolz_get_option( 'header-desktop-items', array() );
			...
			smarttoolz_update_option( 'header-desktop-items', $new_desktop );
			$updated           = true;
		...
		if ( isset( $args['mobile'] ) && is_array( $args['mobile'] ) ) {
			...
			$current_mobile_raw = smarttoolz_get_option( 'header-mobile-items', array() );
			...
			smarttoolz_update_option( 'header-mobile-items', $new_mobile );
			$updated           = true;
		...
		if ( ! $updated ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'Please provide at least one layout to update (desktop or mobile).', 'smarttoolz' )
			);
		}
	...
	/**
	 * Normalize component ID to match SmartToolz's expected format.
...
SmartToolz_Update_Header_Builder::register();
