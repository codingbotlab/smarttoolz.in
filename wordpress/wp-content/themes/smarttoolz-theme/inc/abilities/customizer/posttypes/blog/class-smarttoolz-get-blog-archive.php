...
/**
 * Class SmartToolz_Get_Blog_Archive
 */
class SmartToolz_Get_Blog_Archive extends SmartToolz_Abstract_Ability {
	/**
	...
	public function configure() {
		$this->id          = 'smarttoolz/get-blog-archive';
		$this->label       = __( 'Get SmartToolz Blog Archive Settings', 'smarttoolz' );
		$this->description = __( 'Retrieves the current SmartToolz theme blog archive settings including blog layout, posts per page, sidebar layout, sidebar style, and sidebar width.', 'smarttoolz' );
	...
	public function get_output_schema() {
		return $this->build_output_schema(
			array(
				'blog_layout'               => array(
					'type'        => 'string',
					'description' => __( 'Current blog layout key.', 'smarttoolz' ),
				),
				'blog_layout_label'         => array(
					'type'        => 'string',
					'description' => __( 'Human-readable blog layout name.', 'smarttoolz' ),
				),
				'posts_per_page'            => array(
					'type'        => 'integer',
					'description' => __( 'Number of posts per page.', 'smarttoolz' ),
				),
				...
				'sidebar_width'             => array(
					'type'        => 'integer',
					'description' => __( 'Sidebar width percentage.', 'smarttoolz' ),
				),
				'available_blog_layouts'    => array(
					'type'        => 'object',
					'description' => __( 'Available blog layout options.', 'smarttoolz' ),
				),
	...
	public function execute( $args ) {
		$blog_layout    = smarttoolz_get_option( 'blog-layout', 'blog-layout-5' );
		$posts_per_page = smarttoolz_get_option( 'blog-post-per-page', get_option( 'posts_per_page', 10 ) );
		$sidebar_layout = smarttoolz_get_option( 'site-sidebar-layout', 'right-sidebar' );
		$sidebar_style  = smarttoolz_get_option( 'site-sidebar-style', 'unboxed' );
		$sidebar_width  = smarttoolz_get_option( 'site-sidebar-width', 30 );
		...
		return SmartToolz_Abilities_Response::success(
			...
			sprintf(
				__( 'Your blog archive is using the "%1$s" layout with %2$d posts per page. Sidebar is set to "%3$s" (%4$s style, %5$d%% width).', 'smarttoolz' ),
				$blog_layout_name,
				(int) $posts_per_page,
				$sidebar_layout_name,
				$sidebar_style_name,
...
SmartToolz_Get_Blog_Archive::register();
