...
/**
 * Class SmartToolz_Update_Blog_Archive
 */
class SmartToolz_Update_Blog_Archive extends SmartToolz_Abstract_Ability {
	/**
	...
	public function configure() {
		$this->id          = 'smarttoolz/update-blog-archive';
		$this->category    = 'smarttoolz';
		$this->label       = __( 'Update SmartToolz Blog Archive Settings', 'smarttoolz' );
		$this->description = __( 'Updates the SmartToolz theme blog archive settings including blog layout, posts per page, sidebar layout, sidebar style, and sidebar width.', 'smarttoolz' );
	...
	public function get_input_schema() {
		return array(
			...
			'properties' => array(
				'blog_layout'    => array(
					'type'        => 'string',
					'description' => __( 'Blog layout style. Options: "blog-layout-classic" (Classic), "blog-layout-4" (Grid), "blog-layout-5" (List), "blog-layout-6" (Cover).', 'smarttoolz' ),
					'enum'        => array( 'blog-layout-classic', 'blog-layout-4', 'blog-layout-5', 'blog-layout-6' ),
				),
				'posts_per_page' => array(
					'type'        => 'integer',
					'description' => __( 'Number of posts to display per page (1-500).', 'smarttoolz' ),
				),
				'sidebar_layout' => array(
					'type'        => 'string',
					'description' => __( 'Sidebar layout position. Options: "no-sidebar" (No Sidebar), "left-sidebar" (Left Sidebar), "right-sidebar" (Right Sidebar).', 'smarttoolz' ),
					'enum'        => array( 'no-sidebar', 'left-sidebar', 'right-sidebar' ),
				),
				'sidebar_style'  => array(
					'type'        => 'string',
					'description' => __( 'Sidebar style. Options: "unboxed" (Unboxed), "boxed" (Boxed).', 'smarttoolz' ),
					'enum'        => array( 'unboxed', 'boxed' ),
				),
				'sidebar_width'  => array(
					'type'        => 'integer',
					'description' => __( 'Sidebar width in percentage (15-50).', 'smarttoolz' ),
				),
			),
		);
	}
	...
	public function execute( $args ) {
		...
		if ( isset( $args['blog_layout'] ) && ! empty( $args['blog_layout'] ) ) {
			...
			if ( ! in_array( $blog_layout, $valid_layouts, true ) ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid blog_layout: %s.', 'smarttoolz' ), $blog_layout ),
					__( 'Valid options: blog-layout-classic, blog-layout-4, blog-layout-5, blog-layout-6', 'smarttoolz' )
				);
			}
			...
			smarttoolz_update_option( 'blog-layout', $blog_layout );
			$updated           = true;
		...
		if ( isset( $args['posts_per_page'] ) ) {
			...
			if ( $posts_per_page < 1 || $posts_per_page > 500 ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid posts_per_page: %d.', 'smarttoolz' ), $posts_per_page ),
					__( 'Value must be between 1 and 500.', 'smarttoolz' )
				);
			}

			smarttoolz_update_option( 'blog-post-per-page', $posts_per_page );
			$updated           = true;
		...
		if ( isset( $args['sidebar_layout'] ) && ! empty( $args['sidebar_layout'] ) ) {
			...
			smarttoolz_update_option( 'site-sidebar-layout', $sidebar_layout );
			$updated           = true;
		...
		if ( isset( $args['sidebar_style'] ) && ! empty( $args['sidebar_style'] ) ) {
			...
			smarttoolz_update_option( 'site-sidebar-style', $sidebar_style );
			$updated           = true;
		...
		if ( isset( $args['sidebar_width'] ) ) {
			...
			if ( $sidebar_width < 15 || $sidebar_width > 50 ) {
				return SmartToolz_Abilities_Response::error(
					...
					sprintf( __( 'Invalid sidebar_width: %d.', 'smarttoolz' ), $sidebar_width ),
					__( 'Value must be between 15 and 50 (percentage).', 'smarttoolz' )
				);
			}

			smarttoolz_update_option( 'site-sidebar-width', $sidebar_width );
			$updated           = true;
		...
		if ( ! $updated ) {
			return SmartToolz_Abilities_Response::error(
				...
				__( 'Please provide at least one setting to update (blog_layout, posts_per_page, sidebar_layout, sidebar_style, or sidebar_width).', 'smarttoolz' )
			);
		}
...
SmartToolz_Update_Blog_Archive::register();
