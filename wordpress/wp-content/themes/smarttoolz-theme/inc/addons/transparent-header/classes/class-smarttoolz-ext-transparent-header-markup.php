...
if ( ! class_exists( 'SmartToolz_ExtTransparenty_Header_Markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class SmartToolz_Ext_Transparent_Header_Markup {
		/**
		 * Member Variable
		 *
		 * @var object instance
		...
		public function __construct() {
			...
			/* Fixed header markup */
			add_action( 'smarttoolz_header', array( $this, 'transparent_header_logo' ), 1 );

			/**
			 * Metabox setup
			 */
			add_filter( 'smarttoolz_meta_box_options', array( $this, 'add_options' ) );
			add_action( 'smarttoolz_meta_box_markup_after', array( $this, 'add_options_markup' ) );

			add_action( 'smarttoolz_customizer_save', array( $this, 'customizer_save' ) );
		}
		...
		public function add_body_class( $classes ) {
			...
			$inherit_desk_logo              = smarttoolz_get_option( 'different-transparent-logo', false );
			$transparent_header_logo        = smarttoolz_get_option( 'transparent-header-logo', true );
			$transparent_header_logo_retina = smarttoolz_get_option( 'transparent-header-retina-logo', true );
		...
		/**
		 * SmartToolz check if transparent header is enabled.
		 *
		...
		public static function is_transparent_header() {
			...
			// Transparent Header.
			$enable_trans_header = smarttoolz_get_option( 'transparent-header-enable' );
			$trans_meta_option   = smarttoolz_get_option_meta( 'theme-transparent-header-meta', 'default' );
			...
			if ( $enable_trans_header ) {
				...
				if ( is_404() && '1' == smarttoolz_get_option( 'transparent-header-disable-404-page', smarttoolz_get_option( 'transparent-header-disable-archive' ) ) ) {
				...
				if ( is_search() && '1' == smarttoolz_get_option( 'transparent-header-disable-search-page', smarttoolz_get_option( 'transparent-header-disable-archive' ) ) ) {
					$enable_trans_header = false;
				}
				...
				if ( is_archive() && '1' == smarttoolz_get_option( 'transparent-header-disable-archive-pages', smarttoolz_get_option( 'transparent-header-disable-archive' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_home() && '1' == smarttoolz_get_option( 'transparent-header-disable-index' ) && ( 'posts' !== get_option( 'show_on_front' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_front_page() && 'posts' === get_option( 'show_on_front' ) && '1' == smarttoolz_get_option( 'transparent-header-disable-latest-posts-index' ) ) {
					$enable_trans_header = false;
				}

				if ( is_page() && '1' == smarttoolz_get_option( 'transparent-header-disable-page' ) ) {
					$enable_trans_header = false;
				}

				if ( is_single() && '1' == smarttoolz_get_option( 'transparent-header-disable-posts' ) ) {
					$enable_trans_header = false;
				}
			}

			if ( class_exists( 'SmartToolz_Woocommerce' ) ) {
				if ( is_product() && '1' == smarttoolz_get_option( 'transparent-header-disable-woo-products' ) ) {
					$enable_trans_header = false;
				}
			}
			...
			return apply_filters( 'smarttoolz_is_transparent_header', $enable_trans_header );
		}
		...
		public function transparent_header_logo() {
			...
			if ( self::is_transparent_header() && '1' == $inherit_desk_logo && '' !== $transparent_header_logo ) {
				...
				add_filter( 'smarttoolz_has_custom_logo', '__return_true' );
		...
		public function replace_trans_header_attr( $attr, $attachment, $size ) {
			...
			if ( $custom_logo_id == $attachment->ID ) {
				...
				if ( 'svg' === $file_extension ) {
					$attr['class'] = 'smarttoolz-logo-svg';
				}

				$diff_retina_logo = smarttoolz_get_option( 'different-transparent-retina-logo' );

				if ( '1' == $diff_retina_logo ) {

					$retina_logo = smarttoolz_get_option( 'transparent-header-retina-logo' );

					$attr['srcset'] = '';

					if ( apply_filters( 'smarttoolz_transparent_header_retina', true ) && '' !== $retina_logo ) {
		...
		public function add_options_markup( $meta ) {
			...
			$show_meta_field   = ! smarttoolz_check_is_bb_themer_layout();
			?>

			<?php if ( $show_meta_field ) { ?>
				<div class="transparent-header-wrapper">
					<p class="post-attributes-label-wrapper">
						<strong> <?php esc_html_e( 'Transparent Header', 'smarttoolz' ); ?> </strong><br/>
					</p>
					<select name="theme-transparent-header-meta" id="theme-transparent-header-meta">
						<option value="default" <?php selected( $trans_header_meta, 'default' ); ?>> <?php esc_html_e( 'Customizer Setting', 'smarttoolz' ); ?> </option>
						<option value="enabled" <?php selected( $trans_header_meta, 'enabled' ); ?>> <?php esc_html_e( 'Enabled', 'smarttoolz' ); ?> </option>
						<option value="disabled" <?php selected( $trans_header_meta, 'disabled' ); ?>> <?php esc_html_e( 'Disabled', 'smarttoolz' ); ?> </option>
					</select>
				</div>
			<?php } ?>
		...
		public function customizer_save() {
			...
			if ( '' !== $trans_logo ) {
				...
				SmartToolz_Customizer::generate_logo_by_width( $trans_logo_id );
		...
		public function transparent_logo_image_sizes( $sizes, $metadata ) {
			...
			$logo_width = smarttoolz_get_option( 'transparent-header-logo-width' );
 ...
 */
SmartToolz_Ext_Transparent_Header_Markup::get_instance();
