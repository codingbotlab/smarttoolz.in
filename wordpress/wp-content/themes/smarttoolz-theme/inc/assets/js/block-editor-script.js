 ...
 */
function smarttoolzGetIframeDoc( iframe ) {
	if ( ! iframe ) {
		return null;
	}
	try {
...
// arbitrary classes, and a forged chrome class would capture SmartToolz's injected UI and listeners.
function smarttoolzQueryEditorChrome( doc, selector ) {
	const element = doc ? doc.querySelector( selector ) : null;
	return ( element && element.closest( '[data-block]' ) ) ? null : element;
}

window.addEventListener( 'load', function(e) {
	smarttoolz_onload_function();
});

document.addEventListener('DOMContentLoaded', function () {
	if ( smarttoolzColors?.is_dark_palette ) {
		document.documentElement.classList.add('smarttoolz-dark-mode-enable');
	}
});

function addTitleVisibility() {
	let titleVisibility = smarttoolzQueryEditorChrome( document, '.title-visibility' ),
		titleBlock = smarttoolzQueryEditorChrome( document, '.edit-post-visual-editor__post-title-wrapper' ),
	...
	if ( smarttoolzColors.ast_wp_version_higher_6_3 ) {
		...
		if ( iframe && devicePreview.querySelector('iframe') !== null ) {
			editorDocument = smarttoolzGetIframeDoc( iframe ) || editorDocument;
		}

		// Addressed the WordPress 6.5 issue involving an extraneous iframe layer.
		if ( ! iframe && smarttoolzColors.ast_wp_version_higher_6_4 ) {
			...
			editorDocument = smarttoolzGetIframeDoc( _iframe ) || editorDocument;

			if (editorDocument) {
				titleVisibility = smarttoolzQueryEditorChrome( editorDocument, '.title-visibility' );
				titleBlock = smarttoolzQueryEditorChrome( editorDocument, '.edit-post-visual-editor__post-title-wrapper' );
			}
		}
	}
	...
	if( null !== titleBlock && null === titleVisibility ) {
		...
		let titleVisibilityTriggerElement = smarttoolzQueryEditorChrome( editorDocument, '.title-visibility' );
...
function siteLogoImageChange() {
	let mobileLogoState = smarttoolzColors.mobile_logo_state;

	if (!mobileLogoState) {
		return;
	}

	let mobileLogo = smarttoolzColors.mobile_logo;
	...
	if (iframe) {
		let iframeDoc = smarttoolzGetIframeDoc( iframe );
		if ( ! iframeDoc ) {
			return;
		}
...
function smarttoolz_onload_function() {

	/* Do things after DOM has fully loaded */

	var smarttoolzMetaBox = document.querySelector( '#smarttoolz_settings_meta_box' );
	if( smarttoolzMetaBox != null ){
	...
	const smarttoolzUpdateColorPalettePlaceholders = function () {
		for ( let btnCount = 0; btnCount < customColorPickerButtons.length; btnCount++ ) {
			...
			if ( colorCode.toUpperCase().indexOf( 'VAR(--AST-GLOBAL-COLOR' ) > -1 && smarttoolzColors[ transformedCode ] ) {
				customColorPickerButtons[btnCount].textContent = smarttoolzColors[ transformedCode ];
			}
		}
	};

	// Keeps 'ast-stacked-title-visibility' in sync with the canvas width without reading layout from the store-notification path.
	const smarttoolzObserveCanvasWidth = function ( editorStylesWrapper ) {
	...
	const smarttoolzResolveEditorDocument = function () {
	...
	const smarttoolzSyncGroupInheritClasses = function ( editorDocument ) {
	...
	const smarttoolzGetPreviewDevice = function () {
	...
	const smarttoolzGetEditorLayoutSignature = function () {
		const editorSelect = wp.data.select( 'core/editor' );
		...
		const device = smarttoolzGetPreviewDevice();
	...
	let smarttoolzLastLayoutSignature = null;
	let smarttoolzLastCanvasWrapper = null;
	let smarttoolzLayoutPassScheduled = false;
	let smarttoolzLightSyncScheduled = false;
	...
	const smarttoolzBindTitleVisibilityListeners = function ( editorDocument ) {
		const titleBlock = smarttoolzQueryEditorChrome( editorDocument, '.edit-post-visual-editor__post-title-wrapper' );
		const titleInput = smarttoolzQueryEditorChrome( editorDocument, '.editor-post-title__input' );
		const visibilityIcon = smarttoolzQueryEditorChrome( editorDocument, '.title-visibility' );
		...
		const smarttoolzGetVisibilityIcon = function () {
			return smarttoolzQueryEditorChrome( editorDocument, '.title-visibility' ) || visibilityIcon;
		};
		if ( ! smarttoolzColors.ast_wp_version_higher_6_3 ) {
			editorDocument.addEventListener('click', function (event){
				if( ! titleBlock.contains( event.target ) ){
					smarttoolzGetVisibilityIcon().classList.remove('ast-show-visibility-icon');
		...
		titleBlock.addEventListener('click', function (){
			smarttoolzGetVisibilityIcon().classList.add('ast-show-visibility-icon');
	...
	const smarttoolzRunEditorLightSync = function () {
		smarttoolzLightSyncScheduled = false;
		const editorDocument = smarttoolzResolveEditorDocument();
		smarttoolzUpdateColorPalettePlaceholders();
		siteLogoImageChange();
		addTitleVisibility();
		smarttoolzBindTitleVisibilityListeners( editorDocument );
		smarttoolzSyncGroupInheritClasses( editorDocument );
	};

	wp.data.subscribe(function () {
		if ( ! smarttoolzLightSyncScheduled ) {
			smarttoolzLightSyncScheduled = true;
			setTimeout( smarttoolzRunEditorLightSync, 1 );
		}

		const layoutSignature = smarttoolzGetEditorLayoutSignature();
		...
		const currentCanvasWrapper = smarttoolzResolveEditorDocument().querySelector( '.editor-styles-wrapper' );
		if ( layoutSignature === smarttoolzLastLayoutSignature && currentCanvasWrapper === smarttoolzLastCanvasWrapper ) {
			return;
		}
		smarttoolzLastLayoutSignature = layoutSignature;

		// Coalesce multiple store notifications for the same change into one pass.
		if ( smarttoolzLayoutPassScheduled ) {
			return;
		}
		smarttoolzLayoutPassScheduled = true;

		setTimeout( function () {
			smarttoolzLayoutPassScheduled = false;
			var editorDocument = smarttoolzResolveEditorDocument();
			...
			const contentLayout = ( undefined !== wp.data.select( 'core/editor' ) && null !== wp.data.select( 'core/editor' ) && undefined !== wp.data.select( 'core/editor' ).getEditedPostAttribute( 'meta' ) && wp.data.select( 'core/editor' ).getEditedPostAttribute( 'meta' )['ast-site-content-layout'] ) ? wp.data.select( 'core/editor' ).getEditedPostAttribute( 'meta' )['ast-site-content-layout'] : 'default',
				...
				editorBodyClass = smarttoolzColors.ast_wp_version_higher_6_3 ? editorDocument.querySelector('html') : false;
	...
	if ( document && document.body ) {
		const isSiteBuilderLayout = document.body.classList.contains( 'post-type-smarttoolz-advanced-hook' );
		if ( isSiteBuilderLayout ) {
			...
			if ( viewPostsIcon ) {
				viewPostsIcon.addEventListener( 'click', function(e) {
					e.preventDefault();
					window.location.href = smarttoolzColors.site_builder_url;
				});
			}
		}
	}
...
const updatePageBackground = ( apply_customizer_default = false, isUnboxedContainer = false, device = 'desktop' ) => {
	...
	if (_iframe && smarttoolzColors.ast_wp_version_higher_6_4) {
		editorDoc = smarttoolzGetIframeDoc( _iframe ) || editorDoc;
	}
	...
	if ( smarttoolzColors.ast_wp_version_higher_6_3 ) {
		...
		let iframe = undefined !== devicePreview ? devicePreview.getElementsByTagName('iframe')[0] : undefined;
		if ( iframe && devicePreview.querySelector('iframe') !== null ) {
			editorDoc = smarttoolzGetIframeDoc( iframe ) || editorDoc;
		}
	}
	...
	if ( 'desktop' === device ) {
		...
		const desktopCSS = smarttoolzGetResponsiveBackgroundObj(bgObj, 'desktop');
		...
		if ( smarttoolzColors.apply_content_bg_fullwidth && ( ! is_boxed_based_layout ) ) {

			/** Fullwidth with Content Bg */
			// Get the background object css values and update page content background.
			const desktopContentCSS = smarttoolzGetResponsiveBackgroundObj(contentObj, 'desktop');
			applyStylesToElement('.editor-styles-wrapper', desktopContentCSS, editorDoc );

		}
		else if ( ! smarttoolzColors.apply_content_bg_fullwidth && ( ! is_boxed_based_layout ) ) {
		...
		if ( smarttoolzColors.apply_content_bg_fullwidth && ( ! is_boxed_based_layout ) ) {
			...
			/** Fullwidth with Content Bg */
			// Get the background object css values and update page content background.
			const tabletContentCSS = smarttoolzGetResponsiveBackgroundObj(contentObj, 'tablet');
			...
			/** Fullwidth with Page Bg */
			// Get the background object css values and update page background.
			const tabletCSS = smarttoolzGetResponsiveBackgroundObj(bgObj, 'tablet');
		...
		if ( smarttoolzColors.apply_content_bg_fullwidth && ( ! is_boxed_based_layout ) ) {
			...
			/** Fullwidth with Content Bg */
			// Get the background object css values and update page content background.
			const mobileContentCSS = smarttoolzGetResponsiveBackgroundObj(contentObj, 'mobile');
			...
			/** Fullwidth with Page Bg */
			// Get the background object css values and update page background.
			const mobileCSS = smarttoolzGetResponsiveBackgroundObj(bgObj, 'mobile');
...
*/
function smarttoolzGetResponsiveBackgroundObj(bgObjRes, device) {
 const genBgCss = {};
