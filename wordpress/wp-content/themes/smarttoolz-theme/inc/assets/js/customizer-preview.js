( function( $, api ) {
    ...
    wp.customize.bind( 'preview-ready', function() {
		...
		$document.on(
			...
			function(e) {
				...
				if ( itemId && type ) {
					switch ( type ) {
						...
						case 'control':
							if ( defaultTarget.wp.customize.control( itemId ) ) {
								...
								if ( reference ) {
									switch (itemId) {
										case 'smarttoolz-color-palettes':
											defaultTarget.wp.customize.control( itemId ).container.find( '.components-button.smarttoolz-color-icon-indicate.open').click(); // Close all other opened pickers first.
											defaultTarget.wp.customize.control( itemId ).container.find( '.' + reference + ' .components-button.smarttoolz-color-icon-indicate').click();
										break;

										case 'smarttoolz-settings[ast-headings-font-settings]':
										case 'smarttoolz-settings[ast-body-font-settings]':
										case 'smarttoolz-settings[ast-heading-h1-typo]':
										case 'smarttoolz-settings[ast-heading-h2-typo]':
										case 'smarttoolz-settings[ast-heading-h3-typo]':
										case 'smarttoolz-settings[ast-heading-h4-typo]':
										case 'smarttoolz-settings[ast-heading-h5-typo]':
										case 'smarttoolz-settings[ast-heading-h6-typo]':
					...
					defaultTarget.wp.customize.state('smarttoolz-customizer-tab').set( context );
				}
			}
		);
         ...
         */
        wp.customize( 'smarttoolz-settings[woo-header-cart-click-action]', function( setting ) {
		...
		wp.customize.preview.bind( 'active', function() {
			var partials = $.extend({}, smarttoolzCustomizer.dynamic_partial_options), key;
     ...
     */
    wp.customize( 'smarttoolz-settings[logo-title-inline]', function( value ) {
...
wp.customize( 'smarttoolz-settings[blog-single-width]', function( value ) {
    value.bind( function( value ) {

        var single_post_max_width = wp.customize('smarttoolz-settings[blog-single-max-width]').get();
        ...
        smarttoolz_add_dynamic_css( 'blog-single-width', dynamicStyle );
    } );
} );

// Blog Post Content Width
wp.customize( 'smarttoolz-settings[blog-width]', function( value ) {
    value.bind( function( value ) {

        var blog_max_width = wp.customize('smarttoolz-settings[blog-max-width]').get();
        ...
        smarttoolz_add_dynamic_css( 'blog-width', dynamicStyle );
    } );
} );

// Blog Post Content Width
wp.customize( 'smarttoolz-settings[edd-archive-grids]', function( value ) {
    value.bind( function( value ) {
...
// Blog Post Content Width
wp.customize( 'smarttoolz-settings[edd-archive-width]', function( value ) {
    value.bind( function( value ) {

        var edd_archive_max_width = wp.customize('smarttoolz-settings[edd-archive-max-width]').get();
        ...
        smarttoolz_add_dynamic_css( 'edd-archive-width', dynamicStyle );
    } );
} );

// WooCommerce store notice color configs.
smarttoolz_css( 'smarttoolz-settings[store-notice-text-color]', 'color', 'body p.demo_store, body .woocommerce-store-notice, body p.demo_store a, body .woocommerce-store-notice a' );
smarttoolz_css( 'smarttoolz-settings[store-notice-background-color]', 'background-color', 'body p.demo_store, body .woocommerce-store-notice, body p.demo_store a, body .woocommerce-store-notice a' );

// WooCommerce store notice position preview.
wp.customize( 'smarttoolz-settings[store-notice-position]', function( setting ) {
...
wp.customize( 'smarttoolz-settings[blog-meta-date-type]', function( setting ) {
	setting.bind( function( val ) {
		wp.customize.preview.send( 'refresh' );
	} );
} );
wp.customize( 'smarttoolz-settings[blog-meta-date-format]', function( setting ) {
	setting.bind( function( val ) {
		wp.customize.preview.send( 'refresh' );
	} );
} );
...
smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-hover-effect]'
);

smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-image-ratio-type]'
);

smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-image-size]'
);

smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-image-ratio-pre-scale]'
);

smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-image-custom-scale-width]'
);

smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-image-custom-scale-height]'
);

smarttoolz_refresh_customizer(
    'smarttoolz-settings[blog-post-per-page]'
);
