<?php
/**
 * SmartToolz Builder UI Controller.
 *
...
if ( ! class_exists( 'SmartToolz_Builder_UI_Controller' ) ) {

	/**
	 * Class SmartToolz_Builder_UI_Controller.
	 */
	final class SmartToolz_Builder_UI_Controller {
		/**
		 * SmartToolz SVGs.
		 *
		 * @var mixed ast_svgs
		...
		public static function fetch_svg_icon( $icon = '', $base = true ) {
			...
			if ( ! self::$ast_svgs ) {
				ob_start();
				include_once SMARTTOOLZ_THEME_DIR . 'assets/svg/svgs.json'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
				self::$ast_svgs = json_decode( ob_get_clean(), true );
				self::$ast_svgs = apply_filters( 'smarttoolz_svg_icons', self::$ast_svgs );
			}
		...
		public static function render_social_icon( $index, $builder_type = 'header' ) {
			$items        = smarttoolz_get_option( $builder_type . '-social-icons-' . $index );
			$items        = isset( $items['items'] ) ? $items['items'] : array();
			$show_label   = smarttoolz_get_option( $builder_type . '-social-' . $index . '-label-toggle' );
			$color_type   = smarttoolz_get_option( $builder_type . '-social-' . $index . '-color-type' );
			$social_stack = smarttoolz_get_option( $builder_type . '-social-' . $index . '-stack', 'none' );
			...
			if ( is_array( $items ) && ! empty( $items ) ) {
				...
				foreach ( $items as $item ) {
					if ( $item['enabled'] ) {
						 ...
						 * @since 4.8.10
						 */
						$rel = apply_filters( 'smarttoolz_social_icon_attribute_rel', 'noopener noreferrer', $item );
						...
						if ( isset( $item['icon_type'] ) && 'custom' === $item['icon_type'] && ! empty( $item['custom_svg'] ) ) {
							// Use custom SVG
							echo '<span aria-hidden="true" class="ahfb-svg-iconset ast-inline-flex svg-baseline">' . wp_kses( $item['custom_svg'], SmartToolz_Icons::allowed_svg_args() ) . '</span>';
						} else {
							// Use icon library
			...
			echo wp_kses_post( apply_filters( 'smarttoolz_social_icons_after', '' ) );
			echo '</div>';
			echo '</div>';
		}
		...
		public static function render_html_markup( $index = 'header-html-1' ) {
			...
			$theme_author = smarttoolz_get_theme_author_details();

			$content = smarttoolz_get_i18n_option( $index, SmartToolz_Builder_Helper::get_translatable_string( $index ) );
			if ( $content || is_customize_preview() ) {
				...
				$allowed_html = smarttoolz_get_html_widget_allowed_tags( true, $content );
		...
		public static function render_customizer_edit_button( $class = '' ) { ?>
			<div class="customize-partial-edit-shortcut <?php echo esc_attr( $class ); ?>" data-id="ahfb">
				<button aria-label="<?php esc_attr_e( 'Click to edit this element.', 'smarttoolz' ); ?>"
						title="<?php esc_attr_e( 'Click to edit this element.', 'smarttoolz' ); ?>"
		...
		public static function render_grid_row_customizer_edit_button( $type, $row_position ) {
			...
			<div class="customize-partial-edit-shortcut row-editor-shortcut" data-id="ahfb">
				<button aria-label="<?php esc_attr_e( 'Click to edit this element.', 'smarttoolz' ); ?>"	title="<?php esc_attr_e( 'Click to edit this Row.', 'smarttoolz' ); ?>" class="item-customizer-focus">
					<?php echo self::fetch_svg_icon( 'edit' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</button>
			</div>
			<?php
		...
		public static function render_mobile_trigger() {
			...
			$icon             = smarttoolz_get_option( 'header-trigger-icon' );
			$mobile_label     = smarttoolz_get_i18n_option( 'mobile-header-menu-label', _x( '%smarttoolz%', 'Primary Menu: Menu Label (Mobile Menu)', 'smarttoolz' ) );
			$toggle_btn_style = smarttoolz_get_option( 'mobile-header-toggle-btn-style' );
			$aria_controls    = '';
			...
			<div class="ast-button-wrap">
				<button
					...
					aria-expanded="false"
					aria-label="<?php echo esc_attr__( 'Main menu toggle', 'smarttoolz' ); ?>"
					<?php echo esc_attr( apply_filters( 'smarttoolz_nav_toggle_data_attrs', '' ) ); ?>
		...
		public static function render_button( $index, $builder_type = 'header' ) {
			...
			$button_size = smarttoolz_get_option( $builder_type . '-button' . $index . '-size' );

			echo '<div class="ast-builder-button-wrap ast-builder-button-size-' . esc_attr( $button_size ) . '">';
			echo smarttoolz_get_custom_button( $builder_type . '-button' . $index . '-text', $builder_type . '-button' . $index . '-link-option', 'header-button' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public static function render_mobile_cart_flyout_markup() {
			...
			?>
			<div class="smarttoolz-mobile-cart-overlay"></div>
			<div id="smarttoolz-mobile-cart-drawer" class="smarttoolz-cart-drawer">
				<div class="smarttoolz-cart-drawer-header">
					<button tabindex="0" type="button" class="smarttoolz-cart-drawer-close" aria-label="<?php echo esc_attr__( 'Close Cart Drawer', 'smarttoolz' ); ?>">
							<?php echo self::fetch_svg_icon( 'close' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</button>
					<div class="smarttoolz-cart-drawer-title">
					<?php
						echo esc_html( apply_filters( 'smarttoolz_header_cart_flyout_shopping_cart_text', __( 'Shopping Cart', 'smarttoolz' ) ) );
					?>
					</div>
				</div>
				<div class="smarttoolz-cart-drawer-content <?php echo esc_attr( $is_width_long ); ?>">
					<?php
		...
		public static function render_account() {
			...
			$account_link     = '';
			$link_url         = '';
			$logout_preview   = smarttoolz_get_option( 'header-account-logout-preview' );
			$is_customizer    = is_customize_preview();
			$logged_out_style = smarttoolz_get_option( 'header-account-logout-style' );

			if ( ! $is_logged_in && 'none' === $logged_out_style ) {
				return;
			}

			$icon_skin = '' !== smarttoolz_get_option( 'header-account-icon-type' ) ? smarttoolz_get_option( 'header-account-icon-type' ) : 'account-1';
			...
			<div class="ast-header-account-wrap" tabindex="0">
				...
				if ( $is_logged_in && ( ( ! $logout_preview || ( 'none' === $logged_out_style && $logout_preview ) && $is_customizer ) || ( ! $is_customizer ) ) ) {
					...
					$account_type = smarttoolz_get_option( 'header-account-type' );

					$login_profile_type = smarttoolz_get_option( 'header-account-login-style' );

					$extend_text_profile_type = smarttoolz_get_option( 'header-account-login-style-extend-text-profile-type' );

					$action_type = smarttoolz_get_option( 'header-account-action-type' );
					$link_type   = smarttoolz_get_option( 'header-account-link-type' );

					$account_link = smarttoolz_get_option( 'header-account-login-link' );
					$show_menu    = smarttoolz_get_option( 'header-account-action-menu-display-on' );

					$logged_in_text = smarttoolz_get_i18n_option( 'header-account-logged-in-text', _x( '%smarttoolz%', 'Header Builder: Account Widget - Logged In View Text', 'smarttoolz' ) );

					$switch_to_default = true;
					if ( 'default' !== $account_type && 'default' === $link_type && defined( 'SMARTTOOLZ_EXT_VER' ) ) {
					...
					<div class="ast-header-account-inner-wrap">
						<a class="<?php echo esc_attr( implode( ' ', $link_classes ) ); ?>" role="<?php echo esc_attr( $role ); ?>" aria-label="<?php esc_attr_e( 'Account icon link', 'smarttoolz' ); ?>" <?php echo esc_attr( $link_href . ' ' . $new_tab . ' ' . $link_rel ); ?> >
						...
						<?php
						if ( defined( 'SMARTTOOLZ_EXT_VER' ) && 'menu' === $action_type ) {
							SmartToolz_Header_Account_Component::account_menu_markup();
						}
						?>
					</div>
				<?php } elseif ( ( 'none' !== $logged_out_style ) && ( ! $is_logged_in || ( $is_logged_in && $logout_preview && $is_customizer ) ) ) { ?>
					...
					<?php
					$action_type     = smarttoolz_get_option( 'header-account-logout-action' );
					$logged_out_text = smarttoolz_get_i18n_option( 'header-account-logged-out-text', _x( '%smarttoolz%', 'Header Builder: Account Widget - Logged Out View Text', 'smarttoolz' ) );
					$login_link      = smarttoolz_get_option( 'header-account-logout-link' );

					$extend_text_profile_type = smarttoolz_get_option( 'header-account-logout-style-extend-text-profile-type' );
					...
					?>
					<a class="<?php echo esc_attr( implode( ' ', $logged_out_style_class ) ); ?>" aria-label="<?php esc_attr_e( 'Account icon link', 'smarttoolz' ); ?>" <?php echo esc_attr( $link_href . ' ' . $new_tab . ' ' . $link_rel ); ?> >
					 ...
					 *
					 * @see SmartToolz Addon -> SmartToolz_Addon_Header_Account_Markup::login_popup_form_markup
					 */
					?>
				<?php } ?>
