<?php
/**
 * SmartToolz Builder Base Dynamic CSS.
 *
...
if ( ! class_exists( 'SmartToolz_Builder_Base_Dynamic_CSS' ) ) {

	/**
	 * Class SmartToolz_Builder_Base_Dynamic_CSS.
	 */
	final class SmartToolz_Builder_Base_Dynamic_CSS {
		/**
		 * Member Variable
		 *
		 * @var mixed instance
		...
		public function __construct() {
			...
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'footer_dynamic_css' ) );
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'mobile_header_logo_css' ) );
		}
		...
		public static function prepare_advanced_typography_css( $section_id, $selector ) {
			...
			$font_size = smarttoolz_get_option( 'font-size-' . $section_id );
		...
		public static function mobile_header_logo_css( $dynamic_css ) {
			...
			if ( '' !== $mobile_header_logo && '1' == $different_mobile_header_order ) {
				...
				.ast-header-break-point.ast-mobile-inherit-site-logo .ast-has-mobile-header-logo .smarttoolz-logo-svg {
					display: block;
				}';

				$dynamic_css .= SmartToolz_Enqueue_Scripts::trim_css( $mobile_header_css );
			}
			return $dynamic_css;
		}
		...
		public static function prepare_visibility_css( $section_id, $selector, $default_property = 'flex', $mobile_tablet_default = '' ) {
			...
			$parent_visibility = smarttoolz_get_option(
				$section_id . '-visibility-responsive',
				array(
					'desktop' => ! isset( $smarttoolz_options[ $section_id . '-visibility-responsive' ] ) && isset( $smarttoolz_options[ $section_id . '-hide-desktop' ] ) ? ( $smarttoolz_options[ $section_id . '-hide-desktop' ] ? 0 : 1 ) : 1,
					'tablet'  => ! isset( $smarttoolz_options[ $section_id . '-visibility-responsive' ] ) && isset( $smarttoolz_options[ $section_id . '-hide-tablet' ] ) ? ( $smarttoolz_options[ $section_id . '-hide-tablet' ] ? 0 : 1 ) : 1,
					'mobile'  => ! isset( $smarttoolz_options[ $section_id . '-visibility-responsive' ] ) && isset( $smarttoolz_options[ $section_id . '-hide-mobile' ] ) ? ( $smarttoolz_options[ $section_id . '-hide-mobile' ] ? 0 : 1 ) : 1,
				)
			);
			...
			/* Parse CSS from array() */
			$css_output  = smarttoolz_parse_css( $css_output_desktop );
			$css_output .= smarttoolz_parse_css( $css_output_tablet, '', number_format( absint( smarttoolz_get_tablet_breakpoint() ) + 0.99, 2, '.', '' ) );
			$css_output .= smarttoolz_parse_css( $css_output_mobile, '', smarttoolz_get_mobile_breakpoint() );
	...
	/**
	 *  Prepare if class 'SmartToolz_Builder_Base_Dynamic_CSS' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	SmartToolz_Builder_Base_Dynamic_CSS::get_instance();
}
