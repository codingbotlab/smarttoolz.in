 ...
 * Heading Colors
 */
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_fb_html_dynamic_css' );
 ...
 * @since 3.0.0
 */
function smarttoolz_fb_html_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	$dynamic_css     .= SmartToolz_Html_Component_Dynamic_CSS::smarttoolz_html_dynamic_css( 'footer' );
	$static_css_flg   = false;
	$stati_css_output = '';
	for ( $index = 1; $index <= SmartToolz_Builder_Helper::$num_of_footer_html; $index++ ) {

		if ( ! SmartToolz_Builder_Helper::is_component_loaded( 'html-' . $index, 'footer' ) ) {
			continue;
		}
		...
		$alignment = smarttoolz_get_option( 'footer-html-' . $index . '-alignment' );
	...
	if ( true === $static_css_flg ) {
		$stati_css_output .= smarttoolz_parse_css(
			array(
