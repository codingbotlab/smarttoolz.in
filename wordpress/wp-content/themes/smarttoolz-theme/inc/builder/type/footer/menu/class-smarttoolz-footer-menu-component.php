...
define( 'SMARTTOOLZ_BUILDER_FOOTER_MENU_DIR', SMARTTOOLZ_THEME_DIR . 'inc/builder/type/footer/menu' );
define( 'SMARTTOOLZ_BUILDER_FOOTER_MENU_URI', SMARTTOOLZ_THEME_URI . 'inc/builder/type/footer/menu' );
 ...
 * @since 3.0.0
 */
class SmartToolz_Footer_Menu_Component {
	/**
	...
	public function __construct() {
		...
		if ( ! is_admin() || SmartToolz_Builder_Customizer::smarttoolz_collect_customizer_builder_data() ) {
			require_once SMARTTOOLZ_BUILDER_FOOTER_MENU_DIR . '/dynamic-css/dynamic.css.php';
		}
	...
	public static function menu_markup() {
		...
		$menu_layout               = smarttoolz_get_option( 'footer-menu-layout' );
		...
		if ( ! empty( $desktop_menu_layout ) ) {
			$desktop_menu_layout_class = 'smarttoolz-footer-' . esc_attr( $desktop_menu_layout ) . '-menu';
		}

		if ( ! empty( $tablet_menu_layout ) ) {
			$tablet_menu_layout_class = 'smarttoolz-footer-tablet-' . esc_attr( $tablet_menu_layout ) . '-menu';
		}

		if ( ! empty( $mobile_menu_layout ) ) {
			$mobile_menu_layout_class = 'smarttoolz-footer-mobile-' . esc_attr( $mobile_menu_layout ) . '-menu';
		}
		 ...
		 * @var Array
		 */
		$menu_classes = apply_filters( 'smarttoolz_menu_classes', array( 'ast-nav-menu', 'ast-flex', $desktop_menu_layout_class, $tablet_menu_layout_class, $mobile_menu_layout_class ) );
		...
		if ( has_nav_menu( 'footer_menu' ) ) {
			wp_nav_menu(
				array(
					'depth'           => 1,
					'menu_id'         => 'smarttoolz-footer-menu',
 ...
 */
new SmartToolz_Footer_Menu_Component();
