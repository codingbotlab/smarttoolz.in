 ...
 * Heading Colors
 */
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_fb_widget_dynamic_css' );
 ...
 */
function smarttoolz_support_footer_widget_right_margin() {
	$smarttoolz_settings = smarttoolz_get_options();
	return apply_filters( 'smarttoolz_apply_right_margin_footer_widget_css', isset( $smarttoolz_settings['support-footer-widget-right-margin'] ) ? false : true );
}
 ...
 * @since 3.0.0
 */
function smarttoolz_fb_widget_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	for ( $index = 1; $index <= SmartToolz_Builder_Helper::$num_of_footer_widgets; $index++ ) {

		if ( ! SmartToolz_Builder_Helper::is_component_loaded( 'widget-' . $index, 'footer' ) ) {
			continue;
		}

		$selector = '.footer-widget-area[data-section="sidebar-widgets-footer-widget-' . $index . '"]';

		$alignment = smarttoolz_get_option( 'footer-widget-alignment-' . $index );
	...
	if ( smarttoolz_support_footer_widget_right_margin() && ! is_customize_preview() ) {
		...
		$dynamic_css           .= smarttoolz_parse_css( $footer_area_css_output );
	}

	$dynamic_css .= SmartToolz_Widget_Component_Dynamic_CSS::smarttoolz_widget_dynamic_css( 'footer' );
