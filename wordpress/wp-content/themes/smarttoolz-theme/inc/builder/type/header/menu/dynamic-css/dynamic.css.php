 ...
 * Heading Colors
 */
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_hb_menu_dynamic_css' );
 ...
 * @since 3.0.0
 */
function smarttoolz_hb_menu_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	for ( $index = 1; $index <= SmartToolz_Builder_Helper::$num_of_header_menu; $index++ ) {

		if ( ! SmartToolz_Builder_Helper::is_component_loaded( 'menu-' . $index, 'header' ) ) {
			continue;
		}
		...
		// Sub Menu.
		$sub_menu_border               = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-border' );
		$sub_menu_divider_toggle       = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-item-border' );
		$sub_menu_divider_size         = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-item-b-size' );
		$sub_menu_divider_color        = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-item-b-color' );
		$sub_menu_border_radius_fields = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-border-radius-fields' );
		$sub_menu_top_offset           = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-top-offset' );
		$sub_menu_width                = smarttoolz_get_option( 'header-' . $_prefix . '-submenu-width' );

		// Menu.
		$menu_resp_color           = smarttoolz_get_option( 'header-' . $_prefix . '-color-responsive' );
		$menu_resp_bg_color        = smarttoolz_get_option( 'header-' . $_prefix . '-bg-obj-responsive' );
		$menu_resp_color_hover     = smarttoolz_get_option( 'header-' . $_prefix . '-h-color-responsive' );
		$menu_resp_bg_color_hover  = smarttoolz_get_option( 'header-' . $_prefix . '-h-bg-color-responsive' );
		$menu_resp_color_active    = smarttoolz_get_option( 'header-' . $_prefix . '-a-color-responsive' );
		$menu_resp_bg_color_active = smarttoolz_get_option( 'header-' . $_prefix . '-a-bg-color-responsive' );
		...
		$menu_font_size       = smarttoolz_get_option( 'header-' . $_prefix . '-font-size' );
		$menu_font_weight     = smarttoolz_get_option( 'header-' . $_prefix . '-font-weight' );
		$menu_text_transform  = smarttoolz_get_font_extras( smarttoolz_get_option( 'header-' . $_prefix . '-font-extras' ), 'text-transform' );
		$menu_line_height     = smarttoolz_get_font_extras( smarttoolz_get_option( 'header-' . $_prefix . '-font-extras' ), 'line-height', 'line-height-unit' );
		$menu_letter_spacing  = smarttoolz_get_font_extras( smarttoolz_get_option( 'header-' . $_prefix . '-font-extras' ), 'letter-spacing', 'letter-spacing-unit' );
		$menu_text_decoration = smarttoolz_get_font_extras( smarttoolz_get_option( 'header-' . $_prefix . '-font-extras' ), 'text-decoration' );
		...
		// Spacing.
		$menu_spacing = smarttoolz_get_option( 'header-' . $_prefix . '-menu-spacing' );
		...
		$css_output_desktop = array(
			...
			// Menu.
			$selector                                    => array(
				'font-family'    => smarttoolz_get_font_family( $menu_font_family ),
			...
			$selector . ' .menu-item > .menu-link'       => array(
				...
				'font-size'       => smarttoolz_get_font_css_value( $menu_font_size_desktop, $menu_font_size_desktop_unit ),
				'color'           => $menu_resp_color_desktop,
				'padding-top'     => smarttoolz_responsive_spacing( $menu_spacing, 'top', 'desktop' ),
				'padding-bottom'  => smarttoolz_responsive_spacing( $menu_spacing, 'bottom', 'desktop' ),
				'padding-left'    => smarttoolz_responsive_spacing( $menu_spacing, 'left', 'desktop' ),
				'padding-right'   => smarttoolz_responsive_spacing( $menu_spacing, 'right', 'desktop' ),
			...
			$selector . ' .sub-menu, ' . $selector . ' .inline-on-mobile .sub-menu' => array(
				'border-top-width'           => smarttoolz_get_css_value( $sub_menu_border_top, 'px' ),
				'border-bottom-width'        => smarttoolz_get_css_value( $sub_menu_border_bottom, 'px' ),
				'border-right-width'         => smarttoolz_get_css_value( $sub_menu_border_right, 'px' ),
				'border-left-width'          => smarttoolz_get_css_value( $sub_menu_border_left, 'px' ),
				'border-color'               => esc_attr( smarttoolz_get_option( 'header-' . $_prefix . '-submenu-b-color', $theme_color ) ),
				'border-style'               => 'solid',
				'width'                      => smarttoolz_get_css_value( $sub_menu_width, 'px' ),
				'border-top-left-radius'     => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'top', 'desktop' ),
				'border-top-right-radius'    => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'right', 'desktop' ),
				'border-bottom-right-radius' => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', 'desktop' ),
				'border-bottom-left-radius'  => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'left', 'desktop' ),
			),
			// Fix submenu top offset when above border is assigned.
			$selector . ' .sub-menu .sub-menu'           => array(
				'top' => smarttoolz_get_css_value( strval( -1 * intval( $sub_menu_border_top ) ), 'px' ),
			),
			$selector . ' .main-header-menu > .menu-item > .sub-menu, ' . $selector . ' .main-header-menu > .menu-item > .smarttoolz-full-megamenu-wrapper' => array(
				'margin-top' => smarttoolz_get_css_value( $sub_menu_top_offset, 'px' ),
			),
			'.ast-desktop ' . $selector . ' .main-header-menu > .menu-item > .sub-menu:before, .ast-desktop ' . $selector . ' .main-header-menu > .menu-item > .smarttoolz-full-megamenu-wrapper:before' => array(
				'height' => smarttoolz_calculate_spacing( $sub_menu_top_offset . 'px', '+', intval( $sub_menu_border_top ) . 'px + 5', 'px' ),
			),
			$selector . ' .menu-item.menu-item-has-children > .ast-menu-toggle' => array(
				'top'   => smarttoolz_responsive_spacing( $menu_spacing, 'top', 'desktop' ),
				'right' => smarttoolz_calculate_spacing( smarttoolz_responsive_spacing( $menu_spacing, 'right', 'desktop' ), '-', '0.907', 'em' ),
			),
			// Margin CSS.
		...
		$css_output_desktop[ $selector . ' .main-header-menu, ' . $selector . ' .main-header-menu .sub-menu' ] = smarttoolz_get_responsive_background_obj( $menu_resp_bg_color, 'desktop' );

		$mobile_selector = '.ast-header-break-point .ast-builder-menu-' . $index;

		$menu_spacing_mobile_top = smarttoolz_responsive_spacing( $menu_spacing, 'top', 'mobile' );
		$menu_spacing_mobile_top = isset( $menu_spacing_mobile_top ) && ! empty( $menu_spacing_mobile_top ) ? $menu_spacing_mobile_top : 0;

		$menu_spacing_tablet_top = smarttoolz_responsive_spacing( $menu_spacing, 'top', 'tablet' );
		...
		$css_output_tablet = array(
			...
			$mobile_selector . ' .menu-item > .menu-link' => array(
				'font-size' => smarttoolz_get_font_css_value( $menu_font_size_tablet, $menu_font_size_tablet_unit ),
			),
			$mobile_selector . ' .main-header-menu .menu-item > .menu-link' => array(
				'padding-top'    => smarttoolz_responsive_spacing( $menu_spacing, 'top', 'tablet' ),
				'padding-bottom' => smarttoolz_responsive_spacing( $menu_spacing, 'bottom', 'tablet' ),
				'padding-left'   => smarttoolz_responsive_spacing( $menu_spacing, 'left', 'tablet' ),
				'padding-right'  => smarttoolz_responsive_spacing( $menu_spacing, 'right', 'tablet' ),
			),
			// Sub Menu.
			$selector . ' .sub-menu, ' . $selector . ' .inline-on-mobile .sub-menu' => array(
				'border-top-left-radius'     => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'top', 'tablet' ),
				'border-top-right-radius'    => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'right', 'tablet' ),
				'border-bottom-right-radius' => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', 'tablet' ),
				'border-bottom-left-radius'  => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'left', 'tablet' ),
			),
			...
			$mobile_selector . ' .menu-item.menu-item-has-children > .ast-menu-toggle' => array(
				...
				'right' => smarttoolz_calculate_spacing( smarttoolz_responsive_spacing( $menu_spacing, 'right', 'tablet' ), '-', '0.907', 'em' ),
			),
		...
		$css_output_tablet[ $selector . ' .main-header-menu, ' . $selector . ' .main-header-menu .sub-menu' ] = smarttoolz_get_responsive_background_obj( $menu_resp_bg_color, 'tablet' );

		$css_output_mobile = array(

			$mobile_selector . ' .menu-item > .menu-link' => array(
				'font-size' => smarttoolz_get_font_css_value( $menu_font_size_mobile, $menu_font_size_mobile_unit ),
			),
			$mobile_selector . ' .main-header-menu .menu-item > .menu-link' => array(
				'padding-top'    => smarttoolz_responsive_spacing( $menu_spacing, 'top', 'mobile' ),
				'padding-bottom' => smarttoolz_responsive_spacing( $menu_spacing, 'bottom', 'mobile' ),
				'padding-left'   => smarttoolz_responsive_spacing( $menu_spacing, 'left', 'mobile' ),
				'padding-right'  => smarttoolz_responsive_spacing( $menu_spacing, 'right', 'mobile' ),
			),
			// Sub Menu.
			$selector . ' .sub-menu, ' . $selector . ' .inline-on-mobile .sub-menu' => array(
				'border-top-left-radius'     => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'top', 'mobile' ),
				'border-top-right-radius'    => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'right', 'mobile' ),
				'border-bottom-right-radius' => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', 'mobile' ),
				'border-bottom-left-radius'  => smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'left', 'mobile' ),
			),
			$selector . ' .main-header-menu .menu-item > .menu-link' => array(
			...
			$mobile_selector . ' .menu-item.menu-item-has-children > .ast-menu-toggle' => array(
				...
				'right' => smarttoolz_calculate_spacing( smarttoolz_responsive_spacing( $menu_spacing, 'right', 'mobile' ), '-', '0.907', 'em' ),
			),
			// Margin CSS.
			$margin_selector                              => array(
		...
		$css_output_mobile[ $selector . ' .main-header-menu, ' . $selector . ' .main-header-menu .sub-menu' ] = smarttoolz_get_responsive_background_obj( $menu_resp_bg_color, 'mobile' );

		// Submenu border radius inner items fix.
		$sub_menu_border = strval( $sub_menu_border_top );
		$devices         = array( 'desktop', 'tablet', 'mobile' );
		foreach ( $devices as $device ) {
			$border_top_left_radius     = smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'top', $device );
			$border_top_right_radius    = smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'right', $device );
			$border_bottom_right_radius = smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', $device );
			$border_bottom_left_radius  = smarttoolz_responsive_spacing( $sub_menu_border_radius_fields, 'left', $device );
			...
			$css_output_var[ $selector . ' .sub-menu .menu-item:first-of-type > .menu-link, ' . $selector . ' .inline-on-mobile .sub-menu .menu-item:first-of-type > .menu-link' ] = array(
				'border-top-left-radius'  => $border_top_left_radius ? 'calc(' . $border_top_left_radius . ' - ' . smarttoolz_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
				'border-top-right-radius' => $border_top_right_radius ? 'calc(' . $border_top_right_radius . ' - ' . smarttoolz_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
			);
			...
			$css_output_var[ $selector . ' .sub-menu .menu-item:last-of-type > .menu-link, ' . $selector . ' .inline-on-mobile .sub-menu .menu-item:last-of-type > .menu-link' ] = array(
				'border-bottom-right-radius' => $border_bottom_right_radius ? 'calc(' . $border_bottom_right_radius . ' - ' . smarttoolz_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
				'border-bottom-left-radius'  => $border_bottom_left_radius ? 'calc(' . $border_bottom_left_radius . ' - ' . smarttoolz_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
			);
		}
		...
		$dynamic_css .= SmartToolz_Builder_Base_Dynamic_CSS::prepare_visibility_css( $_section, $selector );

	}

	$dynamic_css .= smarttoolz_menu_hover_style_css();
	return $dynamic_css;
}
 ...
 * @since 3.5.0
 */
function smarttoolz_menu_hover_style_css() {
	$hover_style_flg = false;
	$menu_hover_css  = '';
	for ( $index = 1; $index <= SmartToolz_Builder_Helper::$num_of_header_menu; $index++ ) {
		if ( '' !== smarttoolz_get_option( 'header-menu' . $index . '-menu-hover-animation' ) ) {
	...
	return SmartToolz_Enqueue_Scripts::trim_css( $menu_hover_css );
}
