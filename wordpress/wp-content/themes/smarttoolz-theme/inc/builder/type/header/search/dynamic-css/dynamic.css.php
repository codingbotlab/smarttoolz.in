 ...
 * Search
 */
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_hb_search_dynamic_css' );
 ...
 * @since 3.0.0
 */
function smarttoolz_hb_search_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	if ( ! SmartToolz_Builder_Helper::is_component_loaded( 'search', 'header' ) ) {
		return $dynamic_css;
	}

	$_section                = 'section-header-search';
	$selector                = '.ast-header-search';
	$icon_size               = smarttoolz_get_option( 'header-search-icon-space' );
	$search_width            = smarttoolz_get_option( 'header-search-width' );
	$search_type             = smarttoolz_get_option( 'header-search-box-type' );
	$live_search             = smarttoolz_get_option( 'live-search' );
	$search_width_applicable = ! defined( 'SMARTTOOLZ_EXT_VER' ) || ( defined( 'SMARTTOOLZ_EXT_VER' ) && ( 'slide-search' === $search_type || 'search-box' === $search_type ) ) ? true : false;
	...
	$icon_color_desktop = smarttoolz_get_prop( smarttoolz_get_option( 'header-search-icon-color' ), 'desktop' );
	$icon_color_tablet  = smarttoolz_get_prop( smarttoolz_get_option( 'header-search-icon-color' ), 'tablet' );
	$icon_color_mobile  = smarttoolz_get_prop( smarttoolz_get_option( 'header-search-icon-color' ), 'mobile' );
	...
	$css_output_desktop = array(
		...
		$selector . ' .smarttoolz-search-icon'               => array(
			'color'     => esc_attr( $icon_color_desktop ),
			'font-size' => smarttoolz_get_css_value( $icon_size_desktop, 'px' ),
		),
	...
	if ( $search_width_applicable ) {
		$css_output_desktop[ $selector . ' form.search-form .search-field, ' . $selector . ' .ast-dropdown-active.ast-search-menu-icon.slide-search input.search-field' ] = array(
			'width' => smarttoolz_get_css_value( smarttoolz_get_prop( $search_width, 'desktop' ), 'px' ),
		);
	}

	if ( $live_search && SmartToolz_Builder_Helper::is_component_loaded( 'search', 'header' ) && ! is_customize_preview() && apply_filters( 'smarttoolz_increased_search_icon_zindex', true ) ) {
	...
	$css_output_tablet = array(
		...
		$selector . ' .smarttoolz-search-icon' => array(
			...
			'font-size' => smarttoolz_get_css_value( $icon_size_tablet, 'px' ),
		),
		$selector . ' .search-field::placeholder,' . $selector . ' .ast-icon' => array(
			'color' => esc_attr( $icon_color_tablet ),
		),
	...
	if ( $search_width_applicable ) {
		$css_output_tablet[ $selector . ' form.search-form .search-field, ' . $selector . ' .ast-dropdown-active.ast-search-menu-icon.slide-search input.search-field, .ast-mobile-header-content .ast-search-menu-icon .search-form' ] = array(
			'width' => smarttoolz_get_css_value( smarttoolz_get_prop( $search_width, 'tablet' ), 'px' ),
		);
	}

	if ( $live_search && SmartToolz_Builder_Helper::is_component_loaded( 'search', 'header', 'mobile' ) ) {
	...
	$css_output_mobile = array(
		...
		$selector . ' .smarttoolz-search-icon' => array(
			...
			'font-size' => smarttoolz_get_css_value( $icon_size_mobile, 'px' ),
		),
		$selector . ' .search-field::placeholder,' . $selector . ' .ast-icon' => array(
			'color' => esc_attr( $icon_color_mobile ),
		),
	...
	if ( $search_width_applicable ) {
		$css_output_mobile[ $selector . ' form.search-form .search-field, ' . $selector . ' .ast-dropdown-active.ast-search-menu-icon.slide-search input.search-field, .ast-mobile-header-content .ast-search-menu-icon .search-form' ] = array(
			'width' => smarttoolz_get_css_value( smarttoolz_get_prop( $search_width, 'mobile' ), 'px' ),
		);
	}

	/* Parse CSS from array() */
	$css_output  = smarttoolz_search_static_css();
