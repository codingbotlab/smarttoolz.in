 ...
 *
 * SmartToolz is a very powerful theme and virtually anything can be customized
 * via a child theme.
 *
 * @package     SmartToolz
...
/**
 * SmartToolz_After_Setup_Theme initial setup
 *
 * @since 1.0.0
 */
if ( ! class_exists( 'SmartToolz_After_Setup_Theme' ) ) {

	/**
	 * SmartToolz_After_Setup_Theme initial setup
	 */
	class SmartToolz_After_Setup_Theme {
		/**
		 * Instance
		 *
		 * @var $instance
		...
		public function setup_theme() {
			...
			do_action( 'smarttoolz_class_loaded' );
			...
			if ( class_exists( 'SmartToolz_Starter_Content', false ) ) {
				$smarttoolz_starter_content = new SmartToolz_Starter_Content();
				add_theme_support( 'starter-content', $smarttoolz_starter_content->get() );
			}
			...
			$dir_name    = 'minified';
			$file_prefix = '.min';
			if ( apply_filters( 'smarttoolz_theme_editor_style', true ) ) {
				add_editor_style( 'assets/css/' . $dir_name . '/editor-style' . $file_prefix . '.css' );
			}

			if ( apply_filters( 'smarttoolz_fullwidth_oembed', true ) ) {
			...
			if ( true === apply_filters( 'smarttoolz_rank_math_theme_support', true ) ) {
				add_theme_support( 'rank-math-breadcrumbs' );
			}

			// Native AMP Support.
			if ( true === apply_filters( 'smarttoolz_amp_support', true ) ) {
				add_theme_support(
					'amp',
					apply_filters(
						'smarttoolz_amp_theme_features',
						array(
							'paired' => true,
						)
					)
			...
			// Let WooCommerce know, SmartToolz is not compatible with New Product Editor.
			add_filter( 'option_woocommerce_feature_product_block_editor_enabled', '__return_false' );

			add_filter( 'woocommerce_create_pages', array( $this, 'smarttoolz_enforce_woo_shortcode_pages' ), 99 );
		...
		public function modify_theme_palette_names( $theme_json ) {
			...
			if ( ! empty( $json_data['settings']['color']['palette']['theme'] ) ) {
				...
				$color_labels = SmartToolz_Global_Palette::get_palette_labels(); // Use the reusable function for labels.
				$name_pair    = array();
				foreach ( $color_labels as $index => $label ) {
					$name_pair[ 'ast-global-color-' . $index ] = $label;
				}

				// Core filters the child theme's theme.json when it has one, SmartToolz's own otherwise.
				$stylesheet_directory  = get_stylesheet_directory();
				$smarttoolz_owns_theme_json = get_template_directory() === $stylesheet_directory || ! file_exists( $stylesheet_directory . '/theme.json' );
				$smarttoolz_placeholders    = array();
				$child_theme_names     = array();

				if ( ! $smarttoolz_owns_theme_json ) {
					$child_theme_names = SmartToolz_Global_Palette::get_theme_json_palette_names( $stylesheet_directory );

					// Core translates palette names before this filter runs, so keep both forms.
					foreach ( SmartToolz_Global_Palette::get_theme_json_palette_names( get_template_directory() ) as $smarttoolz_slug => $placeholder ) {
						$smarttoolz_placeholders[ $smarttoolz_slug ] = array(
							$placeholder,
							translate_with_gettext_context( $placeholder, 'Color name', 'smarttoolz' ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText,WordPress.WP.I18n.LowLevelTranslationFunction -- Names come from SmartToolz's own theme.json, mirroring core's translation of that file.
						);
					}
				}
				...
				foreach ( $palette as $index => $color_data ) {
					...
					$smarttoolz_wrote_name = isset( $smarttoolz_placeholders[ $slug ] ) && in_array( $name, $smarttoolz_placeholders[ $slug ], true );
					...
					// SmartToolz relabels only the names it wrote; a child theme's own names stay.
					if ( $smarttoolz_owns_theme_json || '' === $name || $smarttoolz_wrote_name || $filtered_name ) {
		...
		public function init() {
			 ...
			 * to change 'smarttoolz' to the name of your theme in all the template files.
			 */
			load_theme_textdomain( 'smarttoolz', SMARTTOOLZ_THEME_DIR . '/languages' );
		}
			...
			if ( ! isset( $content_width ) ) {
				...
				if ( is_home() || is_post_type_archive( 'post' ) ) {
					...
					if ( 'custom' === $blog_width ) {
						$content_width = apply_filters( 'smarttoolz_content_width', smarttoolz_get_option( 'blog-max-width', 1200 ) );
					} else {
						$content_width = apply_filters( 'smarttoolz_content_width', smarttoolz_get_option( 'site-content-width', 1200 ) );
					}
				} elseif ( is_single() ) {
					...
					if ( 'post' === get_post_type() ) {
						...
						if ( 'custom' === $single_post_max ) {
							$content_width = apply_filters( 'smarttoolz_content_width', smarttoolz_get_option( 'blog-single-max-width', 1200 ) );
						} else {
							$content_width = apply_filters( 'smarttoolz_content_width', smarttoolz_get_option( 'site-content-width', 1200 ) );
						}
					}
		...
		public function responsive_oembed_wrapper( $html, $url, $attr, $core_yt_block = false ) {
			...
			$add_smarttoolz_oembed_wrapper = apply_filters( 'smarttoolz_responsive_oembed_wrapper_enable', true );
			$ast_embed_wrapper_class  = apply_filters( 'smarttoolz_embed_wrapper_class', '' );

			$allowed_providers = apply_filters(
				'smarttoolz_allowed_fullwidth_oembed_providers',
				array(
					'vimeo.com',
					'youtube.com',
					'youtu.be',
			...
			if ( smarttoolz_strposa( $url, $allowed_providers ) && $add_smarttoolz_oembed_wrapper ) {
				if ( $core_yt_block ) {
		 ...
		 * 2. Due to which most of SmartToolz extended features are not working on cart & checkout pages.
		 *
		 * This is temporary workaround, once SmartToolz ready with WooCommerce 8.3 version, this will be removed.
		 *
		 * @since 4.5.1
		 ...
		 * @return array
		 */
		public function smarttoolz_enforce_woo_shortcode_pages( $pages_data ) {
 ...
 */
SmartToolz_After_Setup_Theme::get_instance();
