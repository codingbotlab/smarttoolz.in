<?php
/**
 * SmartToolz Extended Base Dynamic CSS.
 *
 * @package SmartToolz
 */
...
if ( ! class_exists( 'SmartToolz_Extended_Base_Dynamic_CSS' ) ) {

	/**
	 * Class SmartToolz_Extended_Base_Dynamic_CSS.
	 */
	final class SmartToolz_Extended_Base_Dynamic_CSS {
		/**
		 * Member Variable
		 *
		 * @var mixed instance
		...
		public static function prepare_advanced_margin_padding_css( $section_id, $selector ) {
			...
			if ( ! empty( $section_id ) && ! empty( $selector ) ) {
				...
				$padding = smarttoolz_get_option( $section_id . '-padding' );
				$margin  = smarttoolz_get_option( $section_id . '-margin' );
		...
		public static function prepare_inner_section_advanced_css( $section_id, $selector ) {
			...
			if ( ! empty( $section_id ) && ! empty( $selector ) ) {
				...
				$color              = smarttoolz_get_option( $section_id . '-border-color', '' );
				$radius             = smarttoolz_get_option(
				...
				$css_output_desktop = array(
					$selector => array(
						...
						'border-top-width'           => ! empty( $width['top'] ) ? smarttoolz_get_css_value( $width['top'], 'px' ) : '',
						'border-bottom-width'        => ! empty( $width['bottom'] ) ? smarttoolz_get_css_value( $width['bottom'], 'px' ) : '',
						'border-left-width'          => ! empty( $width['left'] ) ? smarttoolz_get_css_value( $width['left'], 'px' ) : '',
						'border-right-width'         => ! empty( $width['right'] ) ? smarttoolz_get_css_value( $width['right'], 'px' ) : '',
						'border-top-left-radius'     => ! empty( $radius['top'] ) ? smarttoolz_get_css_value( $radius['top'], 'px' ) : '',
						'border-bottom-right-radius' => ! empty( $radius['bottom'] ) ? smarttoolz_get_css_value( $radius['bottom'], 'px' ) : '',
						'border-bottom-left-radius'  => ! empty( $radius['left'] ) ? smarttoolz_get_css_value( $radius['left'], 'px' ) : '',
						'border-top-right-radius'    => ! empty( $radius['right'] ) ? smarttoolz_get_css_value( $radius['right'], 'px' ) : '',
					),
				);
	...
	/**
	 *  Prepare if class 'SmartToolz_Extended_Base_Dynamic_CSS' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	SmartToolz_Extended_Base_Dynamic_CSS::get_instance();
}
