<?php
/**
 * SmartToolz Global color palette
 *
 * @package     SmartToolz
 * @subpackage  Class
 ...
 */
class SmartToolz_Global_Palette {
	/**
	...
	public function __construct() {
		...
		if ( ( get_template_directory() !== $get_stylesheet && false === file_exists( $is_theme_json ) ) || smarttoolz_wp_version_compare( '5.8', '<' ) ) {
			add_action( 'after_setup_theme', array( $this, 'support_editor_color_palette' ) );
		}
		add_filter( 'smarttoolz_theme_customizer_js_localize', array( $this, 'localize_variables' ) );
		add_filter( 'smarttoolz_before_foreground_color_generation', array( $this, 'get_color_by_palette_variable' ) );
		$this->includes();
		add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'global_border_compatibility' ) );
	}
	...
	public function format_global_palette( $global_palette ) {
		...
		if ( isset( $global_palette['palette'] ) ) {
			foreach ( $global_palette['palette'] as $key => $color ) {
				...
				// A newer SmartToolz version may have stored more palette slots than this
	...
	public function localize_variables( $object ) {
		...
		if ( isset( $object['customizer'] ) ) {
			...
			$object['customizer']['isElementorActive']              = smarttoolz_is_elemetor_active();
			$object['customizer']['isGlobalColorElementorDisabled'] = smarttoolz_maybe_disable_global_color_in_elementor();
	...
	public static function get_default_color_palette() {
		...
		$update_color_styles_with_presets  = SmartToolz_Dynamic_CSS::smarttoolz_4_5_0_compatibility();
	 ...
	 */
	public static function smarttoolz_get_active_global_palette() {
		$palettes_data   = smarttoolz_get_palette_colors();
		 ...
		 * @since 4.10.0
		 */
		return apply_filters( 'smarttoolz_get_active_global_palette_key', $current_palette );
	}
	...
	public static function is_dark_palette( $palette_key = 'current' ) {
		if ( $palette_key === 'current' ) {
			if ( ! self::$is_dark_palette ) {
				...
				self::$is_dark_palette = self::smarttoolz_get_active_global_palette() === 'palette_4';
			}
	...
	public static function get_palette_labels() {
		$smarttoolz_settings           = SmartToolz_Theme_Options::get_options();
		$color_palette_reorganize = SmartToolz_Dynamic_CSS::smarttoolz_4_8_9_compatibility();
		$new_color_palette_labels = apply_filters( 'smarttoolz_new_color_labels', $color_palette_reorganize || isset( $smarttoolz_settings['new-color-labels'] ) );

		$color_1_label = __( 'Brand', 'smarttoolz' );
		$color_2_label = __( 'Alternate Brand', 'smarttoolz' );
		$color_3_label = __( 'Headings', 'smarttoolz' );
		$color_4_label = __( 'Text', 'smarttoolz' );
		$color_5_label = $color_palette_reorganize ? __( 'Primary Background', 'smarttoolz' ) : __( 'Secondary Background', 'smarttoolz' );
		$color_6_label = $color_palette_reorganize ? __( 'Secondary Background', 'smarttoolz' ) : __( 'Primary Background', 'smarttoolz' );
		$color_7_label = $color_palette_reorganize ? __( 'Alternate Background', 'smarttoolz' ) : __( 'Subtle Background', 'smarttoolz' );
		$color_8_label = $color_palette_reorganize ? __( 'Subtle Background', 'smarttoolz' ) : __( 'Alternate Background', 'smarttoolz' );
		$color_9_label = __( 'Other Supporting', 'smarttoolz' );

		$labels = array(
			$new_color_palette_labels ? $color_1_label : __( 'Color 1', 'smarttoolz' ),
			$new_color_palette_labels ? $color_2_label : __( 'Color 2', 'smarttoolz' ),
			$new_color_palette_labels ? $color_3_label : __( 'Color 3', 'smarttoolz' ),
			$new_color_palette_labels ? $color_4_label : __( 'Color 4', 'smarttoolz' ),
			$new_color_palette_labels ? $color_5_label : __( 'Color 5', 'smarttoolz' ),
			$new_color_palette_labels ? $color_6_label : __( 'Color 6', 'smarttoolz' ),
			$new_color_palette_labels ? $color_7_label : __( 'Color 7', 'smarttoolz' ),
			$new_color_palette_labels ? $color_8_label : __( 'Color 8', 'smarttoolz' ),
			$new_color_palette_labels ? $color_9_label : __( 'Color 9', 'smarttoolz' ),
		);

		// Names a child theme declares in its own theme.json rename SmartToolz's palette slots.
		 ...
		 */
		$filtered_labels = apply_filters( 'smarttoolz_global_palette_labels', $labels );
		...
		// Overwrite in place so SmartToolz's own label survives anything the filter cannot supply.
	...
	public function includes() {
		require_once SMARTTOOLZ_THEME_DIR . 'inc/dynamic-css/global-color-palette.php';// PHPCS:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	}
	...
	public static function generate_global_palette_style() {
		$palette_data = smarttoolz_get_option( 'global-color-palette' );
	...
	public function global_border_compatibility( $dynamic_css ) {
		$global_border_color = smarttoolz_get_option( 'border-color', '#dddddd' );
		...
		return $dynamic_css . SmartToolz_Enqueue_Scripts::trim_css( $global_border );
	}
}

new SmartToolz_Global_Palette();
