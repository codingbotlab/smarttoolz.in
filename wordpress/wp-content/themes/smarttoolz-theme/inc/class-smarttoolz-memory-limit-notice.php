...
/**
 * Class SmartToolz_Memory_Limit_Notice
 */
class SmartToolz_Memory_Limit_Notice {
	/**
	...
	private function display_memory_notice( $notice_data ) {
		...
		$message  = '<div>';
		$message .= '<p><strong>' . esc_html__( '🔔 PHP Memory Limit Notice', 'smarttoolz' ) . '</strong></p>';
		$message .= '<p>' . esc_html__( 'Your site is nearing its PHP memory limit, which may affect stability.', 'smarttoolz' ) . '</p>';
		$message .= '<p>' . sprintf(
			esc_html__( 'We recommend increasing it to at least %s for best performance.', 'smarttoolz' ),
			'<strong>' . esc_html( $recommended_limit ) . '</strong>'
		) . '</p>';
		$message .= '<p><a href="https://wpsmarttoolz.com/docs/system-requirement-for-smarttoolz-theme/" target="_blank" rel="noopener">';
		$message .= esc_html__( 'Learn how to increase your PHP memory', 'smarttoolz' ) . '</a></p>';
		$message .= '</div>';

		$notice_args = array(
			'id'                  => 'smarttoolz-memory-limit-warning',
			'type'                => 'warning',
			'message'             => $message,
			'show_if'             => true,
			...
			'class'               => 'smarttoolz-memory-notice',
		);
	...
	public function add_site_health_tests( $tests ) {
		$tests['direct']['smarttoolz_memory_usage'] = array(
			'label' => __( 'SmartToolz Theme Memory Usage', 'smarttoolz' ),
	...
	public function site_health_memory_test() {
		...
		if ( $memory_percentage >= $this->warning_threshold ) {
			$status      = 'recommended';
			$label       = __( 'High memory usage detected', 'smarttoolz' );
			$badge_color = 'orange';
		} else {
			$status      = 'good';
			$label       = __( 'Memory usage is within acceptable limits', 'smarttoolz' );
			$badge_color = 'green';
		}
	...
	private function get_site_health_description( $memory_percentage, $memory_limit_formatted, $memory_usage_formatted, $memory_remaining_formatted, $percentage_formatted ) {
		...
		if ( $memory_percentage >= $this->warning_threshold ) {
			$description .= sprintf(
				__( 'Your site is using %1$s of %2$s available PHP memory (%3$s%%). Only %4$s remaining.', 'smarttoolz' ),
			...
			$description .= '</p><p>';
			$description .= __( 'While your site is currently functioning, this high memory usage puts you at risk of crashes and errors, especially when using memory-intensive features like the customizer, page builders, or plugins.', 'smarttoolz' );
		} else {
			$description .= sprintf(
				__( 'Your site is using %1$s of %2$s available PHP memory (%3$s%%). You have %4$s remaining.', 'smarttoolz' ),
				'<strong>' . esc_html( $memory_usage_formatted ) . '</strong>',
				'<strong>' . esc_html( $memory_limit_formatted ) . '</strong>',
				'<strong>' . esc_html( $percentage_formatted ) . '</strong>',
				'<strong>' . esc_html( $memory_remaining_formatted ) . '</strong>'
			...
			$description .= '</p><p>';
			$description .= __( 'Your memory usage is within acceptable limits. The SmartToolz theme and your plugins have sufficient memory to operate properly.', 'smarttoolz' );
		}

		$description .= '</p>';

		$description .= '<p>';
		$description .= __( '<strong>About PHP Memory:</strong> PHP memory limit determines how much memory your website can use. Themes, plugins, and WordPress core all consume memory. When the limit is reached, your site may display errors or stop working.', 'smarttoolz' );
		$description .= '</p>';
	...
	private function get_site_health_actions( $memory_percentage ) {
		...
		if ( $memory_percentage >= $this->warning_threshold ) {
			...
			$actions .= '<h4>' . __( 'How to increase PHP memory limit:', 'smarttoolz' ) . '</h4>';
			$actions .= '<ol>';
			$actions .= '<li><strong>' . __( 'Contact your hosting provider', 'smarttoolz' ) . '</strong><br>';
			$actions .= sprintf( __( 'The easiest solution is to ask your hosting provider to increase your PHP memory limit to at least %s.', 'smarttoolz' ), $recommended_limit ) . '</li>';

			$actions .= '<li><strong>' . __( 'Edit wp-config.php file', 'smarttoolz' ) . '</strong><br>';
			$actions .= __( 'Add this line to your wp-config.php file (before the "/* That\'s all, stop editing!" line):', 'smarttoolz' ) . '<br>';
			...
			$actions .= '<li><strong>' . __( 'Edit .htaccess file', 'smarttoolz' ) . '</strong><br>';
			$actions .= __( 'Add this line to your .htaccess file:', 'smarttoolz' ) . '<br>';
			...
			$actions .= '<li><strong>' . __( 'Edit php.ini file', 'smarttoolz' ) . '</strong><br>';
			$actions .= __( 'If you have access to php.ini, change this line:', 'smarttoolz' ) . '<br>';
			...
			$actions .= '<p><strong>' . __( 'Recommended memory limits:', 'smarttoolz' ) . '</strong></p>';
			$actions .= '<ul>';
			$actions .= '<li>' . __( 'Basic WordPress site: 256MB', 'smarttoolz' ) . '</li>';
			$actions .= '<li>' . __( 'Site with multiple plugins: 512MB', 'smarttoolz' ) . '</li>';
			$actions .= '<li>' . __( 'WooCommerce or complex site: 1GB or higher', 'smarttoolz' ) . '</li>';
			$actions .= '</ul>';

			$actions .= '<p>';
			$actions .= sprintf(
				__( 'For detailed instructions, visit our <a href="%s" target="_blank" rel="noopener">documentation on increasing PHP memory limit</a>.', 'smarttoolz' ),
				'https://wpsmarttoolz.com/docs/system-requirement-for-smarttoolz-theme/'
			);
			$actions .= '</p>';
		} else {
			$actions .= '<p>' . __( 'No action required. Your memory usage is within acceptable limits.', 'smarttoolz' ) . '</p>';
			$actions .= '<p>' . __( 'Continue monitoring your memory usage, especially when installing new plugins or themes.', 'smarttoolz' ) . '</p>';
		}
...
new SmartToolz_Memory_Limit_Notice();
