...
if ( ! class_exists( 'SmartToolz_Mobile_Header' ) ) {

	/**
	 * SmartToolz_Mobile_Header
	 *
	 * @since 1.4.0
	 */
	class SmartToolz_Mobile_Header {
		/**
		 * Instance
		 *
		 * @since 1.4.0
		...
		public function __construct() {
			add_action( 'smarttoolz_header', array( $this, 'mobile_header_markup' ), 5 );
			add_action( 'smarttoolz_customizer_save', array( $this, 'generate_mobile_header_logo' ) );
			add_action( 'body_class', array( $this, 'add_body_class' ) );
			add_filter( 'smarttoolz_main_menu_toggle_classes', array( $this, 'menu_toggle_classes' ) );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'toggle_button' ), 20, 4 );
			add_filter( 'smarttoolz_walker_nav_menu_start_el', array( $this, 'toggle_button' ), 10, 4 );
		}
		...
		public function toggle_button( $item_output, $item, $depth, $args ) {
			...
			for ( $index = 3; $index <= SmartToolz_Builder_Helper::$component_limit; $index++ ) {
				array_push( $menu_locations, 'menu_' . $index );
			}

			// Add toggle button if menu is from SmartToolz.
		...
		public function menu_arrow_button_markup( $item_output, $item ) {
			$item_output  = apply_filters( 'smarttoolz_toggle_button_markup', $item_output, $item );
			$item_output .= '<button ' . smarttoolz_attr(
				'ast-menu-toggle',
				array(
					'aria-expanded' => 'false',
					'aria-label'    => __( 'Toggle Menu', 'smarttoolz' ),
				),
				$item
			) . '>' . SmartToolz_Icons::get_icons( 'arrow' ) . '</button>';
		...
		public function menu_toggle_classes( $classes ) {
			return ' ast-mobile-menu-buttons-' . smarttoolz_get_option( 'mobile-header-toggle-btn-style' ) . ' ';
		}
		...
		public function mobile_header_markup() {
			...
			$different_logo     = smarttoolz_get_option( 'different-mobile-logo' );

			if ( '' !== $mobile_header_logo && '1' == $different_logo ) {
				add_filter( 'smarttoolz_has_custom_logo', '__return_true' );
				add_filter( 'get_custom_logo', array( $this, 'smarttoolz_mobile_header_custom_logo' ), 10, 2 );
				add_filter( 'smarttoolz_is_logo_attachment', array( $this, 'add_mobile_logo_svg_class' ), 10, 2 );
			}
		}
		 ...
		 */
		public function smarttoolz_mobile_header_custom_logo( $html, $blog_id ) {
		...
		public function generate_mobile_header_logo() {
			...
			/**
			 * Filter to disable SmartToolz logo image resizing.
			 *
			 * Shared with the desktop logo resize machinery in SmartToolz_Customizer::customize_save().
			 ...
			 *
			 * Example usage:
			 *     add_filter( 'smarttoolz_resize_logo', '__return_false' );
			 *
			 * @since 1.6.9
		...
		public function add_body_class( $classes ) {
			 ...
			 */
			$header_content_layout = smarttoolz_get_option( 'different-mobile-logo' );
	 ...
	 */
	SmartToolz_Mobile_Header::get_instance();
