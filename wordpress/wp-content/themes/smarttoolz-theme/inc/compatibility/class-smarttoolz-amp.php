...
/**
 * SmartToolz BB Ultimate Addon Compatibility
 */
if ( ! class_exists( 'SmartToolz_AMP' ) ) {

	/**
	 * Class SmartToolz_AMP
	 */
	class SmartToolz_AMP {
		/**
		 * Member Variable
		...
		public function __construct() {
			add_action( 'wp', array( $this, 'smarttoolz_amp_init' ) );
		}

		/**
		 * Init SmartToolz Amp Compatibility.
		 * This adds required actions and filters only if AMP endpoinnt is detected.
		 *
		 * @since 1.7.0
		 * @return void
		 */
		public function smarttoolz_amp_init() {
			...
			add_filter( 'smarttoolz_nav_toggle_data_attrs', array( $this, 'add_nav_toggle_attrs' ) );
			add_filter( 'smarttoolz_search_slide_toggle_data_attrs', array( $this, 'add_search_slide_toggle_attrs' ) );
			add_filter( 'smarttoolz_search_field_toggle_data_attrs', array( $this, 'add_search_field_toggle_attrs' ) );
			add_action( 'wp_footer', array( $this, 'render_amp_states' ) );
			add_filter( 'smarttoolz_attr_ast-main-header-bar-alignment', array( $this, 'nav_menu_wrapper' ) );
			add_filter( 'smarttoolz_attr_ast-menu-toggle', array( $this, 'menu_toggle_button' ), 20, 3 );
			add_filter( 'smarttoolz_theme_dynamic_css', array( $this, 'dynamic_css' ) );
			add_filter( 'smarttoolz_toggle_button_markup', array( $this, 'toggle_button_markup' ), 20, 2 );
			add_filter( 'smarttoolz_schema_body', array( $this, 'body_id' ) );
			 ...
			 * @since 4.0.0
			 */
			if ( true === smarttoolz_get_option( 'scroll-to-top-enable' ) ) {
				remove_action( 'wp_footer', array( SmartToolz_Scroll_To_Top_Loader::get_instance(), 'html_markup_loader' ) );
				remove_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_scroll_to_top_dynamic_css' );
			}
		}
		...
		public function body_id( $schema ) {
			return $schema . 'id="smarttoolz-body"';
		}
		...
		public function dynamic_css( $compiled_css ) {
			...
			$parse_css = $compiled_css . smarttoolz_parse_css( $css, '', smarttoolz_header_break_point() );
			...
			$parse_css .= smarttoolz_parse_css( $css, '', smarttoolz_header_break_point() );

			if ( true === SmartToolz_Builder_Helper::$is_header_footer_builder_active ) {

				$smarttoolz_break_point_navigation = array(
				...
				// Tablet CSS.
				$smarttoolz_medium_break_point_navigation = array(
					'.ast-amp .footer-sml-layout-2 .ast-small-footer-section-2' => array(
						'margin-top' => '1em',
					),
				);

				$parse_css .= smarttoolz_parse_css( $smarttoolz_medium_break_point_navigation, smarttoolz_get_tablet_breakpoint() );

				// Mobile CSS.
				$smarttoolz_small_break_point_navigation = array(
				...
				$parse_css .= smarttoolz_parse_css( $smarttoolz_small_break_point_navigation, '', smarttoolz_get_mobile_breakpoint() );
			}

			$parse_css .= smarttoolz_parse_css( $smarttoolz_break_point_navigation, '', smarttoolz_header_break_point() );
			...
			$smarttoolz_break_point_navigation = array(
				...
				'.ast-amp .ast-has-mobile-header-logo .custom-logo-link, .ast-amp .ast-has-mobile-header-logo .smarttoolz-logo-svg' => array(
					'display' => 'none',
				),
				'.ast-amp .ast-has-mobile-header-logo .custom-mobile-logo-link' => array(
					'display' => 'inline-block',
				),
				'.ast-theme.ast-mobile-inherit-site-logo .ast-has-mobile-header-logo .custom-logo-link, .ast-theme.ast-mobile-inherit-site-logo .ast-has-mobile-header-logo .smarttoolz-logo-svg' => array(
					'display' => 'block',
				),
			...
			if ( false === SmartToolz_Icons::is_svg_icons() ) {
				$smarttoolz_break_point_navigation['.ast-amp .main-navigation ul.children li a:before, .ast-amp .main-navigation ul.sub-menu li a:before']     = array(
					'content'         => '"\e900"',
					'font-family'     => '"SmartToolz"',
					'font-size'       => '0.65em',
					'text-decoration' => 'inherit',
				...
				$smarttoolz_break_point_navigation['.ast-amp .main-header-bar .main-header-bar-navigation .menu-item-has-children > .ast-menu-toggle::before'] = array(
					'font-weight'     => 'bold',
					'content'         => '"\e900"',
					'font-family'     => '"SmartToolz"',
					'text-decoration' => 'inherit',
				...
				$smarttoolz_break_point_navigation['[data-section="section-header-mobile-trigger"] .ast-button-wrap .ast-mobile-menu-buttons-minimal'] = array(
					'background' => 'transparent',
					'border'     => 'none',
				);
			}

			return $parse_css . smarttoolz_parse_css( $smarttoolz_break_point_navigation, '', smarttoolz_header_break_point() );
		}
		...
		public function nav_menu_wrapper( $attr ) {
			$attr['[class]']         = '( smarttoolzAmpMenuExpanded ? \'ast-main-header-bar-alignment toggle-on\' : \'ast-main-header-bar-alignment\' )';
			$attr['aria-expanded']   = 'false';
			$attr['[aria-expanded]'] = '(smarttoolzAmpMenuExpanded ? \'true\' : \'false\')';
		...
		public function toggle_button_markup( $item_output, $item ) {
			$item_output .= '<amp-state id="smarttoolzNavMenuItemExpanded' . esc_attr( $item->ID ) . '"><script type="application/json">false</script></amp-state>';
		...
		public function menu_toggle_button( $attr, $context, $args ) {
			$attr['[class]'] = '( smarttoolzNavMenuItemExpanded' . $args->ID . ' ? \' ast-menu-toggle dropdown-open\' : \'ast-menu-toggle\')';
			$attr['on']      = 'tap:AMP.setState( { smarttoolzNavMenuItemExpanded' . $args->ID . ': ! smarttoolzNavMenuItemExpanded' . $args->ID . ' } )';
		...
		public function render_amp_states() {
			echo '<amp-state id="smarttoolzAmpMenuExpanded">';
		...
		public function add_search_slide_toggle_attrs( $input ) {
			$input .= ' on="tap:AMP.setState( { smarttoolzAmpSlideSearchMenuExpanded: ! smarttoolzAmpSlideSearchMenuExpanded } )" ';
			$input .= ' [class]="( smarttoolzAmpSlideSearchMenuExpanded ? \'ast-search-menu-icon slide-search ast-dropdown-active\' : \'ast-search-menu-icon slide-search\' )" ';
			$input .= ' aria-expanded="false" [aria-expanded]="smarttoolzAmpSlideSearchMenuExpanded ? \'true\' : \'false\'" ';
		...
		public function add_nav_toggle_attrs( $input ) {
			$input .= ' on="tap:AMP.setState( { smarttoolzAmpMenuExpanded: ! smarttoolzAmpMenuExpanded } ),smarttoolz-body.toggleClass(class=ast-main-header-nav-open)" ';
			$input .= ' [class]="\'menu-toggle main-header-menu-toggle  ast-mobile-menu-buttons-minimal\' + ( smarttoolzAmpMenuExpanded ? \' toggled\' : \'\' )" ';
			$input .= ' aria-expanded="false" ';
			$input .= ' [aria-expanded]="smarttoolzAmpMenuExpanded ? \'true\' : \'false\'" ';
