...
/**
 * SmartToolz Beaver Themer Compatibility
 */
if ( ! class_exists( 'SmartToolz_Beaver_Themer' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class SmartToolz_Beaver_Themer {
		/**
		 * Member Variable
		 *
		 * @var object instance
		...
		public function __construct() {
			...
			add_action( 'smarttoolz_blog_archive_read_more_before', array( $this, 'reset_post_link_filter' ), 1 );
			...
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'beaver_themer_compatibility_styles' ) );
		}
		...
		public function beaver_themer_compatibility_styles( $dynamic_css ) {
			...
			if ( class_exists( 'FLBuilderModel' ) ) {
				...
				$parse_css = smarttoolz_parse_css( $beaver_themer_styles );
		...
		public function render_post_class( $classes ) {
			...
			if ( count( $result ) > 0 ) {
				...
				// Reset via reset_post_link_filter() before SmartToolz's own archive loop renders its Read More link.
				add_filter( 'smarttoolz_post_link_enabled', '__return_false' );
			}
		...
		/**
		 * Remove the stale `smarttoolz_post_link_enabled` filter added while rendering a Beaver Builder posts module,
		 * so SmartToolz's own archive loop can render its Read More link.
		 *
		 * @return void
		 * @since 4.13.9
		 */
		public function reset_post_link_filter() {
			remove_filter( 'smarttoolz_post_link_enabled', '__return_false' );
		}
		...
		public function theme_header_footer_render() {
			...
			if ( ! empty( $header_ids ) ) {
				...
				add_action( 'smarttoolz_header', 'FLThemeBuilderLayoutRenderer::render_header' );
			}
			...
			if ( ! empty( $footer_ids ) ) {
				...
				add_action( 'smarttoolz_footer', 'FLThemeBuilderLayoutRenderer::render_footer' );
			}
			...
			if ( ! empty( $template_ids ) ) {
				...
				if ( 'archive' === $template_type || 'singular' === $template_type || '404' === $template_type ) {
					...
					if ( 'disabled' === $footer_widgets ) {
						add_filter( 'smarttoolz_advanced_footer_disable', '__return_true' );
					}
				}
			}
		}
		...
		/**
		 * Function to SmartToolz theme parts
		 *
		 * @since 1.0.0
		 */
		public function register_part_hooks() {
			...
			return array(
				array(
					'label' => 'Page',
					'hooks' => array(
						'smarttoolz_body_top'    => __( 'Before Page', 'smarttoolz' ),
						'smarttoolz_body_bottom' => __( 'After Page', 'smarttoolz' ),
					),
				),
				array(
					'label' => 'Header',
					'hooks' => array(
						'smarttoolz_header_before' => __( 'Before Header', 'smarttoolz' ),
						'smarttoolz_header_after'  => __( 'After Header', 'smarttoolz' ),
					),
				),
				array(
					'label' => 'Content',
					'hooks' => array(
						'smarttoolz_primary_content_top'    => __( 'Before Content', 'smarttoolz' ),
						'smarttoolz_primary_content_bottom' => __( 'After Content', 'smarttoolz' ),
					),
				),
				array(
					'label' => 'Footer',
					'hooks' => array(
						'smarttoolz_footer_before' => __( 'Before Footer', 'smarttoolz' ),
						'smarttoolz_footer_after'  => __( 'After Footer', 'smarttoolz' ),
					),
				),
				array(
					'label' => 'Sidebar',
					'hooks' => array(
						'smarttoolz_sidebars_before' => __( 'Before Sidebar', 'smarttoolz' ),
						'smarttoolz_sidebars_after'  => __( 'After Sidebar', 'smarttoolz' ),
					),
				),
				array(
					'label' => 'Posts',
					'hooks' => array(
						'loop_start'                 => __( 'Loop Start', 'smarttoolz' ),
						'smarttoolz_entry_top'            => __( 'Before Post', 'smarttoolz' ),
						'smarttoolz_entry_content_before' => __( 'Before Post Content', 'smarttoolz' ),
						'smarttoolz_entry_content_after'  => __( 'After Post Content', 'smarttoolz' ),
						'smarttoolz_entry_bottom'         => __( 'After Post', 'smarttoolz' ),
						'smarttoolz_comments_before'      => __( 'Before Comments', 'smarttoolz' ),
						'smarttoolz_comments_after'       => __( 'After Comments', 'smarttoolz' ),
						'loop_end'                   => __( 'Loop End', 'smarttoolz' ),
					),
				),
			);
		}
		...
		public function builder_before_render_content( $post_id ) {
			...
			?>
			<?php if ( 'left-sidebar' === smarttoolz_page_layout() ) { ?>
		...
		public function builder_after_render_content( $post_id ) {
			...
			<?php if ( 'right-sidebar' === smarttoolz_page_layout() ) { ?>
 ...
 */
SmartToolz_Beaver_Themer::get_instance();
