...
/**
 * SmartToolz Elementor Compatibility
 */
if ( ! class_exists( 'SmartToolz_Elementor_Pro' ) ) {
	 ...
	 * @since 1.2.7
	 */
	class SmartToolz_Elementor_Pro {
		/**
		 * Member Variable
		 *
		 * @var object instance
		...
		public function __construct() {
			...
			add_action( 'smarttoolz_header', array( $this, 'do_header' ), 0 );
			add_action( 'smarttoolz_footer', array( $this, 'do_footer' ), 0 );
			add_action( 'smarttoolz_template_parts_content_top', array( $this, 'do_template_parts' ), 0 );

			add_action( 'smarttoolz_entry_content_404_page', array( $this, 'do_template_part_404' ), 0 );
			 ...
			 * @since  3.7.5
			 */
			add_filter( 'smarttoolz_theme_woocommerce_dynamic_css', array( $this, 'elementor_wc_widgets_compatibility_styles' ) );

			if ( $this->is_elementor_editor() && class_exists( 'WooCommerce' ) && smarttoolz_check_elementor_pro_3_5_version() ) {
				add_action( 'init', array( $this, 'update_woocommerce_checkout' ) );
			}

			add_filter( 'smarttoolz_shop_add_to_cart_js_localize', array( $this, 'smarttoolz_shop_add_to_cart_js_localize' ), 10, 1 );
		}
		 ...
		 * @return Array
		 */
		public function smarttoolz_shop_add_to_cart_js_localize( $localize_data ) {
		...
		public function elementor_wc_widgets_compatibility_styles( $css_output ) {
			...
			if ( ! smarttoolz_check_elementor_pro_3_5_version() ) {
				return $css_output;
			}
			...
			$css_output .= smarttoolz_parse_css( $woo_widgets_desktop_css );
		...
		public function do_template_parts() {
			...
			if ( $did_location ) {
				// Search and default.
				remove_action( 'smarttoolz_template_parts_content', array( \SmartToolz_Loop::get_instance(), 'template_parts_search' ) );// phpcs:ignore PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
				remove_action( 'smarttoolz_template_parts_content', array( \SmartToolz_Loop::get_instance(), 'template_parts_default' ) );// phpcs:ignore PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound

				// Remove pagination.
				remove_action( 'smarttoolz_pagination', 'smarttoolz_number_pagination' );
				remove_action( 'smarttoolz_entry_after', 'smarttoolz_single_post_navigation_markup' );

				// Content.
				remove_action( 'smarttoolz_entry_content_single', 'smarttoolz_entry_content_single_template' );
			}
			...
			if ( $did_location ) {
				...
				remove_action( 'smarttoolz_page_template_parts_content', array( \SmartToolz_Loop::get_instance(), 'template_parts_page' ) );
				remove_action( 'smarttoolz_template_parts_content', array( \SmartToolz_Loop::get_instance(), 'template_parts_post' ) );
				remove_action( 'smarttoolz_template_parts_content', array( \SmartToolz_Loop::get_instance(), 'template_parts_comments' ), 15 );
				remove_action( 'smarttoolz_page_template_parts_content', array( \SmartToolz_Loop::get_instance(), 'template_parts_comments' ), 15 );
		...
		public function do_template_part_404() {
			if ( is_404() ) {
				...
				$did_location = Module::instance()->get_locations_manager()->do_location( 'single' );
				if ( $did_location ) {
					remove_action( 'smarttoolz_entry_content_404_page', 'smarttoolz_entry_content_404_page_template' );
				}
			}
		}
		...
		public function override_with_post_meta( $post_id = 0 ) {
			...
			if ( 'disabled' === $title ) {
				...
				// Archive page.
				add_filter( 'smarttoolz_the_title_enabled', '__return_false', 99 );

				// Single page.
				add_filter( 'smarttoolz_the_title_enabled', '__return_false' );
				remove_action( 'smarttoolz_archive_header', 'smarttoolz_archive_page_info' );
			}
			...
			if ( 'disabled' === $footer_widgets ) {
				add_filter(
					'smarttoolz_advanced_footer_disable',
					static function() {
						return true;
					}
				);
			...
			if ( 'disabled' === $main_header_display ) {
				...
				add_filter(
					'smarttoolz_main_header_display',
		...
		public function do_header() {
			...
			if ( $did_location ) {
				...
				if ( true === \SmartToolz_Builder_Helper::$is_header_footer_builder_active ) { // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
					remove_action( 'smarttoolz_header', array( \SmartToolz_Builder_Header::get_instance(), 'header_builder_markup' ) ); // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
				}
			}
		}
		...
		public function do_footer() {
			...
			if ( $did_location ) {
				remove_action( 'smarttoolz_footer', 'smarttoolz_footer_markup' );
				if ( true === \SmartToolz_Builder_Helper::$is_header_footer_builder_active ) { // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
					remove_action( 'smarttoolz_footer', array( \SmartToolz_Builder_Footer::get_instance(), 'footer_markup' ) ); // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
				}
			}
		}
	 ...
	 */
	SmartToolz_Elementor_Pro::get_instance();
