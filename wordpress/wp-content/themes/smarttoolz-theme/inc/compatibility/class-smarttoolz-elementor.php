...
use SmartToolz_Dynamic_CSS;
use SmartToolz_Global_Palette;
 ...
 */
if ( ! class_exists( 'SmartToolz_Elementor' ) ) {
	...
	class SmartToolz_Elementor {
		...
		public function __construct() {
			 ...
			 * @since  2.4.5
			 */
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'enqueue_elementor_compatibility_styles' ) );
			...
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'generate_global_elementor_style' ), 11 );
			 ...
			 * @since  4.1.0
			 */
			add_filter( 'smarttoolz_entry_header_class', array( $this, 'smarttoolz_entry_header_class_custom' ), 1, 99 );
		}

		/**
		 * SmartToolz post layout 2 disable compatibility.
		 *
		 * @param array $classes Array of elementor edit mode check.
		 *
		 * @since 4.1.0
		 */
		public function smarttoolz_entry_header_class_custom( $classes ) {
			$edit_mode         = get_post_meta( smarttoolz_get_post_id(), '_elementor_edit_mode', true );
			$smarttoolz_layout_type = smarttoolz_get_option( 'ast-dynamic-single-' . get_post_type() . '-layout', 'layout-1' );

			if ( ( $edit_mode && $edit_mode === 'builder' ) || ( $edit_mode === 'builder' && $smarttoolz_layout_type === 'layout-2' ) ) {
				$classes[] = 'ast-header-without-markup';
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				if ( $smarttoolz_layout_type === 'layout-2' && in_array( 'ast-header-without-markup', $classes ) ) {
		...
		public function enqueue_elementor_compatibility_styles( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			if ( $this->is_elementor_activated( $id ) ) {
				...
				$parse_css = smarttoolz_parse_css( $elementor_heading_margin_comp );
				...
				if ( smarttoolz_can_remove_elementor_toc_margin_space() ) {
					$elementor_base_css['.elementor-toc__list-wrapper'] = array(
						'margin' => 0,
					);
				}

				if ( smarttoolz_can_add_styling_for_hr() ) {
				...
				$parse_css .= smarttoolz_parse_css( $elementor_rtl_support_css );
			...
			$dynamic_css .= smarttoolz_parse_css( $elementor_posts_container_css );
			...
			$dynamic_css               .= smarttoolz_parse_css( $elementor_archive_page_css );
		...
		public function elementor_default_setting() {
			...
			if ( false === smarttoolz_enable_page_builder_compatibility() || 'post' === get_post_type() ) {
				return;
			}
			...
			$page_builder_flag = get_post_meta( $id, '_smarttoolz_content_layout_flag', true );
			if ( isset( $post ) && empty( $page_builder_flag ) && ( is_admin() || is_singular() ) ) {

				if ( empty( $post->post_content ) && $this->is_elementor_activated( $id ) ) {

					update_post_meta( $id, '_smarttoolz_content_layout_flag', 'disabled' );
					 ...
					 * @since 4.11.11
					 */
					$use_default_settings = apply_filters( 'smarttoolz_elementor_use_default_settings', false, $id );
					...
					add_filter( 'smarttoolz_the_post_title_enabled', '__return_false' );
					add_filter( 'smarttoolz_featured_image_enabled', '__return_false' );
				}
			}
		}
		...
		public function elementor_add_theme_colors( $response, $handler, $request ) {
			...
			if ( smarttoolz_maybe_disable_global_color_in_elementor() ) {
				return $response;
			}
			...
			$slugs          = SmartToolz_Global_Palette::get_palette_slugs();
			$labels         = SmartToolz_Global_Palette::get_palette_labels();
		...
		public function display_global_colors_front_end( $response, $handler, $request ) {
			...
			$slug_map      = array();
			$palette_slugs = SmartToolz_Global_Palette::get_palette_slugs();
			...
			$colors = smarttoolz_get_option( 'global-color-palette' );
			return rest_ensure_response(
				array(
					'id'    => esc_attr( $rest_id ),
					'title' => SmartToolz_Global_Palette::get_css_variable_prefix() . esc_html( $slug_map[ $rest_id ] ),
		...
		public function generate_global_elementor_style( $dynamic_css ) {
			...
			if ( isset( $global_palette['palette'] ) ) {
				foreach ( $global_palette['palette'] as $color_index => $color ) {
					// Tolerate palette slots stored by a newer SmartToolz version ( see note above ).
			...
			// Apply SmartToolz Mini Cart CSS if Elementor Mini Cart Template is disabled.
			$is_site_rtl = is_rtl();
			...
			if ( defined( 'ELEMENTOR_PRO_VERSION' ) && 'no' === get_option( 'elementor_use_mini_cart_template' ) ) {
				...
				$dynamic_css           .= smarttoolz_parse_css( $mini_cart_template_css );
			}
			return $dynamic_css;
		}
		...
		public function elementor_add_scripts() {
			...
			wp_enqueue_style( 'smarttoolz-elementor-editor-style', SMARTTOOLZ_THEME_URI . 'inc/assets/css/ast-elementor-editor' . $style . '.css', array(), SMARTTOOLZ_THEME_VERSION );
		}
	}
 ...
 */
SmartToolz_Elementor::get_instance();
