 ...
 */
class SmartToolz_Starter_Content {
	...
	public function __construct() {
		...
		// Save smarttoolz settings into database.
		add_action(
			'customize_save_after',
			array(
				$this,
				'save_smarttoolz_settings',
			),
			10,
			3
		);
		...
		add_filter( 'smarttoolz_global_color_palette', array( $this, 'theme_color_palettes_defaults' ) );
	}
	...
	public function get_customizer_json() {
		...
		$file = SMARTTOOLZ_THEME_DIR . 'inc/compatibility/starter-content/smarttoolz-settings-export.json';
	...
	/**
	 *  Save SmartToolz customizer settings into database.
	 *
	 * @since 4.0.0
	 */
	public function save_smarttoolz_settings() {
		...
		// Clear the existing smarttoolz-settings only when the payload will replace it,
		// otherwise a payload without this key would wipe settings with nothing to restore.
		if ( isset( $settings['customizer-settings']['smarttoolz-settings'] ) ) {
			delete_option( 'smarttoolz-settings' );
		}
		 ...
		 * inc/compatibility/starter-content/smarttoolz-settings-export.json.
		 */
		$allowed_options = array( 'smarttoolz-settings', 'smarttoolz-color-palettes', 'smarttoolz-typography-presets' );
	...
	/**
	 * Load default smarttoolz settings.
	 *
	 * @since 4.0.0
	...
	public function theme_defaults( $defaults ) {
		...
		if ( ! empty( $settings['customizer-settings'] ) ) {
			$json = $settings['customizer-settings']['smarttoolz-settings'];
		}
	...
	public function theme_color_palettes_defaults( $defaults ) {
		...
		if ( ! empty( $settings['customizer-settings'] ) ) {
			$json = $settings['customizer-settings']['smarttoolz-color-palettes'];
		}
	...
	public function get() {
		...
		$nav_items_header = array(
			...
			'about'    => array(
				'title' => __( 'Services', 'smarttoolz' ),
				'type'  => 'custom',
				'url'   => '{{' . self::SERVICES_SLUG . '}}',
			),
			'services' => array(
				'title' => __( 'About', 'smarttoolz' ),
				'type'  => 'custom',
				'url'   => '{{' . self::ABOUT_SLUG . '}}',
			),
			'reviews'  => array(
				'title' => __( 'Reviews', 'smarttoolz' ),
				'type'  => 'custom',
				'url'   => '{{' . self::REVIEWS_SLUG . '}}',
			),
			'faq'      => array(
				'title' => __( 'Why Us', 'smarttoolz' ),
				'type'  => 'custom',
				'url'   => '{{' . self::WHY_US_SLUG . '}}',
			),
			'contact'  => array(
				'title' => __( 'Contact', 'smarttoolz' ),
				'type'  => 'custom',
		...
		$content = array(
			'attachments' => array(
				'logo' => array(
					'post_title' => _x( 'Logo', 'Theme starter content', 'smarttoolz' ),
			...
			'nav_menus'   => array(
				'primary'     => array(
					'name'  => esc_html__( 'Primary', 'smarttoolz' ),
			...
			'posts'       => array(
				self::HOME_SLUG => require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/starter-content/home.php', // PHPCS:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			),
		);

		return apply_filters( 'smarttoolz_starter_content', $content );
	}
}
