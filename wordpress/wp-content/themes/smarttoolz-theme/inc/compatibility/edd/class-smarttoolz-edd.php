...
/**
 * SmartToolz Easy Digital Downloads Compatibility
 */
if ( ! class_exists( 'SmartToolz_Edd' ) ) {
	...
	class SmartToolz_Edd {
		...
		public function __construct() {
			...
			require_once SMARTTOOLZ_THEME_DIR . 'inc/compatibility/edd/edd-common-functions.php';// phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			...
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'add_inline_styles' ) );
			...
			add_filter( 'smarttoolz_single_post_navigation', array( $this, 'edd_single_post_navigation' ) );

			// Header Cart Icon.
			add_action( 'smarttoolz_edd_header_cart_icons_before', array( $this, 'header_cart_icon_markup' ) );
			add_filter( 'smarttoolz_edd_cart_in_menu_class', array( $this, 'header_cart_icon_class' ), 99 );
		}
		...
		public function header_cart_icon_markup() {
			...
			$icon               = smarttoolz_get_option( 'edd-header-cart-icon' );
			$cart_total_display = smarttoolz_get_option( 'edd-header-cart-total-display' );
			$cart_count_display = apply_filters( 'smarttoolz_edd_header_cart_count', true );
			$cart_title_display = smarttoolz_get_option( 'edd-header-cart-title-display' );
			...
			$cart_icon = sprintf(
				'<span class="smarttoolz-icon ast-icon-shopping-%1$s %2$s"
							%3$s
						>%4$s</span>',
				$icon ? $icon : '',
			...
			if ( 'default' === $icon || ! defined( 'SMARTTOOLZ_EXT_VER' ) || ( defined( 'SMARTTOOLZ_EXT_VER' ) && ! SmartToolz_Ext_Extension::is_active( 'edd' ) ) ) {
				...
				add_filter( 'smarttoolz_edd_default_header_cart_icon', '__return_false' );
		...
		public function header_cart_icon_class( $classes ) {
			...
			$header_cart_icon_style = smarttoolz_get_option( 'edd-header-cart-icon-style' );

			$classes[]                  = 'ast-edd-menu-cart-' . $header_cart_icon_style;
			$header_cart_icon_has_color = smarttoolz_get_option( 'edd-header-cart-icon-color' );
		...
		public function edd_set_defaults_initialization() {
			...
			$smarttoolz_theme_options = smarttoolz_get_raw_options();
			$edd_settings        = get_option( 'edd_settings' );

			// Set flag to set the EDD style disable only once for the very first time.
			if ( ! isset( $smarttoolz_theme_options['ast-edd-disable-styles'] ) ) {
				$smarttoolz_theme_options['ast-edd-disable-styles'] = true;
				$edd_settings['disable_styles']                = true;
				update_option( 'smarttoolz-settings', $smarttoolz_theme_options );
		...
		public function edd_single_post_navigation( $args ) {
			$is_edd_single_product_page        = smarttoolz_is_edd_single_product_page();
			$disable_single_product_navigation = smarttoolz_get_option( 'disable-edd-single-product-nav' );
		...
		public function edd_initialization() {
			$is_edd_archive_page        = smarttoolz_is_edd_archive_page();
			$is_edd_single_product_page = smarttoolz_is_edd_single_product_page();

			if ( $is_edd_archive_page ) {
				add_action( 'smarttoolz_template_parts_content', array( $this, 'edd_content_loop' ) );
				remove_action( 'smarttoolz_template_parts_content', array( SmartToolz_Loop::get_instance(), 'template_parts_default' ) );

				// Add edd wrapper.
				add_action( 'smarttoolz_template_parts_content_top', array( $this, 'smarttoolz_edd_templat_part_wrap_open' ), 25 );
				add_action( 'smarttoolz_template_parts_content_bottom', array( $this, 'smarttoolz_edd_templat_part_wrap_close' ), 5 );

				// Remove closing and ending div 'ast-row'.
				remove_action( 'smarttoolz_template_parts_content_top', array( SmartToolz_Loop::get_instance(), 'smarttoolz_templat_part_wrap_open' ), 25 );
				remove_action( 'smarttoolz_template_parts_content_bottom', array( SmartToolz_Loop::get_instance(), 'smarttoolz_templat_part_wrap_close' ), 5 );
			}
			if ( $is_edd_single_product_page ) {
				remove_action( 'smarttoolz_template_parts_content', array( SmartToolz_Loop::get_instance(), 'template_parts_post' ) );

				add_action( 'smarttoolz_template_parts_content', array( $this, 'edd_single_template' ) );
		 ...
		 * @return void
		 */
		public function smarttoolz_edd_templat_part_wrap_open() {
			?>
		 ...
		 */
		public function smarttoolz_edd_templat_part_wrap_close() {
			?>
		...
		public function header_section_elements( $options ) {
			...
			$options['edd'] = __( 'Easy Digital Downloads', 'smarttoolz' );
		...
		public function edd_content_loop() {
			...
			<div <?php post_class(); ?>>
				 ...
				 */
				do_action( 'smarttoolz_edd_archive_product_content' );
				?>
			</div>
			<?php
		}
		...
		public function render_post_class( $classes ) {
			...
			if ( count( $result ) > 0 ) {
				$classes = array_diff(
					$classes,
					array(
						// SmartToolz common grid.
						'ast-col-sm-12',
						'ast-col-md-8',
						'ast-col-md-6',
						'ast-col-md-12',

						// SmartToolz Blog / Single Post.
						'ast-article-post',
						'ast-article-single',
						'ast-separate-posts',
		...
		public function smarttoolz_header_cart( $output, $section, $section_type ) {
			...
			if ( 'edd' === $section_type && apply_filters( 'smarttoolz_edd_header_cart_icon', true ) ) {
		...
		public function edd_mini_cart_markup() {
			...
			$cart_menu_classes = apply_filters( 'smarttoolz_edd_cart_in_menu_class', array( 'ast-menu-cart-with-border' ) );
			...
			<div class="ast-edd-site-header-cart <?php echo esc_attr( implode( ' ', $cart_menu_classes ) ); ?>">
				<div class="ast-edd-site-header-cart-wrap <?php echo esc_attr( $class ); ?>">
					<?php $this->smarttoolz_get_edd_cart(); ?>
				</div>
		 ...
		 * @since  1.5.5
		 */
		public function smarttoolz_get_edd_cart() {

			$view_shopping_cart = apply_filters( 'smarttoolz_edd_view_shopping_cart_title', __( 'View your shopping cart', 'smarttoolz' ) );
			$edd_cart_link      = apply_filters( 'smarttoolz_edd_cart_link', edd_get_checkout_uri() );
			...
			<a class="ast-edd-cart-container" href="<?php echo esc_url( $edd_cart_link ); ?>" title="<?php echo esc_attr( $view_shopping_cart ); ?>">
						...
						<?php
						do_action( 'smarttoolz_edd_header_cart_icons_before' );

						if ( apply_filters( 'smarttoolz_edd_default_header_cart_icon', true ) ) {
							?>
							<div class="ast-edd-cart-menu-wrap">
								<span class="count">
									<?php
									if ( apply_filters( 'smarttoolz_edd_header_cart_total', true ) ) {
										$cart_items = count( edd_get_cart_contents() );
						...
						do_action( 'smarttoolz_edd_header_cart_icons_after' );
		...
		public function add_styles( $assets ) {
			$assets['css']['smarttoolz-edd'] = SmartToolz_Builder_Helper::apply_flex_based_css() ? 'compatibility/edd-grid' : 'compatibility/edd';
			return $assets;
		}
		...
		public function add_inline_scripts() {
			...
			$edd_css_output = smarttoolz_parse_css( $max_tablet_edd_css, smarttoolz_get_mobile_breakpoint( '', 1 ), smarttoolz_get_tablet_breakpoint() );
			...
			if ( ! SmartToolz_Builder_Helper::apply_flex_based_css() ) {
				...
				/* Parse CSS from array() -> max-width: (tablet-breakpoint) px & min-width: (mobile-breakpoint + 1) px */
				$edd_css_output .= smarttoolz_parse_css( $max_tablet_edd_lang_direction_css, smarttoolz_get_mobile_breakpoint( '', 1 ), smarttoolz_get_tablet_breakpoint() );
			}
			...
			$edd_css_output .= smarttoolz_parse_css( $mobile_edd_css, '', smarttoolz_get_mobile_breakpoint() );
			...
			/* Parse CSS from array() -> max-width: (mobile-breakpoint) px */
			$edd_css_output .= smarttoolz_parse_css( $mobile_edd_lang_direction_css, '', smarttoolz_get_mobile_breakpoint() );

			wp_add_inline_style( 'smarttoolz-edd', apply_filters( 'smarttoolz_theme_edd_dynamic_css', $edd_css_output ) );
			...
			wp_add_inline_script(
				...
				"jQuery( document ).ready( function($) {
					/**
					 * SmartToolz - Easy Digital Downloads Cart Quantity & Total Amount
					 */
					var cartQuantity = jQuery('.ast-edd-site-header-cart-wrap .count'),
						iconQuantity = jQuery('.ast-edd-site-header-cart-wrap .smarttoolz-icon'),
		...
		public function add_inline_styles( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$edd_archive_width     = smarttoolz_get_option( 'edd-archive-width' );
			$edd_archive_max_width = smarttoolz_get_option( 'edd-archive-max-width' );
			$css_output            = '';
			...
			if ( 'custom' === $edd_archive_width ) {
				...
				$site_width  = array(
					'.ast-edd-archive-page .site-content > .ast-container' => array(
						'max-width' => smarttoolz_get_css_value( $edd_archive_max_width, 'px' ),
					),
				);
			...
			$dynamic_css .= apply_filters( 'smarttoolz_theme_edd_dynamic_css', $css_output );
		...
		public function theme_defaults( $defaults ) {
			...
			$defaults['edd-archive-add-to-cart-button-text'] = __( 'Add To Cart', 'smarttoolz' );
			$defaults['edd-archive-variable-button']         = 'button';
			$defaults['edd-archive-variable-button-text']    = __( 'View Details', 'smarttoolz' );
		...
		public function edd_products_item_class( $classes = '' ) {
			...
			if ( $is_edd_archive_page ) {
				$shop_grid = smarttoolz_get_option( 'edd-archive-grids' );
		...
		public function store_widgets_init() {
			register_sidebar(
				apply_filters(
					'smarttoolz_edd_sidebar_init',
					array(
						'name'          => esc_html__( 'Easy Digital Downloads Sidebar', 'smarttoolz' ),
						'id'            => 'smarttoolz-edd-sidebar',
			...
			register_sidebar(
				apply_filters(
					'smarttoolz_edd_single_product_sidebar_init',
					array(
						'name'          => esc_html__( 'EDD Single Product Sidebar', 'smarttoolz' ),
						'id'            => 'smarttoolz-edd-single-product-sidebar',
						'description'   => __( 'This sidebar will be used on EDD Single Product page.', 'smarttoolz' ),
						'before_widget' => '<div id="%1$s" class="widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h2 class="widget-title">',
						'after_title'   => '</h2>',
		...
		public function replace_store_sidebar( $sidebar ) {
			...
			$is_edd_page                = smarttoolz_is_edd_page();
			$is_edd_single_product_page = smarttoolz_is_edd_single_product_page();

			if ( $is_edd_page && ! $is_edd_single_product_page ) {
				$sidebar = 'smarttoolz-edd-sidebar';
			} elseif ( $is_edd_single_product_page ) {
				$sidebar = 'smarttoolz-edd-single-product-sidebar';
			}
		...
		public function store_sidebar_layout( $sidebar_layout ) {
			...
			if ( $is_edd_page ) {
				...
				// Global.
				$edd_sidebar = smarttoolz_get_option( 'site-sidebar-layout' );
				...
				// Customizer General.
				$edd_customizer_sidebar = smarttoolz_get_option( 'edd-general-sidebar-layout' );
				...
				if ( $is_edd_single_product_page ) {
					$edd_single_product_sidebar = smarttoolz_get_option( 'single-download-sidebar-layout' );
					...
					$edd_sidebar_layout = smarttoolz_get_option( 'archive-download-sidebar-layout' );
				} else {
					$edd_sidebar_layout = smarttoolz_get_option_meta( 'site-sidebar-layout', '', true );
				}
		...
		public function store_content_layout( $layout ) {
			...
			$is_edd_page         = smarttoolz_is_edd_page();
			$is_edd_single_page  = smarttoolz_is_edd_single_page();
			$is_edd_archive_page = smarttoolz_is_edd_archive_page();

			if ( $is_edd_page ) {

				// Global.
				$edd_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'global', false );
				...
				// Customizer General.
				$edd_customizer_layout = smarttoolz_toggle_layout( 'edd-ast-content-layout', 'global', false );
				...
				if ( $is_edd_single_page ) {
					$edd_single_product_layout = smarttoolz_toggle_layout( 'single-download-ast-content-layout', 'single', false );
					...
					$edd_page_layout = smarttoolz_toggle_layout( 'archive-download-ast-content-layout', 'archive', false );
				} else {
					$edd_page_layout = smarttoolz_get_option_meta( 'site-content-layout', '', true );
					if ( isset( $edd_page_layout ) ) {
						$edd_page_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', false, $edd_page_layout );
					} else {
						$edd_page_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', false );
					}
				}
		...
		public function customize_register( $wp_customize ) {
			 ...
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/edd/customizer/class-smarttoolz-customizer-register-edd-section.php';

			/**
			 * Sections
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/edd/customizer/sections/class-smarttoolz-edd-container-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/edd/customizer/sections/class-smarttoolz-edd-sidebar-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/edd/customizer/sections/layout/class-smarttoolz-edd-archive-layout-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/edd/customizer/sections/layout/class-smarttoolz-edd-single-product-layout-configs.php';
...
if ( apply_filters( 'smarttoolz_enable_edd_integration', true ) ) {
	SmartToolz_Edd::get_instance();
}
