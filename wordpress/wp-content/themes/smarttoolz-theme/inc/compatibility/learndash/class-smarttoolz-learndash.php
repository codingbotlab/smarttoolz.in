...
/**
 * SmartToolz LearnDash Compatibility
 */
if ( ! class_exists( 'SmartToolz_LearnDash' ) ) {
	...
	class SmartToolz_LearnDash {
		...
		public function __construct() {
			...
			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'add_dynamic_styles' ) );
		...
		public function add_dynamic_styles( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$link_forground_color  = smarttoolz_get_foreground_color( $link_color );
			$theme_forground_color = smarttoolz_get_foreground_color( $theme_color );
			...
			$css_output = array(
				...
				'.learndash .notcompleted:before, #learndash_profile .notcompleted:before, .learndash_topic_dots ul .topic-notcompleted span:before, .learndash_navigation_lesson_topics_list .topic-notcompleted span:before, .learndash_navigation_lesson_topics_list ul .topic-notcompleted span:before, .learndash .topic-notcompleted span:before' => array(
					'color' => smarttoolz_hex_to_rgba( $text_color, .5 ),
				),
				...
				'body .thumbnail.course .ld_course_grid_price:before' => array(
					'border-top-color'   => smarttoolz_hex_to_rgba( $theme_color, .75 ),
					'border-right-color' => smarttoolz_hex_to_rgba( $theme_color, .75 ),
				),
				'body .wpProQuiz_loadQuiz, body .wpProQuiz_lock' => array(
					'border-color'     => smarttoolz_hex_to_rgba( $link_color, .5 ),
					'background-color' => smarttoolz_hex_to_rgba( $link_color, .1 ),
				),
			...
			if ( ! smarttoolz_get_option( 'learndash-lesson-serial-number' ) ) {
			...
			if ( ! smarttoolz_get_option( 'learndash-differentiate-rows' ) ) {
			...
			$tablet_typography = array(
				...
				'#ld_course_list .entry-title' => array(
					'font-size' => smarttoolz_responsive_font( $archive_post_title_font_size, 'tablet', 30 ),
				),
			);
			/* Parse CSS from array()*/
			$css_output .= smarttoolz_parse_css( $tablet_typography, '', smarttoolz_get_tablet_breakpoint() );
			...
			$css_output .= smarttoolz_parse_css( $mobile_min_width_css, smarttoolz_get_mobile_breakpoint( '', 1 ) );
			...
			$mobile_typography = array(
				...
				'#ld_course_list .entry-title'          => array(
					'font-size' => smarttoolz_responsive_font( $archive_post_title_font_size, 'mobile', 30 ),
				),
			...
			$css_output .= smarttoolz_parse_css( $mobile_typography, '', smarttoolz_get_mobile_breakpoint() );
			...
			/* Parse CSS from array() -> max-width: (mobile-breakpoint) px */
			$css_output .= smarttoolz_parse_css( $mobile_typography_lang_direction_css, '', smarttoolz_get_mobile_breakpoint() );

			$dynamic_css .= apply_filters( 'smarttoolz_theme_learndash_dynamic_css', $css_output );
		...
		public function customize_register( $wp_customize ) {
			 ...
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/learndash/customizer/class-smarttoolz-customizer-register-learndash-section.php';

			/**
			 * Sections
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/learndash/customizer/sections/class-smarttoolz-learndash-container-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/learndash/customizer/sections/class-smarttoolz-learndash-sidebar-configs.php';

			if ( 'ld30' !== $active_ld_theme ) {
				require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/learndash/customizer/sections/layout/class-smarttoolz-learndash-general-configs.php';
			}
		...
		public function add_styles( $assets ) {
			$assets['css']['smarttoolz-learndash'] = 'compatibility/learndash';
			return $assets;
		}
		...
		public function sidebar_layout( $layout ) {
			...
			if ( is_singular( 'sfwd-courses' ) || is_singular( 'sfwd-lessons' ) || is_singular( 'sfwd-topic' ) || is_singular( 'sfwd-quiz' ) || is_singular( 'sfwd-certificates' ) || is_singular( 'sfwd-assignment' ) ) {
				...
				$learndash_sidebar = smarttoolz_get_option( 'learndash-sidebar-layout' );
				...
				$sidebar = smarttoolz_get_option_meta( 'site-sidebar-layout', '', true );
			...
			if ( $learndash_shortcode_used && ! ( is_singular( 'sfwd-courses' ) || is_singular( 'sfwd-lessons' ) || is_singular( 'sfwd-topic' ) || is_singular( 'sfwd-quiz' ) || is_singular( 'sfwd-certificates' ) || is_singular( 'sfwd-assignment' ) ) ) {
				...
				if ( ! did_action( 'smarttoolz_header' ) ) {
					// Page Meta Sidebar.
					$layout = smarttoolz_get_option_meta( 'site-sidebar-layout', '', true );
					if ( empty( $layout ) ) {
						// Page Sidebar.
						$layout = smarttoolz_get_option( 'single-page-sidebar-layout' );
						// Default Site Sidebar.
						if ( 'default' === $layout || empty( $layout ) ) {
							// Get the global sidebar value.
							// NOTE: Here not used `true` in the below function call.
							$layout = smarttoolz_get_option( 'site-sidebar-layout' );
						}
					}
				}
			}
		...
		public function content_layout( $layout ) {
			...
			if ( is_singular( 'sfwd-courses' ) || is_singular( 'sfwd-lessons' ) || is_singular( 'sfwd-topic' ) || is_singular( 'sfwd-quiz' ) || is_singular( 'sfwd-certificates' ) || is_singular( 'sfwd-assignment' ) ) {
				...
				$learndash_layout = smarttoolz_toggle_layout( 'learndash-ast-content-layout', 'global', false );
				...
				$learndash_layout = smarttoolz_get_option_meta( 'site-content-layout', '', true );
				if ( isset( $learndash_layout ) ) {
					$learndash_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', false, $learndash_layout );
				} else {
					$learndash_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', false );
				}
		...
		public static function ld_static_css() {
			...
			return SmartToolz_Enqueue_Scripts::trim_css( $ld_static_css );
		}
	}

}

if ( apply_filters( 'smarttoolz_enable_learndash_integration', true ) ) {

	/**
	 * Kicking this off by calling 'get_instance()' method
	 */
	SmartToolz_LearnDash::get_instance();
}
