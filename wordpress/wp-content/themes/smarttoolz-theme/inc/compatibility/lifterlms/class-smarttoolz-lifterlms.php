...
/**
 * SmartToolz Lifter LMS Compatibility
 */
if ( ! class_exists( 'SmartToolz_LifterLMS' ) ) {
	...
	class SmartToolz_LifterLMS {
		...
		public function __construct() {
			...
			// Sidebar Layout.
			add_filter( 'smarttoolz_page_layout', array( $this, 'sidebar_layout' ) );
			// Content Layout.
			add_filter( 'smarttoolz_get_content_layout', array( $this, 'content_layout' ) );
		...
		public function lifterlms_init() {
			...
			if ( is_lesson() ) {
				...
				remove_action( 'smarttoolz_entry_after', 'smarttoolz_single_post_navigation_markup' );
				if ( 'yes' !== apply_filters( 'llms_blocks_is_post_migrated', get_post_meta( get_the_ID(), '_llms_blocks_migrated', true ), get_the_ID() ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
					add_action( 'smarttoolz_entry_after', 'lifterlms_template_lesson_navigation' );
				}
			}
			...
			if ( 'llms_certificate' === $lifter_certificate_post_type || 'llms_my_certificate' === $lifter_certificate_post_type ) {
				if ( ! is_admin() ) {
					add_filter( 'post_class', 'smarttoolz_certificate_class' );
					 ...
					 * @return array
					 */
					function smarttoolz_certificate_class( $array ) {
		...
		public function customize_register( $wp_customize ) {
			 ...
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/lifterlms/customizer/class-smarttoolz-liferlms-section-configs.php';

			/**
			 * Sections
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/lifterlms/customizer/sections/class-smarttoolz-lifter-container-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/lifterlms/customizer/sections/class-smarttoolz-lifter-sidebar-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/lifterlms/customizer/sections/layout/class-smarttoolz-lifter-general-configs.php';
		...
		public function single_reviews() {
			...
			if ( get_post_meta( get_the_ID(), '_llms_display_reviews', true ) ) {
				?>
				<div id="old_reviews">
				<h3><?php echo esc_html( apply_filters( 'lifterlms_reviews_section_title', __( 'What Others Have Said', 'smarttoolz' ) ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound ?></h3>
				<?php
				$args = array(
				...
				foreach ( $posts_array as $post ) {
					...
					<div class="llms_review" style="background-color:<?php echo esc_attr( $styles['background-color'] ); ?>;">
						...
						<h6 style="color:<?php echo esc_attr( $styles['text-color'] ); ?>;"><?php echo esc_html( sprintf( __( 'By: %s', 'smarttoolz' ), get_the_author_meta( 'display_name', absint( $post->post_author ) ) ) ); ?></h6>
			...
			if ( get_post_meta( get_the_ID(), '_llms_reviews_enabled', true ) && is_user_logged_in() ) {
				...
				if ( get_post_meta( get_the_ID(), '_llms_multiple_reviews_disabled', true ) && $posts_array ) {
					...
					<div id="thank_you_box">
						<h2><?php echo esc_html( apply_filters( 'llms_review_thank_you_text', __( 'Thank you for your review!', 'smarttoolz' ) ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound ?></h2>
					</div>
					<?php
				} else {
					...
					<h3><?php esc_html_e( 'Write a Review', 'smarttoolz' ); ?></h3>
						<input type="text" name="review_title" placeholder="<?php esc_attr_e( 'Review Title', 'smarttoolz' ); ?>" id="review_title">
						<h5 style="color:red; display:none" id="review_title_error"><?php esc_html_e( 'Review Title is required.', 'smarttoolz' ); ?></h5>
						<textarea name="review_text" placeholder="<?php esc_attr_e( 'Review Text', 'smarttoolz' ); ?>" id="review_text"></textarea>
						<h5 style="color:red; display:none" id="review_text_error"><?php esc_html_e( 'Review Text is required.', 'smarttoolz' ); ?></h5>
						...
						<input type="submit" class="button" value="<?php esc_attr_e( 'Leave Review', 'smarttoolz' ); ?>" id="llms_review_submit_button">
					</div>
		...
		public function course_grid( $grid ) {
			...
			$course_grid = smarttoolz_get_option( 'llms-course-grid' );
		...
		public function course_responsive_grid( $classes ) {
			...
			$llms_grid = smarttoolz_get_option( 'llms-course-grid' );
			if ( in_array( 'llms-membership-list', $classes ) ) {
				$llms_grid = smarttoolz_get_option( 'llms-membership-grid' );
		...
		public function add_dynamic_styles() {
			...
			$theme_forground_color = smarttoolz_get_foreground_color( $link_color );
			...
			$btn_bg_color   = smarttoolz_get_option( 'button-bg-color', '', $link_color );
			...
			$btn_border_color   = smarttoolz_get_option( 'theme-button-border-group-border-color', $link_color );
			$btn_border_h_color = smarttoolz_get_option( 'theme-button-border-group-border-h-color', $link_h_color );
			...
			$theme_btn_font_extras     = smarttoolz_get_option( 'font-extras-button' );
			$theme_btn_text_transform  = smarttoolz_get_font_extras( $theme_btn_font_extras, 'text-transform' );
			$theme_btn_line_height     = smarttoolz_get_font_extras( $theme_btn_font_extras, 'line-height', 'line-height-unit' );
			$theme_btn_letter_spacing  = smarttoolz_get_font_extras( $theme_btn_font_extras, 'letter-spacing', 'letter-spacing-unit' );
			$theme_btn_text_decoration = smarttoolz_get_font_extras( $theme_btn_font_extras, 'text-decoration' );
			...
			$css_output = array(
				'.lifterlms a.llms-button-primary, .lifterlms a.llms-button-secondary, .lifterlms .llms-button-action, .lifterlms button.llms-field-button, .lifterlms a.llms-field-button' => array(
					...
					'border-top-width'           => isset( $btn_border_size['top'] ) && '' !== $btn_border_size['top'] ? smarttoolz_get_css_value( $btn_border_size['top'], 'px' ) : '0',
					'border-right-width'         => isset( $btn_border_size['right'] ) && '' !== $btn_border_size['right'] ? smarttoolz_get_css_value( $btn_border_size['right'], 'px' ) : '0',
					'border-left-width'          => isset( $btn_border_size['left'] ) && '' !== $btn_border_size['left'] ? smarttoolz_get_css_value( $btn_border_size['left'], 'px' ) : '0',
					'border-bottom-width'        => isset( $btn_border_size['bottom'] ) && '' !== $btn_border_size['bottom'] ? smarttoolz_get_css_value( $btn_border_size['bottom'], 'px' ) : '0',
			...
			wp_add_inline_style( 'lifterlms-styles', apply_filters( 'smarttoolz_theme_lifterlms_dynamic_css', $css_output ) );
		}
		...
		public function add_styles( $assets ) {
			$assets['css']['smarttoolz-lifterlms'] = ! SmartToolz_Builder_Helper::apply_flex_based_css() ? 'compatibility/lifterlms' : 'compatibility/lifterlms-flex';
			return $assets;
		}
		...
		public function sidebar_layout( $layout ) {
			...
			if ( is_lifterlms() || is_llms_account_page() || is_llms_checkout() ) {
				...
				$llms_sidebar = smarttoolz_get_option( 'lifterlms-sidebar-layout' );
				if ( is_lesson() || is_course() ) {
					$llms_sidebar = smarttoolz_get_option( 'lifterlms-course-lesson-sidebar-layout' );
				}
				...
				if ( in_array( $post_type, $supported_post_types ) ) {
					...
					if ( is_singular() ) {
						$dynamic_sidebar_layout = smarttoolz_get_option( 'single-' . $post_type . '-sidebar-layout' );
					}

					if ( is_archive() ) {
						$dynamic_sidebar_layout = smarttoolz_get_option( 'archive-' . $post_type . '-sidebar-layout' );
					}
		...
		public function content_layout( $layout ) {
			...
			if ( is_lifterlms() || is_llms_account_page() || is_llms_checkout() ) {
				...
				$llms_layout = smarttoolz_toggle_layout( 'lifterlms-ast-content-layout', 'global', false );
				...
				if ( in_array( $post_type, $supported_post_types ) ) {
					...
					if ( is_singular() ) {
						$dynamic_sidebar_layout = smarttoolz_toggle_layout( 'single-' . $post_type . '-ast-content-layout', 'single', false );
					}
					if ( is_archive() ) {
						$dynamic_sidebar_layout = smarttoolz_toggle_layout( 'archive-' . $post_type . '-ast-content-layout', 'archive', false );
					}
		...
		public function register_builder_fields( $default_fields ) {
			...
			$show_meta_field = ! SmartToolz_Meta_Boxes::is_bb_themer_layout();
			/**
			 * Main Header.
			 */
			$disable_fields[] = array(
				...
				'label'     => esc_html__( 'Disable Primary Header', 'smarttoolz' ),
				'switch_on' => 'disabled',
				'type'      => 'switch',
			);
			/**
			...
			$disable_fields[] = array(
				...
				'label'     => esc_html__( 'Disable Above Header', 'smarttoolz' ),
				'switch_on' => 'disabled',
				'type'      => 'switch',
			);
			/**
			...
			$disable_fields[] = array(
				...
				'label'     => esc_html__( 'Disable Below Header', 'smarttoolz' ),
				'switch_on' => 'disabled',
				'type'      => 'switch',
			);
			/**
			...
			$disable_fields[] = array(
				...
				'label'     => esc_html__( 'Disable Mobile Header', 'smarttoolz' ),
				'switch_on' => 'disabled',
				'type'      => 'switch',
			);
			if ( $show_meta_field ) {
				$disable_fields[] = array(
					...
					'id'        => 'site-post-title',
					'label'     => esc_html__( 'Disable Title', 'smarttoolz' ),
					'switch_on' => 'disabled',
					'type'      => 'switch',
				);
			}
			if ( $show_meta_field && 'disabled' !== smarttoolz_get_option( 'footer-adv' ) ) {
				$disable_fields[] = array(
					'attribute' => 'footer-adv-display',
					'id'        => 'footer-adv-display',
					'label'     => esc_html__( 'Disable Footer Widgets', 'smarttoolz' ),
					'switch_on' => 'disabled',
					'type'      => 'switch',
				);
			}
			if ( 'disabled' !== smarttoolz_get_option( 'footer-sml-layout' ) ) {
				$disable_fields[] = array(
					'attribute' => 'footer-sml-layout',
					'id'        => 'footer-sml-layout',
					'label'     => esc_html__( 'Disable Footer', 'smarttoolz' ),
					'switch_on' => 'disabled',
					'type'      => 'switch',
				);
			}
			...
			$fields                         = array();
			$fields['smarttoolz_theme_settings'] = array(
				'title'      => __( 'SmartToolz Settings', 'smarttoolz' ),
				'toggleable' => true,
				'fields'     => apply_filters(
					'smarttoolz_theme_lifterlms_settings',
					array(
						array(
							array(
								'attribute' => 'site-sidebar-layout',
								'id'        => 'site-sidebar-layout',
								'label'     => esc_html__( 'Sidebar', 'smarttoolz' ),
								'type'      => 'ast-select',
								'options'   => array(
									'default'       => esc_html__( 'Customizer Setting', 'smarttoolz' ),
									'left-sidebar'  => esc_html__( 'Left Sidebar', 'smarttoolz' ),
									'right-sidebar' => esc_html__( 'Right Sidebar', 'smarttoolz' ),
									'no-sidebar'    => esc_html__( 'No Sidebar', 'smarttoolz' ),
								),
							),
							array(
								'attribute' => 'site-content-layout',
								'id'        => 'site-content-layout',
								'label'     => esc_html__( 'Content Layout', 'smarttoolz' ),
								'type'      => 'ast-select',
								'options'   => array(
									'default'         => esc_html__( 'Customizer Setting', 'smarttoolz' ),
									'boxed-container' => esc_html__( 'Boxed', 'smarttoolz' ),
									'content-boxed-container' => esc_html__( 'Content Boxed', 'smarttoolz' ),
									'plain-container' => esc_html__( 'Full Width / Contained', 'smarttoolz' ),
									'page-builder'    => esc_html__( 'Full Width / Stretched', 'smarttoolz' ),
								),
							),
						),
						$disable_fields,
		...
		public function llms_single_lesson_css() {
			...
			return SmartToolz_Enqueue_Scripts::trim_css( $single_lesson_static_css );
		}
	}
 ...
 */
if ( apply_filters( 'smarttoolz_enable_lifterlms_integration', true ) ) {
	SmartToolz_LifterLMS::get_instance();
}
