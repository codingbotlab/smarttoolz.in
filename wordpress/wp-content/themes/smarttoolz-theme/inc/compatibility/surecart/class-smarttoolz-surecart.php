...
/**
 * SmartToolz SureCart Compatibility
 *
 * @since 4.4.0
 */
class SmartToolz_SureCart {
	/**
	 * The post type slug.
	 *
	 * @var string
	...
	public function __construct() {
		...
		add_action( 'smarttoolz_header_after', array( $this, 'smarttoolz_surecart_archive_page_banner_support' ) );
		add_action( 'smarttoolz_entry_top', array( $this, 'revert_surecart_support' ) );
		add_filter( 'smarttoolz_page_layout', array( $this, 'sc_shop_sidebar_layout' ) );
		add_filter( 'smarttoolz_get_content_layout', array( $this, 'sc_shop_content_layout' ) );
		add_action( 'wp', array( $this, 'remove_navigation_for_sc_product' ) );
		add_action( 'smarttoolz_theme_dynamic_css', array( $this, 'surecart_compatibility_dynamic_css' ) );

		// Boxed layout support.
		add_filter( 'smarttoolz_is_content_layout_boxed', array( $this, 'sc_shop_content_boxed_layout' ) );
		add_filter( 'smarttoolz_is_sidebar_layout_boxed', array( $this, 'sc_shop_sidebar_boxed_layout' ) );
		add_action( 'customize_register', array( $this, 'customize_register' ), 2 );
		add_filter( 'smarttoolz_theme_defaults', array( $this, 'smarttoolz_surecart_default_options' ) );
		add_filter( 'smarttoolz_archive_post_title', array( $this, 'customize_surecart_archive_title_area' ), 10, 2 );
		add_filter( 'smarttoolz_single_post_title', array( $this, 'customize_surecart_single_title_area' ), 10, 2 );
	...
	public function customize_register( $wp_customize ) {
		 ...
		 */
		require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/surecart/customizer/class-smarttoolz-customizer-register-surecart-section.php';
	}
	 ...
	 * @since 4.4.0
	 */
	public function smarttoolz_is_surecart_shop_page() {
		...
		$page_id = absint( smarttoolz_get_post_id() );
	...
	public function sc_shop_sidebar_layout( $sidebar_layout ) {
		if ( $this->smarttoolz_is_surecart_shop_page() ) {
			$sc_shop_sidebar = smarttoolz_get_option( 'archive-' . $this->post_type . '-sidebar-layout', 'default' );
		...
		return apply_filters( 'smarttoolz_get_surecart_shop_sidebar_layout', $sidebar_layout );
	}
	...
	public function sc_shop_content_layout( $content_layout ) {
		if ( $this->smarttoolz_is_surecart_shop_page() ) {
			$sc_shop_layout = smarttoolz_toggle_layout( 'archive-' . $this->post_type . '-ast-content-layout', 'single', false );
		...
		return apply_filters( 'smarttoolz_get_store_content_layout', $content_layout );
	}
	...
	public function sc_shop_content_boxed_layout( $is_style_boxed ) {
		if ( $this->smarttoolz_is_surecart_shop_page() ) {
			$sc_shop_layout_style = smarttoolz_get_option( 'archive-' . $this->post_type . '-content-style', 'default' );
		...
		return apply_filters( 'smarttoolz_get_store_layout_style', $is_style_boxed );
	}
	...
	public function sc_shop_sidebar_boxed_layout( $is_style_boxed ) {
		if ( $this->smarttoolz_is_surecart_shop_page() ) {
			$sc_shop_layout_style = smarttoolz_get_option( 'archive-' . $this->post_type . '-sidebar-style', 'default' );
		...
		return apply_filters( 'smarttoolz_get_store_sidebar_style', $is_style_boxed );
	}
	 ...
	 * @since 4.4.0
	 */
	public function smarttoolz_surecart_archive_page_banner_support() {
		if ( false === $this->smarttoolz_is_surecart_shop_page() ) {
			return;
		}
		...
		$visibility = apply_filters( 'smarttoolz_banner_title_area_visibility', $visibility );
		if ( 'disabled' === $visibility ) {
			$this->disable_page_loaded_banner_area();
			return;
		}

		$banner_layout = smarttoolz_get_option( 'ast-dynamic-archive-sc_product-layout', 'layout-1' );
		add_filter( 'smarttoolz_banner_elements_structure', array( $this, 'update_smarttoolz_banner_elements_structure' ) );
		add_filter( 'smarttoolz_banner_elements_post_type', array( $this, 'update_smarttoolz_banner_elements_post_type' ) );
		add_filter( 'smarttoolz_banner_elements_prefix', array( $this, 'update_smarttoolz_banner_elements_prefix' ) );
		add_filter( 'the_title', array( $this, 'update_the_title' ), 10, 2 );

		$title_area_enabled = smarttoolz_get_option( 'ast-archive-sc_product-title' );
		if ( $title_area_enabled ) {
			if ( 'layout-2' === $banner_layout ) {
				$smarttoolz_banner_hook = apply_filters( 'smarttoolz_banner_hook', 'smarttoolz_content_before' );
				add_action( $smarttoolz_banner_hook, array( $this, 'smarttoolz_surecart_hero_section' ), 20 );
			} else {
				add_filter( 'smarttoolz_single_layout_one_banner_visibility', '__return_false' );
				add_filter( 'smarttoolz_apply_hero_header_banner', '__return_false' );
				add_action( 'smarttoolz_primary_content_top', array( $this, 'smarttoolz_force_render_banner_layout_1' ) );
			}
		}
	}
	 ...
	 * @return void
	 */
	public function smarttoolz_force_render_banner_layout_1() {
		add_filter( 'smarttoolz_remove_entry_header_content', '__return_false' );
		?>
	 ...
	 * @since 4.4.0
	 */
	public function smarttoolz_surecart_hero_section() {
		if ( false === apply_filters( 'smarttoolz_apply_hero_header_banner', true ) ) {
			return;
		}

		$args = array( 'post_type' => $this->post_type );
		do_action( 'smarttoolz_before_archive_' . $this->post_type . '_banner_content' );
		get_template_part( 'template-parts/archive', 'banner', $args );
		do_action( 'smarttoolz_after_archive_' . $this->post_type . '_banner_content' );
	 ...
	 * @since 4.4.0
	 */
	public function update_smarttoolz_banner_elements_structure( $structure ) {
		return smarttoolz_get_option( 'ast-dynamic-archive-' . $this->post_type . '-structure', array( 'ast-dynamic-archive-' . $this->post_type . '-title', 'ast-dynamic-archive-' . $this->post_type . '-description' ) );
	}
	 ...
	 * @since 4.4.0
	 */
	public function update_smarttoolz_banner_elements_post_type( $post_type ) {
	 ...
	 * @since 4.4.0
	 */
	public function update_smarttoolz_banner_elements_prefix( $prefix ) {
		return 'archive';
	}
	...
	public function update_the_title( $title, $post_id ) {
		...
		$custom_title = smarttoolz_get_option( 'ast-dynamic-archive-' . $this->post_type . '-custom-title', '' );
		return ! empty( $custom_title ) ? $custom_title : $title;
	}

	/**
	 * Disable SmartToolz's next page's banner as we already loaded.
	 *
	 * @since 4.4.0
	 */
	public function disable_page_loaded_banner_area() {
		add_filter( 'smarttoolz_apply_hero_header_banner', '__return_false' );
		add_filter( 'smarttoolz_remove_entry_header_content', '__return_true' );
		add_filter( 'smarttoolz_single_layout_one_banner_visibility', '__return_false' );
	}

	/**
	 * Removed SmartToolz's navigation markup from SureCart single product page.
	 *
	 * @since 4.8.2
	 */
	...
	public function revert_surecart_support() {
		...
		remove_filter( 'smarttoolz_is_content_layout_boxed', array( $this, 'sc_shop_content_boxed_layout' ) );
		remove_filter( 'smarttoolz_is_sidebar_layout_boxed', array( $this, 'sc_shop_sidebar_boxed_layout' ) );

		remove_filter( 'smarttoolz_banner_elements_structure', array( $this, 'update_smarttoolz_banner_elements_structure' ) );
		remove_filter( 'smarttoolz_banner_elements_post_type', array( $this, 'update_smarttoolz_banner_elements_post_type' ) );
		remove_filter( 'smarttoolz_banner_elements_prefix', array( $this, 'update_smarttoolz_banner_elements_prefix' ) );
		remove_filter( 'the_title', array( $this, 'update_the_title' ), 10 );
	}

	/**
	 * SmartToolz SureCart default options.
	 *
	 * @param array $defaults Array of SmartToolz's options.
	 * @return array Filtered options array.
	 *
	 * @since 4.7.3
	 */
	public function smarttoolz_surecart_default_options( $defaults ) {
		...
		$surecart_banner_padding = SmartToolz_Posts_Structure_Loader::get_customizer_default( 'responsive-padding' );
	...
	public function customize_surecart_title_area( $title, $post_type, $singular = false ) {
		$surecart_titles = array(
			'sc_product'  => array(
				'single'  =>  __( 'Product', 'smarttoolz' ),
				'archive' => __( 'Products', 'smarttoolz' ),
			),
			'sc_collection' => array(
				'single'  =>  __( 'Collection', 'smarttoolz' ),
				'archive' => __( 'Collections', 'smarttoolz' ),
			),
			'sc_upsell'   => array(
				'single'  =>  __( 'Upsell', 'smarttoolz' ),
				'archive' => __( 'Upsells', 'smarttoolz' ),
			),
		);
		...
		if ( isset( $surecart_titles[ $post_type ][ $type ] ) ) {
			$title_area_suffix = ' ' . __( 'Title Area', 'smarttoolz' );
