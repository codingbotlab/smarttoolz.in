...
/**
 * SmartToolz WooCommerce Compatibility
 */
if ( ! class_exists( 'SmartToolz_Woocommerce' ) ) {
	...
	class SmartToolz_Woocommerce {
		...
		public function __construct() {
			...
			require_once SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/woocommerce-common-functions.php';// phpcs:ignore: WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			...
			add_filter( 'smarttoolz_get_sidebar', array( $this, 'replace_store_sidebar' ) );
			// Store Sidebar Layout.
			add_filter( 'smarttoolz_page_layout', array( $this, 'store_sidebar_layout' ) );
			// Store Content Layout.
			add_filter( 'smarttoolz_get_content_layout', array( $this, 'store_content_layout' ) );
			...
			add_filter( 'smarttoolz_get_dynamic_header_content', array( $this, 'smarttoolz_header_cart' ), 10, 3 );

			// Add Cart option in dropdown.
			add_filter( 'smarttoolz_header_section_elements', array( $this, 'header_section_elements' ) );
			...
			add_filter( 'woocommerce_get_stock_html', 'smarttoolz_woo_product_in_stock', 10, 2 );

			add_filter( 'smarttoolz_schema_body', array( $this, 'remove_body_schema' ) );

			// Header Cart Icon.
			add_action( 'smarttoolz_woo_header_cart_icons_before', array( $this, 'header_cart_icon_markup' ) );

			add_action( 'smarttoolz_cart_in_menu_class', array( $this, 'header_cart_icon_class' ), 99 );

			// WooCommerce Store Notice.
			add_filter( 'woocommerce_demo_store', array( $this, 'smarttoolz_woocommerce_update_store_notice_atts' ) );

			add_filter( 'smarttoolz_dynamic_theme_css', array( $this, 'smarttoolz_woocommerce_store_dynamic_css' ) );

			// Single Product Free shipping.
			add_action( 'smarttoolz_woo_single_price_after', array( $this, 'woocommerce_shipping_text' ) );
			...
			add_action( 'admin_bar_menu', array( $this, 'smarttoolz_update_customize_admin_bar_link' ), 45 );
			...
			if ( ! defined( 'SMARTTOOLZ_EXT_VER' ) || ( defined( 'SMARTTOOLZ_EXT_VER' ) && ! SmartToolz_Ext_Extension::is_active( 'woocommerce' ) ) ) {
		...
		/**
		 * As WooCommerce-SmartToolz pro options moved to theme, decide here to load from theme's end after 3.9.2 version.
		 *
		 * @since 3.9.2
		 * @return bool true|false.
		 */
		public static function load_theme_side_woocommerce_strcture() {
			return ! defined( 'SMARTTOOLZ_EXT_VER' ) || smarttoolz_addon_check_version( '3.9.2', '>=' );
		}
		...
		public function modern_add_to_cart() {
			...
			// Product link markup.
			$header_woo_cart = smarttoolz_get_option( 'woo-header-cart-icon', 'default' );
			$cart_icon       = true === SmartToolz_Icons::is_svg_icons() ? SmartToolz_Icons::get_icons( 'default' === $header_woo_cart ? 'bag' : $header_woo_cart ) : SmartToolz_Builder_UI_Controller::fetch_svg_icon( 'shopping-' . $header_woo_cart, false );
			$classes         = implode(
				' ',
				array_filter(
					array(
		...
		public function add_modern_triggers_on_image() {
			...
			if ( smarttoolz_is_shop_page_modern_style() ) {
				...
				$html = apply_filters( 'smarttoolz_addon_shop_cards_buttons_html', $markup, $product );
		...
		public function get_sale_flash_markup( $sale_notification, $product ) {
			$text = __( 'Sale!', 'smarttoolz' ); // Default text.

			// CSS classes.
			$classes = array();
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$classes[] = smarttoolz_is_shop_page_modern_style() ? 'ast-on-card-button ast-onsale-card' : 'onsale';
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$classes = implode( ' ', $classes );

			// Generate markup.
			return '<span  ' . smarttoolz_attr(
		...
		public function sale_flash( $markup, $post, $product ) {
			...
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( ( ! is_singular( 'product' ) && smarttoolz_is_shop_page_modern_style() ) ) {
				/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				return '';
			}
		...
		public function change_cart_close_icon( $string ) {
			return preg_replace_callback(
				...
				static function( $matches ) {
					return $matches[1] . SmartToolz_Builder_UI_Controller::fetch_svg_icon( 'close', false ) . $matches[3];
				},
				$string
			);
		}
		...
		public function store_widgets_dynamic() {
			$shop_filter_array = array(
				'name'          => esc_html__( 'WooCommerce Sidebar', 'smarttoolz' ),
				'id'            => 'smarttoolz-woo-shop-sidebar',
				'description'   => __( 'This sidebar will be used on Product archive, Cart, Checkout and My Account pages.', 'smarttoolz' ),
				'before_widget' => '<div id="%1$s" class="ast-woo-sidebar-widget widget %2$s">',
				'after_widget'  => '</div>',
			);

			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( smarttoolz_has_pro_woocommerce_addon() && smarttoolz_get_option( 'shop-filter-accordion' ) ) {
				$shop_filter_array['before_title']   = '<h2 class="widget-title">';
				$shop_filter_array['after_title']    = SmartToolz_Builder_UI_Controller::fetch_svg_icon( 'angle-down', false ) . '</h2>';
			...
			register_sidebar(
				apply_filters(
					'smarttoolz_woocommerce_shop_sidebar_init',
					$shop_filter_array
				)
			);
		}
		...
		/**
		 * Update WooCommerce store notice. Extending this function to add custom data-attr as per SmartToolz's configuration.
		 *
		 * @since 3.9.0
		 *
		 ...
		 */
		public function smarttoolz_woocommerce_update_store_notice_atts( $notice ) {

			$store_notice_position = smarttoolz_get_option( 'store-notice-position' );
		...
		public function woocommerce_shipping_text() {
			if ( smarttoolz_get_option( 'single-product-enable-shipping' ) ) {
				$shipping_text = smarttoolz_get_i18n_option( 'single-product-shipping-text', _x( '%smarttoolz%', 'WooCommerce Single Product: Shipping Text', 'smarttoolz' ), false );
		 ...
		 */
		public function smarttoolz_woocommerce_store_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$store_notice_color    = smarttoolz_get_option( 'store-notice-text-color' );
			$store_notice_bg_color = smarttoolz_get_option( 'store-notice-background-color' );
			/**
			...
			if ( ! is_customize_preview() ) {
				...
				if ( empty( $notice ) ) {
					$notice = __( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'smarttoolz' );
				}
			...
			if ( ! $notice_hidden && 'hang-over-top' === smarttoolz_get_option( 'store-notice-position' ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				$dynamic_css .= smarttoolz_parse_css( $admin_bar_desktop_css, '783' );
				...
				$dynamic_css .= smarttoolz_parse_css( $admin_bar_responsive_css, '', '782' );
			}
			return $dynamic_css;
		}
		...
		public function svg_cart_icon( $cart_total_label_position, $cart_label_markup, $cart_info_markup, $cart_icon ) {
			...
			add_filter( 'smarttoolz_woo_default_header_cart_icon', '__return_false' );
			...
			printf(
				...
				$cart_icon ? wp_kses(
					$cart_icon,
					array_merge(
						SmartToolz_Icons::allowed_svg_args(),
						array(
							'i' => array(
								'class'           => array(),
		...
		public function header_cart_icon_markup() {
			...
			if ( apply_filters( 'smarttoolz_woocommerce_cart_icon', $woo_cart_icon_new_user ) ) {
				...
				if ( false === SmartToolz_Builder_Helper::$is_header_footer_builder_active && ! defined( 'SMARTTOOLZ_EXT_VER' ) ) {
					return;
				}
			}

			$defaults           = apply_filters( 'smarttoolz_woocommerce_cart_icon', $woo_cart_icon_new_user ) ? 'bag' : 'default';
			$icon               = smarttoolz_get_option( 'woo-header-cart-icon', $defaults );
			$cart_count_display = apply_filters( 'smarttoolz_header_cart_count', true );
			$cart_title         = apply_filters( 'smarttoolz_header_cart_title', __( 'Cart', 'smarttoolz' ) );

			$cart_title_markup         = '<span class="ast-woo-header-cart-title">' . esc_html( $cart_title ) . '</span>';
			$cart_total_label_position = smarttoolz_get_option( 'woo-header-cart-icon-total-label-position' );
			$cart_total_markup         = '';
			...
			$cart_check_total = smarttoolz_get_option( 'woo-header-cart-total-label' ) && null !== WC()->cart ? intval( WC()->cart->get_cart_contents_total() ) > 0 : true;
			/** @psalm-suppress RedundantConditionGivenDocblockType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			$display_cart_label = smarttoolz_get_i18n_option( 'woo-header-cart-label-display', _x( '%smarttoolz%', 'Header Builder: Cart Widget - Cart Label', 'smarttoolz' ) );
			...
			$cart_icon = sprintf(
				'<i class="smarttoolz-icon ast-icon-shopping-%1$s %2$s"
							%3$s
						>%4$s</i>',
				$icon ? $icon : '',
				$cart_count_display ? '' : 'no-cart-total',
				$cart_count_display ? 'data-cart-total="' . $cart_contents_count . '"' : '',
				$icon ? ( false !== SmartToolz_Icons::is_svg_icons() ? SmartToolz_Icons::get_icons( $icon ) : '' ) : ''
			);
		...
		public function header_cart_icon_class( $classes ) {
			...
			$header_cart_icon_style = smarttoolz_get_option( 'woo-header-cart-icon-style' );

			$classes[]                  = 'ast-menu-cart-' . $header_cart_icon_style;
			$header_cart_icon_has_color = smarttoolz_get_option( 'woo-header-cart-icon-color' );
		 ...
		 * WooCommerce adds it's own product schema hence schema data from SmartToolz should be disabled here.
		 *
		 * @since 1.8.0
		...
		public function rating_markup( $html, $rating, $count ) {
			...
			if ( $rating > 0 ) {
				...
				if ( $review_count > 0 && smarttoolz_get_option( 'shop-ratings-product-archive' ) === 'count_string' && ! is_product() ) {
					$single_caption = __( ' review', 'smarttoolz' );
					$plural_caption = __( ' reviews', 'smarttoolz' );
		...
		public function cart_page_upselles() {
			...
			$upselles_enabled = smarttoolz_get_option( 'enable-cart-upsells' );
			if ( ! $upselles_enabled ) {
		...
		public function woo_filter_style( $styles ) {
			...
			if ( ! SmartToolz_Enqueue_Scripts::should_load_woocommerce_css() ) {
				return array();
			}
			...
			$css_uri = SMARTTOOLZ_THEME_URI . 'assets/css/' . $dir_name . '/compatibility/woocommerce/';
			...
			if ( ! SmartToolz_Builder_Helper::apply_flex_based_css() ) {
				$styles = array(
					'woocommerce-layout'      => array(
						...
						'deps'    => '',
						'version' => SMARTTOOLZ_THEME_VERSION,
						'media'   => 'all',
						'has_rtl' => true,
					),
					'woocommerce-smallscreen' => array(
						...
						'version' => SMARTTOOLZ_THEME_VERSION,
						'media'   => 'only screen and (max-width: ' . apply_filters( 'woocommerce_style_smallscreen_breakpoint', smarttoolz_get_tablet_breakpoint() . 'px' ) . ')', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
						'has_rtl' => true,
					),
			...
			if ( is_product() && smarttoolz_get_option( 'single-product-sticky-add-to-cart' ) ) {
			...
			if ( has_block( 'woocommerce/cart' ) || has_block( 'woocommerce/checkout' ) ) {
				$styles['smarttoolz-woocommerce-blocks'] = array(
			...
			if (
				...
				$styles['smarttoolz-wc-dokan-compatibility'] = array(
		...
		public function subcategory_count_markup( $content, $category ) {
			...
			$content = sprintf( // WPCS: XSS OK.
				...
				_nx( '%1$s Product', '%1$s Products', $category->count, 'product categories', 'smarttoolz' ),
		...
		public function theme_defaults( $defaults ) {
			...
			$theme_options = get_option( 'smarttoolz-settings' );

			// Backward compatibility.
			$defaults['smarttoolz-woocommerce-cart-icons-flag'] = true;
			...
			$defaults['single-product-shipping-text']         = __( '& Free Shipping', 'smarttoolz' );
			$defaults['single-product-variation-tabs-layout'] = 'vertical';

			/* Cart button*/
			$defaults['woo-enable-cart-button-text'] = false;
			$defaults['woo-cart-button-text']        = __( 'Proceed to checkout', 'smarttoolz' );
			...
			$defaults['single-product-plus-minus-button'] = smarttoolz_has_pro_woocommerce_addon() ? true : false;
			$defaults['cart-plus-minus-button-type']      = 'normal';

			// Single Product Payments.
			$defaults['single-product-payment-icon-color'] = 'inherit';
			$defaults['single-product-payment-text']       = __( 'Guaranteed Safe Checkout', 'smarttoolz' );
			...
			$defaults['single-product-payment-list'] = array(
				'items' => array(
					...
					array(
						...
						'icon'    => 'cc-mastercard',
						'image'   => '',
						'label'   => __( 'Mastercard', 'smarttoolz' ),
					),
					array(
						'id'      => 'item-3',
						'enabled' => true,
		...
		public function shop_columns( $col ) {
			...
			$smarttoolz_shop_col = smarttoolz_get_option(
				'shop-grids',
				array(
					'desktop' => 4,
					'tablet'  => 3,
			...
			return $smarttoolz_shop_col['desktop'];
		}
		 ...
		 * @return bool
		 */
		public function smarttoolz_woo_is_subcategory( $category_id = null ) {
		...
		public function shop_no_of_products() {
			...
			if ( is_product_taxonomy() && 'subcategories' === $taxonomy_page_display ) {
				$products = smarttoolz_get_option( 'shop-no-of-products' );
				if ( $this->smarttoolz_woo_is_subcategory() ) {
					return $products;
				}
		...
		public function shop_page_products_item_class( $classes = '' ) {
			...
			if ( is_shop() || is_product_taxonomy() ) {
				$shop_grid = smarttoolz_get_option(
					'shop-grids',
					array(
						'desktop' => 4,
						'tablet'  => 3,
			...
			$rt_section = smarttoolz_get_option( 'header-main-rt-section' );

			if ( 'woocommerce' === $rt_section ) {
				$classes[] = 'ast-woocommerce-cart-menu';
			}

			if ( is_store_notice_showing() && 'hang-over-top' === smarttoolz_get_option( 'store-notice-position' ) ) {
		...
		public function get_grid_column_count( $type = 'archive', $device = 'desktop', $default = 2 ) {
			...
			if ( 'archive' === $type ) {
				$products_grid = smarttoolz_get_option(
					'shop-grids',
					array(
						'desktop' => 4,
						'tablet'  => 3,
				...
				$products_grid = smarttoolz_get_option( 'single-product-related-upsell-grid' );
			}
		...
		public function store_widgets_init() {
			...
			register_sidebar(
				apply_filters(
					'smarttoolz_woocommerce_single_sidebar_init',
					array(
						'name'          => esc_html__( 'Product Sidebar', 'smarttoolz' ),
						'id'            => 'smarttoolz-woo-single-sidebar',
						'description'   => __( 'This sidebar will be used on Single Product page.', 'smarttoolz' ),
						'before_widget' => '<div id="%1$s" class="widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h2 class="widget-title">',
						'after_title'   => '</h2>',
		...
		public function replace_store_sidebar( $sidebar ) {
			...
			if ( is_shop() || is_product_taxonomy() || is_checkout() || is_cart() || is_account_page() ) {
				$sidebar = 'smarttoolz-woo-shop-sidebar';
			} elseif ( is_product() ) {
				$sidebar = 'smarttoolz-woo-single-sidebar';
			}
		...
		public function store_sidebar_layout( $sidebar_layout ) {
			...
			if ( is_shop() || is_product_taxonomy() || is_checkout() || is_cart() || is_account_page() || is_product() ) {
				...
				$woo_sidebar                 = smarttoolz_get_option( 'woocommerce-sidebar-layout' );
				$smarttoolz_with_modern_ecommerce = smarttoolz_get_option( 'modern-ecommerce-setup', true );
				...
				if ( is_shop() || is_product_taxonomy() ) {
					$global_page_specific_layout = smarttoolz_get_option( 'archive-product-sidebar-layout', 'default' );
				}

				if ( is_product() ) {
					$single_product_fallback_sidebar = false === $smarttoolz_with_modern_ecommerce ? smarttoolz_get_option( 'site-sidebar-layout' ) : smarttoolz_get_option( 'woocommerce-sidebar-layout' );
					$single_product_sidebar          = smarttoolz_get_option( 'single-product-sidebar-layout', 'default' );
				...
				if ( is_shop() && ! is_search() ) {
					...
					$shop_sidebar = smarttoolz_get_option_meta( 'site-sidebar-layout', '', true );
				}
			...
			return apply_filters( 'smarttoolz_get_store_sidebar_layout', $sidebar_layout );
		}
		/**
		 * WooCommerce Container
		 *
		...
		public function store_content_layout( $layout ) {
			...
			if ( is_woocommerce() || is_checkout() || is_cart() || is_account_page() ) {
				...
				$woo_layout = smarttoolz_toggle_layout( 'woocommerce-ast-content-layout', 'global', false );
				...
				if ( is_shop() || is_product_taxonomy() ) {
					$global_page_specific_layout = smarttoolz_toggle_layout( 'archive-product-ast-content-layout', 'archive', false );
				}

				if ( is_product() ) {
					$global_page_specific_layout = smarttoolz_toggle_layout( 'single-product-ast-content-layout', 'single', false );
				}
				...
				if ( is_shop() && ! is_search() ) {
					$shop_page_id = get_option( 'woocommerce_shop_page_id' );
					$shop_layout  = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', $shop_page_id );
				} elseif ( is_product_taxonomy() || is_search() ) {
					$shop_layout = 'default';
				} else {
					$old_meta_layout = smarttoolz_get_option_meta( 'site-content-layout', '', true );
					if ( isset( $old_meta_layout ) ) {
						$shop_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', false, $old_meta_layout );
					} else {
						$shop_layout = smarttoolz_toggle_layout( 'ast-site-content-layout', 'meta', false );
					}
				}
			...
			return apply_filters( 'smarttoolz_get_store_content_layout', $layout );
		}
		...
		public function shop_meta_option() {
			...
			if ( is_shop() && ! is_search() ) {
				...
				if ( 'disabled' === $footer_layout ) {
					remove_action( 'smarttoolz_footer_content', 'smarttoolz_footer_small_footer_template', 5 );
				}
			}
		}
		...
		public function shop_customization() {
			...
			if ( ! apply_filters( 'smarttoolz_woo_shop_product_structure_override', false ) ) {

				add_action( 'woocommerce_before_shop_loop_item', 'smarttoolz_woo_shop_thumbnail_wrap_start', 6 );
				/**
				 * Add sale flash before shop loop.
				 */
				add_action( 'woocommerce_before_shop_loop_item', 'woocommerce_show_product_loop_sale_flash', 9 );

				add_action( 'woocommerce_after_shop_loop_item', 'smarttoolz_woo_shop_thumbnail_wrap_end', 8 );
				/**
				 * Add Out of Stock to the Shop page
				 */
				add_action( 'woocommerce_shop_loop_item_title', 'smarttoolz_woo_shop_out_of_stock', 8 );
				 ...
				 */
				add_action( 'woocommerce_after_shop_loop_item', 'smarttoolz_woo_woocommerce_shop_product_content' );
			}
		}
		...
		public function single_product_customization() {
			...
			if ( smarttoolz_get_option( 'single-product-breadcrumb-disable' ) ) {
		...
		public function woocommerce_init() {
			add_action( 'woocommerce_after_mini_cart', array( $this, 'smarttoolz_update_flyout_cart_layout' ) );
		...
		public function before_main_content_start() {
			$site_sidebar = smarttoolz_page_layout();
		 ...
		 * @return bool
		 */
		public static function smarttoolz_update_default_font_styling() {
			$smarttoolz_settings = smarttoolz_get_options();
			return apply_filters( 'smarttoolz_default_font_style_update', isset( $smarttoolz_settings['ast-font-style-update'] ) ? false : true );
		}
		...
		public function add_scripts_styles() {
			if ( ! SmartToolz_Enqueue_Scripts::should_load_woocommerce_js() ) {
				return;
			}
			...
			$if_free_shipping                        = smarttoolz_get_option( 'single-product-enable-shipping' );
			$single_product_heading_tab_active_color = smarttoolz_get_option( 'single-product-heading-tab-active-color' );
			...
			$font_style_updates                      = self::smarttoolz_update_default_font_styling();
			...
			$can_update_cart_color   = smarttoolz_cart_color_default_icon_old_header();
			$cart_new_color_setting  = smarttoolz_get_option( $icon_cart_color_slug, $theme_color );
			$header_cart_count_color = $can_update_cart_color ? $cart_new_color_setting : $theme_color;

			$btn_color = smarttoolz_get_option( 'button-color' );
			if ( empty( $btn_color ) ) {
				$btn_color = smarttoolz_get_foreground_color( $theme_color );
			}

			$btn_h_color = smarttoolz_get_option( 'button-h-color' );
			if ( empty( $btn_h_color ) ) {
				$btn_h_color = smarttoolz_get_foreground_color( $link_h_color );
			}
			...
			$cart_h_color = smarttoolz_get_foreground_color( $link_h_color );

			$site_content_width         = smarttoolz_get_option( 'site-content-width', 1200 );
			$woo_shop_archive_width     = smarttoolz_get_option( 'shop-archive-width' );
			$woo_shop_archive_max_width = smarttoolz_get_option( 'shop-archive-max-width' );
			...
			$smarttoolz_support_woo_btns_global = SmartToolz_Dynamic_CSS::smarttoolz_woo_support_global_settings();

			if ( $smarttoolz_support_woo_btns_global ) {
				$woo_btn_compatibility_desktop = smarttoolz_get_font_array_css( smarttoolz_get_option( 'font-family-button' ), smarttoolz_get_option( 'font-weight-button' ), $theme_btn_font_size, 'font-extras-button' );
				$woo_btn_compatibility_tablet  = array(
			...
			if ( ! smarttoolz_wc_is_star_rating_compatibility() ) {
			...
			if ( SmartToolz_Dynamic_CSS::v4_block_editor_compat() ) {
			...
			if ( is_cart() && false === SmartToolz_Builder_Helper::apply_flex_based_css() && true === smarttoolz_get_option( 'cart-modern-layout' ) && true === smarttoolz_get_option( 'enable-cart-upsells' ) ) {
				$css_desktop_output['.woocommerce[class*="rel-up-columns-"] .site-main div.product .related.products ul.products li.product, .woocommerce-page .site-main ul.products li.product'] = array(
					'width' => '100%',
				);
			}
			...
			if ( apply_filters( 'smarttoolz_theme_woocommerce_global_h2_font', smarttoolz_get_option( 'woo_support_global_settings', false ) ) ) {
			...
			if ( false === SmartToolz_Icons::is_svg_icons() ) {
				...
				$css_desktop_output['.ast-site-header-cart i.smarttoolz-icon:before'] = array(
					'font-family' => 'SmartToolz',
				);
			...
			$css_desktop_output['.ast-site-header-cart.ast-menu-cart-fill i.smarttoolz-icon'] = array(
				' font-size' => '1.1em',
			);
			...
			$css_desktop_output['li.woocommerce-custom-menu-item .ast-site-header-cart i.smarttoolz-icon:after'] = array(
				' padding-left' => '2px',
			);
			...
			$css_desktop_output['.ast-site-header-cart i.smarttoolz-icon:after'] = array(
				' background' => $header_cart_count_color,
			);

			// Legacy fallback: SmartToolz Pro 4.13.7+ outputs these off-canvas Product Filters rules itself (for the Shop page and product taxonomy archives), so only print them here for older/absent addon versions.
			$sidebars_widgets = wp_get_sidebars_widgets();
			if ( is_shop() && isset( $sidebars_widgets['smarttoolz-woo-product-off-canvas-sidebar'] ) && ( ! defined( 'SMARTTOOLZ_EXT_VER' ) || version_compare( SMARTTOOLZ_EXT_VER, '4.13.7', '<' ) ) ) {
				$css_output .= '
					@media ( max-width: 601px ) {
						.woocommerce-shop .smarttoolz-off-canvas-sidebar .wp-block-woocommerce-product-filters {
							width: 100%;
						}
						.woocommerce-shop .smarttoolz-off-canvas-sidebar .wp-block-woocommerce-product-filters :is(.wc-block-product-filters__open-overlay, .wc-block-product-filters__overlay-header, .wc-block-product-filters__overlay-footer) {
							display: none;
						}
						.woocommerce-shop .smarttoolz-off-canvas-sidebar .wp-block-woocommerce-product-filters .wc-block-product-filters__overlay {
							pointer-events: auto;
							position: relative;
							width: 100%;
						}
						.woocommerce-shop .smarttoolz-off-canvas-sidebar .wp-block-woocommerce-product-filters .wc-block-product-filters__overlay .wc-block-product-filters__overlay-dialog {
							position: relative;
							transform: translateY(0);
						}
						.woocommerce-shop .smarttoolz-off-canvas-sidebar .wp-block-woocommerce-product-filters .wc-block-product-filters__overlay-dialog .wc-block-product-filters__overlay-content {
							overflow: auto;
							padding: 2px;
						}
					}
			...
			if ( is_account_page() && false === smarttoolz_get_option( 'modern-woo-account-view', false ) ) {
				$css_output .= '
			...
			if ( 'flyout' === smarttoolz_get_option( 'woo-header-cart-click-action' ) ) {
				$css_output .= '.woocommerce a.added_to_cart { display: none; }';
			}

			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( ! ( defined( 'SMARTTOOLZ_EXT_VER' ) && class_exists( 'SmartToolz_Ext_Extension' ) && SmartToolz_Ext_Extension::is_active( 'woocommerce' ) ) ) {
				/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= '
			...
			if ( true === smarttoolz_check_is_structural_setup() ) {
			...
			$css_output .= smarttoolz_parse_css( $css_desktop_output );
			...
			$css_output .= smarttoolz_parse_css( $tablet_css_shop_page_grid, smarttoolz_get_mobile_breakpoint( '', 1 ), smarttoolz_get_tablet_breakpoint() );
			...
			if ( ! SmartToolz_Builder_Helper::apply_flex_based_css() ) {
				...
				$css_output .= smarttoolz_parse_css( $tablet_shop_page_grid_lang_direction_css, smarttoolz_get_mobile_breakpoint( '', 1 ), smarttoolz_get_tablet_breakpoint() );
			}
			...
			$css_output .= smarttoolz_parse_css( $min_tablet_css, smarttoolz_get_tablet_breakpoint( '', 1 ) );
			...
			$css_output .= smarttoolz_parse_css( $css_global_button_tablet, '', smarttoolz_get_tablet_breakpoint() );
			...
			$css_output .= smarttoolz_parse_css( $css_global_button_mobile, '', smarttoolz_get_mobile_breakpoint() );
			...
			$css_output .= smarttoolz_parse_css( $global_button_mobile_lang_direction_css, '', smarttoolz_get_mobile_breakpoint() );

			if ( 'page-builder' !== smarttoolz_get_content_layout() ) {
				...
				if ( 'custom' === $woo_shop_archive_width ) {
					...
					$site_width  = array(
						'.ast-woo-shop-archive .site-content > .ast-container' => array(
							'max-width' => smarttoolz_get_css_value( $woo_shop_archive_max_width, 'px' ),
						),
					);
					$css_output .= smarttoolz_parse_css( $site_width, smarttoolz_get_tablet_breakpoint( '', 1 ) );
			...
			/* Parse CSS from array()*/
			$css_output .= smarttoolz_parse_css( $woo_product_css, smarttoolz_get_tablet_breakpoint( '', 1 ) );
			...
			*/
			if ( self::smarttoolz_global_btn_woo_comp() ) {
				if ( ! $smarttoolz_support_woo_btns_global ) {
					...
					$css_output           .= smarttoolz_parse_css( $woo_global_button_css );
				}
				...
				if ( $if_free_shipping ) {
					...
					$css_output            .= smarttoolz_parse_css( $woo_free_shipping_text );
				}
			}

			if ( ! is_shop() && ! is_product() ) {

				$css_output .= smarttoolz_parse_css(
					array(
			...
			$css_output .= smarttoolz_parse_css( $woo_product_lang_direction_css, smarttoolz_get_tablet_breakpoint( '', 1 ) );
			/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			/**
			 * Single page cart button size.
			 */
			$single_product_cart_button_width = smarttoolz_get_option( 'single-product-cart-button-width' );
			...
			$css_output_cart_button_width_desktop = array(
				...
				$single_cart_button => array(
					'width' => smarttoolz_get_css_value( $single_product_cart_button_width_desktop, '%' ),
				),
			);

			$css_output .= smarttoolz_parse_css( $css_output_cart_button_width_desktop );

			$css_output_cart_button_width_mobile = array(

				$single_cart_button => array(
					'width' => smarttoolz_get_css_value( $single_product_cart_button_width_mobile, '%' ),
				),
			);
			...
			$css_output_cart_button_width_tablet = array(
				...
				$single_cart_button => array(
					'width' => smarttoolz_get_css_value( $single_product_cart_button_width_tablet, '%' ),
				),
			);
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= smarttoolz_parse_css( $css_output_cart_button_width_tablet, '', smarttoolz_get_tablet_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= smarttoolz_parse_css( $css_output_cart_button_width_mobile, '', smarttoolz_get_mobile_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			$css_output .= smarttoolz_parse_css( $css_output_woo_select_default );

			$is_sticky_add_to_cart_position_active = smarttoolz_get_option( 'single-product-sticky-add-to-cart' );
			...
			if ( is_product() && $is_sticky_add_to_cart_position_active ) {
				 ...
				 */
				$sticky_add_to_cart_position         = smarttoolz_get_option( 'single-product-sticky-add-to-cart-position' );
				$sticky_add_to_cart_text_color       = smarttoolz_get_option( 'single-product-sticky-add-to-cart-text-color' );
				$sticky_add_to_cart_bg_color         = smarttoolz_get_option( 'single-product-sticky-add-to-cart-bg-color' );
				$sticky_add_to_cart_btn_text_color   = smarttoolz_get_option( 'single-product-sticky-add-to-cart-btn-n-color' );
				$sticky_add_to_cart_btn_text_color_h = smarttoolz_get_option( 'single-product-sticky-add-to-cart-btn-h-color' );
				$sticky_add_to_cart_btn_bg_color     = smarttoolz_get_option( 'single-product-sticky-add-to-cart-btn-bg-n-color' );
				$sticky_add_to_cart_btn_bg_color_h   = smarttoolz_get_option( 'single-product-sticky-add-to-cart-btn-bg-h-color' );
				...
				if ( is_admin_bar_showing() ) {
					...
					$css_output .= smarttoolz_parse_css( $sticky_add_to_cart_admin_bar, '601' );
				}

				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= smarttoolz_parse_css( $sticky_add_to_cart_responsive_tablet, '', smarttoolz_get_tablet_breakpoint() );
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= smarttoolz_parse_css( $sticky_add_to_cart_responsive_mobile, '', smarttoolz_get_mobile_breakpoint() );
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

				$css_output .= smarttoolz_parse_css( $sticky_add_to_cart_p );
				$css_output .= smarttoolz_parse_css( $sticky_add_to_cart );
			}

			$smarttoolz_add_to_cart_quantity_btn_enabled = smarttoolz_add_to_cart_quantity_btn_enabled();

			// Add to cart quantity button.
			if ( $smarttoolz_add_to_cart_quantity_btn_enabled ) {
			...
			if ( 'shop-page-modern-style' === smarttoolz_get_option( 'shop-style' ) ) {
				$modern_shop_page_css = '';

				if ( 'none' !== smarttoolz_get_option( 'product-sale-notification', 'default' ) ) {
					$modern_shop_page_css .= '
						.ast-onsale-card {
							position: absolute;
							top: 1.5em;
				...
				$modern_shop_page_css .= '
					...
					.smarttoolz-shop-thumbnail-wrap:hover .ast-on-card-button:not(.ast-onsale-card) {
						opacity: 1;
						visibility: visible;
					}
			...
			if ( self::load_theme_side_woocommerce_strcture() ) {
				$css_output .= $this->smarttoolz_shop_summary_box_alignment();
			}
			...
			$woo_variation_layout = smarttoolz_get_option( 'single-product-variation-tabs-layout' );
			...
			if ( 'horizontal' === $woo_variation_layout ) {
				...
				$css_output .= smarttoolz_parse_css( $css_output_woo_variation_layout );
			}
			...
			$css_output .= smarttoolz_parse_css( $woo_active_filter_css );

			// Single product payment.
			$single_product_payment_array = smarttoolz_get_option( 'single-product-structure' );
			...
			if (
				smarttoolz_load_woocommerce_login_form_password_icon()
				&& (
					is_account_page() ||
					(
						is_checkout() &&
				...
				$css_output                   .= smarttoolz_parse_css( $css_output_show_password_icon );
			}

			wp_add_inline_style( 'woocommerce-general', apply_filters( 'smarttoolz_theme_woocommerce_dynamic_css', $css_output ) );
			...
			/* Parse CSS from array() */
			$yith_wcwl_main_style = smarttoolz_parse_css( $yith_wcwl_main_style );
			...
			/* Parse CSS from array()*/
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$yith_wcwl_main_style .= smarttoolz_parse_css( $yith_wcwl_main_style_small, '', smarttoolz_get_tablet_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		 ...
		 * @return string
		 */
		public function smarttoolz_shop_summary_box_alignment() {
			$shop_product_alignment = smarttoolz_get_option( 'shop-product-align-responsive' );
			$desktop_alignment      = isset( $shop_product_alignment['desktop'] ) ? $shop_product_alignment['desktop'] : '';
			$tablet_alignment       = isset( $shop_product_alignment['tablet'] ) ? $shop_product_alignment['tablet'] : '';
			$mobile_alignment       = isset( $shop_product_alignment['mobile'] ) ? $shop_product_alignment['mobile'] : '';
			...
			$tablet_breakpoint = smarttoolz_get_tablet_breakpoint();
			$mobile_breakpoint = smarttoolz_get_mobile_breakpoint();
		...
		public function customize_register( $wp_customize ) {
			 ...
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/class-smarttoolz-customizer-register-woo-section.php';

			/**
			 * Sections
			 */
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-smarttoolz-woo-shop-container-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-smarttoolz-woo-shop-sidebar-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smarttoolz-woo-shop-layout-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smarttoolz-woo-shop-single-layout-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smarttoolz-woo-shop-cart-layout-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smarttoolz-woo-shop-misc-layout-configs.php';
			require SMARTTOOLZ_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-smarttoolz-woo-store-notice-configs.php';
		 ...
		 * @since 1.0.0
		 */
		public function smarttoolz_header_cart( $output, $section, $section_type ) {

			if ( 'woocommerce' === $section_type && apply_filters( 'smarttoolz_woo_header_cart_icon', true ) ) {
		...
		public function woo_mini_cart_markup( $device = 'desktop' ) {
			...
			$cart_click_action   = smarttoolz_get_option( 'woo-header-cart-click-action', 'default' );
			$desktop_cart_flyout = 'flyout' === $cart_click_action ? 'ast-desktop-cart-flyout' : '';
			$cart_menu_classes   = apply_filters( 'smarttoolz_cart_in_menu_class', array( 'ast-menu-cart-with-border', $desktop_cart_flyout ) );
			ob_start();
			if ( is_customize_preview() && true === SmartToolz_Builder_Helper::$is_header_footer_builder_active ) {
			...
			<div class="ast-site-header-cart <?php echo esc_attr( implode( ' ', $cart_menu_classes ) ); ?>">
				<div class="ast-site-header-cart-li <?php echo esc_attr( $class ); ?>">
					<?php $this->smarttoolz_get_cart_link(); ?>
				</div>
		 ...
		 * @since  1.0.0
		 */
		public function smarttoolz_get_cart_link() {
			// Suppress TooFewArguments warning for this line
			/** @psalm-suppress TooFewArguments */
			$woo_cart_link = smarttoolz_woocommerce_get_cart_url();
			/** @psalm-suppress RedundantConditionGivenDocblockType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			<a href="<?php echo esc_url( $woo_cart_link ); ?>" class="cart-container ast-cart-desktop-position-<?php echo esc_attr( $cart_total_label_position['desktop'] ); ?> ast-cart-mobile-position-<?php echo esc_attr( $cart_total_label_position['mobile'] ); ?> ast-cart-tablet-position-<?php echo esc_attr( $cart_total_label_position['tablet'] ); ?>" aria-label="<?php echo esc_attr( $aria_label ); ?>">
						...
						<?php
						do_action( 'smarttoolz_woo_header_cart_icons_before' );

						if ( apply_filters( 'smarttoolz_woo_default_header_cart_icon', true ) ) {
							?>
							<div class="ast-cart-menu-wrap">
								<span class="count">
								<span class="ast-count-text">
									<?php
									if ( apply_filters( 'smarttoolz_woo_header_cart_total', true ) && null != WC()->cart ) {
						...
						do_action( 'smarttoolz_woo_header_cart_icons_after' );
		...
		public function cart_link_fragment( $fragments ) {
			...
			add_filter( 'woocommerce_get_cart_url', 'smarttoolz_woocommerce_get_cart_url' );

			ob_start();
			$this->smarttoolz_get_cart_link();
			...
			remove_filter( 'woocommerce_get_cart_url', 'smarttoolz_woocommerce_get_cart_url' );
		 ...
		 * @since 3.9.0
		 */
		public function smarttoolz_update_flyout_cart_layout() {
			if ( WC()->cart->is_empty() ) {
				do_action( 'smarttoolz_empty_cart_before' );
				?>
					<div class="ast-mini-cart-empty">
						<div class="ast-mini-cart-message">
							<p class="woocommerce-mini-cart__empty-message"><?php echo esc_html( apply_filters( 'smarttoolz_mini_cart_empty_msg', __( 'No products in the cart.', 'smarttoolz' ) ) ); ?></p>
						</div>
						<?php do_action( 'smarttoolz_empty_cart_content' ); ?>
						<div class="woocommerce-mini-cart__buttons">
							<a href="<?php /** @psalm-suppress PossiblyFalseArgument */  echo esc_url( get_permalink( wc_get_page_id( 'shop' ) ) ); ?>" class="button wc-forward ast-continue-shopping"><?php esc_html_e( 'Continue Shopping', 'smarttoolz' ); ?></a> <?php // phpcs:ignore Generic.Commenting.DocComment.MissingShort ?>
						</div>
					</div>
				<?php
				do_action( 'smarttoolz_empty_cart_after' );
			}
		}
		...
		public function woocommerce_proceed_to_checkout_button_html() {
			$cart_button_text = smarttoolz_get_i18n_option( 'woo-cart-button-text', _x( '%smarttoolz%', 'WooCommerce Cart: Cart Button Text', 'smarttoolz' ) );
		...
		public function woocommerce_proceed_to_checkout_button() {
			...
			$enable_cart_button_text = smarttoolz_get_option( 'woo-enable-cart-button-text' );
			$cart_button_text        = smarttoolz_get_i18n_option( 'woo-cart-button-text', _x( '%smarttoolz%', 'WooCommerce Cart: Cart Button Text', 'smarttoolz' ) );
		 ...
		 */
		public function smarttoolz_update_customize_admin_bar_link( $wp_admin_bar ) {
		 ...
		 */
		public static function smarttoolz_global_btn_woo_comp() {
			$smarttoolz_settings = smarttoolz_get_options();
			return apply_filters( 'smarttoolz_global_btn_woo_comp', isset( $smarttoolz_settings['global-btn-woo-css'] ) ? false : true );
		}
		 ...
		 */
		public function smarttoolz_woo_woocommerce_template_product_title( $product_type ) {
		...
		public function woocommerce_product_single_payments() {
			$section_title    = smarttoolz_get_i18n_option( 'single-product-payment-text', _x( '%smarttoolz%', 'WooCommerce Single Product: Payments - Payment Title.', 'smarttoolz' ) );
			$if_color_version = smarttoolz_get_option( 'single-product-payment-icon-color' );
			...
			$payment_list     = smarttoolz_get_option( 'single-product-payment-list' );
						...
						if ( isset( $single['source'] ) && $single['source'] ) {
							...
							<li class="ast-custom-payment">
								...
								if ( isset( $single['icon'] ) && $single['icon'] ) {
									...
									?>
								<li class="ast-custom-payment">
										<?php echo do_shortcode( SmartToolz_Builder_UI_Controller::fetch_svg_icon( $selected_icon, false ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</li>
									<?php
								}
							}
		...
		public function single_product_content_structure( $product_type = '' ) {
			...
			$single_structure = apply_filters( 'smarttoolz_woo_single_product_structure', smarttoolz_get_option( 'single-product-structure' ), $product_type );
			...
			if ( is_array( $single_structure ) && ! empty( $single_structure ) ) {
				 ...
				 */
				$smarttoolz_addons_condition = smarttoolz_has_pro_woocommerce_addon();
				...
				foreach ( $single_structure as $value ) {
					...
					switch ( $value ) {
						case 'title':
							 ...
							 */
							do_action( 'smarttoolz_woo_single_title_before' );
							$this->smarttoolz_woo_woocommerce_template_product_title( $product_type );
							do_action( 'smarttoolz_woo_single_title_after' );
							break;
						case 'price':
							/**
							 * Add Product Price on single product page for all products.
							 */
							do_action( 'smarttoolz_woo_single_price_before' );
							woocommerce_template_single_price();
							do_action( 'smarttoolz_woo_single_price_after' );
							break;
						case 'ratings':
							/**
							 * Add rating on single product page for all products.
							 */
							do_action( 'smarttoolz_woo_single_rating_before' );
							woocommerce_template_single_rating();
							do_action( 'smarttoolz_woo_single_rating_after' );
							break;
						case 'short_desc':
							do_action( 'smarttoolz_woo_single_short_description_before' );
							woocommerce_template_single_excerpt();
							do_action( 'smarttoolz_woo_single_short_description_after' );
							break;
						case 'add_cart':
							do_action( 'smarttoolz_woo_single_add_to_cart_before' );
							woocommerce_template_single_add_to_cart();
							do_action( 'smarttoolz_woo_single_add_to_cart_after' );
							break;
						case 'summary-extras':
							/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
							if ( $smarttoolz_addons_condition && is_callable( array( SMARTTOOLZ_Ext_WooCommerce_Markup::get_instance(), 'single_product_extras' ) ) ) {
								/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
								do_action( 'smarttoolz_woo_single_extras_before' );
								SMARTTOOLZ_Ext_WooCommerce_Markup::get_instance()->single_product_extras();
								do_action( 'smarttoolz_woo_single_extras_after' );
							}
							break;
						case 'single-product-payments':
								do_action( 'smarttoolz_woo_single_product_payments_before' );
								$this->woocommerce_product_single_payments();
								do_action( 'smarttoolz_woo_single_product_payments_after' );

							break;
						case 'meta':
							do_action( 'smarttoolz_woo_single_category_before' );
							woocommerce_template_single_meta();
							do_action( 'smarttoolz_woo_single_category_after' );
							break;
						case 'category':
							do_action( 'smarttoolz_woo_single_product_category_before' );
							$this->single_product_category();
							do_action( 'smarttoolz_woo_single_product_category_after' );
							break;
						default:
							break;
					}
				...
				if ( $smarttoolz_addons_condition && smarttoolz_get_option( 'accordion-inside-woo-summary' ) && 'accordion' === smarttoolz_get_option( 'single-product-tabs-layout' ) && smarttoolz_get_option( 'single-product-tabs-display' ) ) {
					/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					SMARTTOOLZ_Ext_WooCommerce_Markup::get_instance()->woo_product_tabs_layout_output();
				}
			}
		}
		...
		public function single_product_sticky_add_to_cart() {
			...
			if ( is_product() && smarttoolz_get_option( 'single-product-sticky-add-to-cart' ) ) {
				...
				$sticky_position  = smarttoolz_get_option( 'single-product-sticky-add-to-cart-position' );
				$add_to_cart_ajax = smarttoolz_get_option( 'single-product-ajax-add-to-cart' );
				// @codingStandardsIgnoreStart
				/**
				...
				// @codingStandardsIgnoreEnd
					if ( is_customize_preview() ) {
						echo '<div class="ast-sticky-add-to-cart customizer-item-block-preview customizer-navigate-on-focus ' . esc_attr( $sticky_position ) . '" data-section="smarttoolz-settings[single-product-sticky-add-to-cart]" data-type="control">';
						SmartToolz_Builder_UI_Controller::render_customizer_edit_button( 'row-editor-shortcut' );
					} else {
		...
		public function option_woocommerce_enable_ajax_add_to_cart( $value ) {
			$smarttoolz_shop_add_to_cart = smarttoolz_get_option( 'shop-add-to-cart-action' );
			if ( $smarttoolz_shop_add_to_cart && 'default' !== $smarttoolz_shop_add_to_cart ) {
				return 'yes';
			}
			return $value;
		}
		...
		public function get_quantity_btn_class() {
			$quantity_btn_type = smarttoolz_get_option( 'cart-plus-minus-button-type', 'normal' );
...
if ( apply_filters( 'smarttoolz_enable_woocommerce_integration', true ) ) {
	SmartToolz_Woocommerce::get_instance();
}
