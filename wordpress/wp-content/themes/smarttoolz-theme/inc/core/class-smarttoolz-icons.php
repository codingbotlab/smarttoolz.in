 ...
 * @since 3.3.0
 */
class SmartToolz_Icons {
	/**
	 * Constructor function that initializes required actions and hooks
	 */
	public function __construct() {
		// Remove smarttoolz.woff and other format of SmartToolz font files when SVG is enabled.
		if ( self::is_svg_icons() ) {
			add_filter( 'smarttoolz_enable_default_fonts', '__return_false' );
		}
	}
	...
	public static function is_svg_icons() {
		...
		return apply_filters( 'smarttoolz_is_svg_icons', isset( $smarttoolz_settings['can-update-smarttoolz-icons-svg'] ) ? $smarttoolz_settings['can-update-smarttoolz-icons-svg'] : true );
	}
	...
	public static function get_icons( $icon, $is_echo = false, $replace = false, $menu_location = 'main' ) {
		...
		if ( true === self::is_svg_icons() ) {
			...
			if ( 'menu-bars' === $icon ) {
				$menu_icon = apply_filters( 'smarttoolz_' . $menu_location . '_menu_toggle_icon', 'menu-toggle-icon' );
				$output    = '<span class="' . esc_attr( $menu_icon ) . '"></span>';
			}
		}

		$output = apply_filters( 'smarttoolz_svg_icon_element', $output, $icon );
		...
		$output = apply_filters(
			'smarttoolz_svg_icon', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			sprintf(
