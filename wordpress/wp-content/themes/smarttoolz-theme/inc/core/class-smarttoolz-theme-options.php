 ...
 * Theme Options
 */
if ( ! class_exists( 'SmartToolz_Theme_Options' ) ) {
	/**
	 * Theme Options
	 */
	class SmartToolz_Theme_Options {
		/**
		 * Class instance.
		 *
		 * @var SmartToolz_Theme_Options|null $instance Class instance.
		 */
		...
		/**
		 * A static theme smarttoolz-options variable.
		 *
		 * @since 4.0.2
		 * @var mixed $smarttoolz_options
		 */
		public static $smarttoolz_options = null;
		...
		public static function defaults() {
			...
			/**
			 * Update SmartToolz customizer default values. To not update directly on existing users site, added backwards.
			 *
			 * @since 3.6.3
			 */
			 ...
			 * @since 4.5.2
			 */
			$apply_scndry_default_padding_values = smarttoolz_scndry_btn_default_padding();
			$update_secondary_paddings           = SmartToolz_Dynamic_CSS::smarttoolz_4_6_4_compatibility();
			$update_secondary_border             = SmartToolz_Dynamic_CSS::smarttoolz_4_8_0_compatibility();
			...
			$smarttoolz_options        = self::get_smarttoolz_options();
			$post_per_page        = intval( get_option( 'posts_per_page' ) );
			$blog_defaults_update = SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility();
			$reorder_color_seq    = SmartToolz_Dynamic_CSS::smarttoolz_4_8_9_compatibility();

			// Update SmartToolz heading 5 font size & handled backward case
			$update_heading_five_font_size  = SmartToolz_Dynamic_CSS::smarttoolz_4_6_14_compatibility();
			...
			self::$defaults = apply_filters(
				...
				array(
					...
					'section-search-page-title-custom-title' => __( 'Search Results for:', 'smarttoolz' ),
					'section-search-page-title-found-custom-description' => __( 'Here are the search results for your search.', 'smarttoolz' ),
					'section-search-page-title-not-found-custom-description' => __( 'Sorry, but we could not find anything related to your search terms. Please try again.', 'smarttoolz' ),
					...
					'blog-excerpt-marker'                  => __( '[&hellip;]', 'smarttoolz' ),
					...
					'post-card-background-overlay'         => array(
						'background-type'  => '',
						'background-color' => smarttoolz_get_option( 'post-card-featured-overlay' ), // fetch from old option.
					),
					...
					'button-radius-fields'                 => array(
						'desktop'      => array(
							'top'    => ! isset( $smarttoolz_options['button-radius'] ) ? '' : $smarttoolz_options['button-radius'],
							'right'  => ! isset( $smarttoolz_options['button-radius'] ) ? '' : $smarttoolz_options['button-radius'],
							'bottom' => ! isset( $smarttoolz_options['button-radius'] ) ? '' : $smarttoolz_options['button-radius'],
							'left'   => ! isset( $smarttoolz_options['button-radius'] ) ? '' : $smarttoolz_options['button-radius'],
						),
						'tablet'       => array(
							'top'    => '',
							'right'  => '',
					...
					'footer-sml-section-1-credit'          => __( 'Copyright &copy; [current_year] [site_title] | Powered by [theme_author]', 'smarttoolz' ),
					'footer-sml-section-2'                 => '',
					'footer-sml-section-2-credit'          => __( 'Copyright &copy; [current_year] [site_title] | Powered by [theme_author]', 'smarttoolz' ),
					...
					'header-main-rt-section-html'          => '<button>' . __( 'Contact Us', 'smarttoolz' ) . '</button>',
					'header-main-rt-section-button-text'   => __( 'Button', 'smarttoolz' ),
					'header-main-rt-section-button-link'   => apply_filters( 'smarttoolz_site_url', 'https://www.wpsmarttoolz.com' ),
					...
					// Entry Content.
					'wp-blocks-ui'                         => false === smarttoolz_check_is_structural_setup() ? 'custom' : 'comfort',
					...
					// Container.
					'single-page-ast-content-layout'       => false === smarttoolz_check_is_structural_setup() ? 'default' : 'normal-width-container',
					'single-page-content-style'            => false === smarttoolz_check_is_structural_setup() ? 'default' : 'unboxed',
					...
					'body-font-extras'                     => array(
						'line-height'         => ! isset( $smarttoolz_options['body-font-extras'] ) && isset( $smarttoolz_options['body-line-height'] ) ? $smarttoolz_options['body-line-height'] : '1.65',
						'line-height-unit'    => SmartToolz_Dynamic_CSS::smarttoolz_4_6_14_compatibility() ? '' : 'em',
						'letter-spacing'      => '',
						'letter-spacing-unit' => 'px',
						'text-transform'      => ! isset( $smarttoolz_options['body-font-extras'] ) && isset( $smarttoolz_options['body-text-transform'] ) ? $smarttoolz_options['body-text-transform'] : '',
						'text-decoration'     => '',
					),
					'headings-font-extras'                 => array(
						'line-height'         => ! isset( $smarttoolz_options['headings-font-extras'] ) && isset( $smarttoolz_options['headings-line-height'] ) ? $smarttoolz_options['headings-line-height'] : '',
						'line-height-unit'    => 'em',
						'letter-spacing'      => '',
						'letter-spacing-unit' => 'px',
						'text-transform'      => ! isset( $smarttoolz_options['headings-font-extras'] ) && isset( $smarttoolz_options['headings-text-transform'] ) ? $smarttoolz_options['headings-text-transform'] : '',
						'text-decoration'     => '',
					),
					'para-margin-bottom'                   => '',
					...
					// Sidebar.
					'site-sidebar-layout'                  => false === smarttoolz_check_old_sidebar_user() ? 'right-sidebar' : 'no-sidebar',
					'site-sidebar-width'                   => 30,
					'single-page-sidebar-layout'           => false === smarttoolz_check_is_structural_setup() ? 'default' : 'no-sidebar',
					...
					'ast-dynamic-single-download-structure' => true === smarttoolz_enable_edd_featured_image_defaults() ? array(
		...
		/**
		 * Get smarttoolz-options DB values.
		 *
		 * @return array Return array of theme options from database.
		 *
		 * @since 4.0.0
		 */
		public static function get_smarttoolz_options() {
			if ( is_null( self::$smarttoolz_options ) || is_customize_preview() ) {
				self::$smarttoolz_options = get_option( SMARTTOOLZ_THEME_SETTINGS, array() );
			}
			return self::$smarttoolz_options;
		}
		...
		public static function get_db_options() {
			self::$db_options_no_defaults = self::get_smarttoolz_options();
 ...
 */
SmartToolz_Theme_Options::get_instance();
