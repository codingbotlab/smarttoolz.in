...
final class SmartToolz_Fonts {
	...
	public static function get_fonts() {
		...
		do_action( 'smarttoolz_get_fonts' );
		return apply_filters( 'smarttoolz_add_fonts', self::$fonts );
	}

	/**
	 * Renders the <link> tag for all fonts in the $fonts array.
	 *
	 * @since 1.0.16 Added the filter 'smarttoolz_render_fonts' to support custom fonts.
	 * @since 1.0.0
	 * @return void
	 */
	public static function render_fonts() {

		$font_list = apply_filters( 'smarttoolz_render_fonts', self::get_fonts() );
		...
		foreach ( $font_list as $name => $font ) {
			if ( ! empty( $name ) && ! isset( $system_fonts[ $name ] ) ) {
				...
				// Add Subset.
				$subset = apply_filters( 'smarttoolz_font_subset', '', $name );
				if ( ! empty( $subset ) ) {
					$font_subset[] = $subset;
				}
			}
		...
		$is_self_hosted = SmartToolz_API_Init::get_admin_settings_option( 'self_hosted_gfonts', false );
		...
		if ( $is_self_hosted && ! is_customize_preview() && ! is_admin() ) {
			if ( SmartToolz_API_Init::get_admin_settings_option( 'preload_local_fonts', false ) ) {
				smarttoolz_load_preload_local_fonts( $google_font_url );
			}
			wp_enqueue_style( 'smarttoolz-google-fonts', smarttoolz_get_webfont_url( $google_font_url ), array(), SMARTTOOLZ_THEME_VERSION, 'all' );
		} else {
			wp_enqueue_style( 'smarttoolz-google-fonts', $google_font_url, array(), SMARTTOOLZ_THEME_VERSION, 'all' );
		}
	}
	...
	public static function google_fonts_url( $fonts, $subsets = array() ) {
		...
		$fonts = apply_filters( 'smarttoolz_google_fonts', $fonts );

		$fonts = apply_filters( 'smarttoolz_google_fonts_selected', $fonts );
		...
		foreach ( $fonts as $font_name => $font_weight ) {
			...
			if ( ! empty( $font_weight ) ) {
				...
				$font_family = str_replace( "'", '', smarttoolz_get_prop( $font_family, 0 ) );
		...
		if ( ! empty( $family ) ) {
			...
			$font_args['display'] = smarttoolz_get_fonts_display_property();
