...
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_comments_css', 11 );
 ...
 * @since 3.2.0
 */
function smarttoolz_comments_css( $dynamic_css ) {

	if ( smarttoolz_check_current_post_comment_enabled() || 0 < get_comments_number() ) {
		...
		$is_site_rtl                 = is_rtl();
		$reply_title_space_threshold = SmartToolz_Dynamic_CSS::smarttoolz_4_4_0_compatibility() ? 1.3 : 1.66666;
		...
		$desktop_comment_global = array(
			'.comment-reply-title'                         => array(
				'font-size' => smarttoolz_get_font_css_value( (int) $body_font_size_desktop * $reply_title_space_threshold ),
			),
			// Single Post Meta.
			'.ast-comment-meta'                            => array(
				'line-height' => '1.666666667',
				'color'       => esc_attr( $link_color ),
				'font-size'   => smarttoolz_get_font_css_value( (int) $body_font_size_desktop * 0.8571428571 ),
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
				'font-size' => smarttoolz_responsive_font( $body_font_size, 'desktop' ),
			),
		);
		$dynamic_css .= smarttoolz_parse_css( $desktop_comment_global );

		$update_customizer_strctural_defaults = true === smarttoolz_check_is_structural_setup();
		...
		$blog_improvements                    = SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility();
		...
		$single_post_comment_css = '.comments-title {
          ...
          .comment-reply-title {
            font-weight: ' . esc_attr( SmartToolz_Dynamic_CSS::smarttoolz_4_4_0_compatibility() ? '600' : 'normal' ) . ';
            line-height: 1.65;
          }
		...
		if ( SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility() ) {
			...
			if ( $post_with_boxed_layout && 'inside' !== smarttoolz_get_option( 'comments-box-placement', '' ) ) {
				$single_post_comment_css .= '
		...
		$dynamic_css .= SmartToolz_Enqueue_Scripts::trim_css( $single_post_comment_css );
		...
		$dynamic_css                 .= smarttoolz_parse_css( $static_layout_css_min_comment, smarttoolz_get_tablet_breakpoint( '', '1' ) );
		$global_button_comment_mobile = array(
			'.ast-separate-container .ast-comment-list li.depth-1' => array(
				'padding'       => SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility() ? '' : '1.5em 1em',
				'margin-bottom' => SmartToolz_Dynamic_CSS::smarttoolz_4_4_0_compatibility() ? '0' : '1.5em',
			),
			...
			// Single Post Meta.
			'.ast-comment-meta'                            => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? smarttoolz_get_font_css_value( (int) $body_font_size['mobile'] * 0.8571428571, 'px', 'mobile' ) : '',
			),
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? smarttoolz_get_font_css_value( (int) $body_font_size['mobile'] * $reply_title_space_threshold, 'px', 'mobile' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
		...
		$dynamic_css .= smarttoolz_parse_css( $global_button_comment_mobile, '', smarttoolz_get_mobile_breakpoint() );
		...
		$global_button_comment_tablet = array(
			...
			'.ast-comment-meta'                            => array(
				...
				'font-size' => ! empty( $body_font_size['tablet'] ) ? smarttoolz_get_font_css_value( (int) $body_font_size['tablet'] * 0.8571428571, 'px', 'tablet' ) : '',
			),
			'.ast-separate-container .ast-comment-list li.depth-1' => array(
				'padding' => SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility() ? '' : '1.5em 2.14em',
			),
			'.ast-separate-container .comment-respond'     => array(
				'padding' => SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility() ? '' : '2em 2.14em',
			),
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['tablet'] ) ? smarttoolz_get_font_css_value( (int) $body_font_size['tablet'] * $reply_title_space_threshold, 'px', 'tablet' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
		...
		if ( SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility() ) {
			$dynamic_css .= '
				...
				@media(min-width: ' . strval( smarttoolz_get_mobile_breakpoint( '', 1 ) ) . 'px) {
					header.ast-comment-meta {
						display: flex;
						width: 100%;
						margin-bottom: 0;
					...
					.ast-comment .comment-reply-link:hover {
						background: ' . smarttoolz_get_option( 'theme-color' ) . ';
						color: #fff;
					}
				...
				@media(min-width: ' . strval( smarttoolz_get_tablet_breakpoint() ) . 'px) {
			...
			';
			if ( 'above' === smarttoolz_get_option( 'comment-form-position' ) ) {
				$dynamic_css .= '
					.comment-respond {
			...
			$comments_section_placement = smarttoolz_get_option( 'comments-box-placement', '' );
		...
		return $dynamic_css .= smarttoolz_parse_css( $global_button_comment_tablet, '', smarttoolz_get_tablet_breakpoint() );
	}
	return $dynamic_css;
}
