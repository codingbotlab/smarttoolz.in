 ...
 *
 * Case: In customizer-defaults update case SmartToolz v3.8.3 we introduced some padding for stretched layout, that should not load for page builder layouts, that is why this compatibility added here.
 *
 * @param int $post_id Current post ID.
 *
 * @since 3.8.3
 */
function smarttoolz_check_any_page_builder_is_active( $post_id ) {
 ...
 * @since 3.3.0
 */
function smarttoolz_container_layout_css() {
	$container_layout = smarttoolz_get_content_layout();
	...
	$tab_one_max_breakpoint = smarttoolz_get_tablet_breakpoint( '', 1 );
	...
	$image_position    = smarttoolz_get_option( 'ast-dynamic-single-' . $current_post_type . '-article-featured-image-position-layout-1', 'behind' );
	...
	$customizer_default_update = smarttoolz_check_is_structural_setup();
	$page_title_header_padding = true === $customizer_default_update ? '2em' : '4em';
	// Transparent Header.
	$display_title = get_post_meta( absint( smarttoolz_get_post_id() ), 'site-post-title', true );
	...
	$page_container_css .= '
		...
		@media (min-width: ' . strval( smarttoolz_get_tablet_breakpoint( '', 1 ) ) . 'px) {
	...
	if ( 'page-builder' === $container_layout ) {
		...
		$smarttoolz_blog_improvements  = SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility();
		$post_navigation_selector = $smarttoolz_blog_improvements ? ', .ast-page-builder-template .post-navigation' : '';
		if ( true === $customizer_default_update ) {
			...
			if ( 'disabled' !== $display_title && true === apply_filters( 'smarttoolz_stretched_layout_with_spacing', true ) && false === smarttoolz_check_any_page_builder_is_active( smarttoolz_get_post_id() ) ) {
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$page_container_css .= smarttoolz_parse_selector( '.ast-single-post.ast-page-builder-template .site-main > article, .woocommerce.ast-page-builder-template .site-main' . $post_navigation_selector, 'wc' ) . ' {
						padding-top: 2em;
						padding-left: 20px;
						padding-right: 20px;
					}
		...
		if ( false === smarttoolz_get_option( 'improve-gb-editor-ui', true ) ) {
			/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		return SmartToolz_Enqueue_Scripts::trim_css( $page_container_css );
	}
