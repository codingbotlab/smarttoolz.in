...
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_inline_on_mobile_css' );
 ...
 * @return string
 */
function smarttoolz_inline_on_mobile_css( $dynamic_css ) {

	$inline_on_mobile_enable = false;
	for ( $index = 1; $index <= SmartToolz_Builder_Helper::$component_limit; $index++ ) {
		if ( false === smarttoolz_get_option( 'header-menu' . $index . '-menu-stack-on-mobile' ) ) {
	...
	return $dynamic_css . SmartToolz_Enqueue_Scripts::trim_css( $inline_on_mobile_css );
}
