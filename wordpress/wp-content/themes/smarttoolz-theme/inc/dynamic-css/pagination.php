...
add_filter( 'smarttoolz_dynamic_theme_css', 'smarttoolz_pagination_css', 11 );
 ...
 * @since 3.3.0
 */
function smarttoolz_pagination_css( $dynamic_css ) {

	if ( smarttoolz_check_pagination_enabled() || false !== apply_filters( 'smarttoolz_enable_pagination_css', false ) ) {
		...
		$pagination_color_output = array(
			...
			'.ast-pagination a, .nav-links a'    => array(
				'color' => esc_attr( SmartToolz_Dynamic_CSS::smarttoolz_4_6_0_compatibility() ? $text_color : $link_color ),
			),
			'.ast-pagination a:hover, .ast-pagination a:focus, .ast-pagination > span:hover:not(.dots), .ast-pagination > span.current' => array(
				'color' => esc_attr( $link_hover_color ),
			),
		);
		$dynamic_css            .= smarttoolz_parse_css( $pagination_color_output );
		...
		return $dynamic_css .= SmartToolz_Enqueue_Scripts::trim_css( $pagination_static_css );
	}
	return $dynamic_css;
}
