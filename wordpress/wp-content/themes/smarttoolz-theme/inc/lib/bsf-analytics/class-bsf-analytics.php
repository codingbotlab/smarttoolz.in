	...
	class BSF_Analytics {
		...
		public function option_notice() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$notice_string = sprintf(
					__( '<strong>Help shape the future of %1$s.</strong><br><br>Share how you use the plugin so we can build features that matter, fix issues faster, and make smarter decisions.', 'smarttoolz' ),
					esc_html( $data['product_name'] )
				);
				
				if ( is_multisite() ) {
					$notice_string .= __( 'This will be applicable for all sites from the network.', 'smarttoolz' );
				}
					...
					array(
						...
						'message'                    => sprintf(
							'<div class="notice-content">
									...
									<div class="smarttoolz-notices-container">
										<a href="%2$s" class="smarttoolz-notices button-primary">
										%3$s
										</a>
										<a href="%4$s" data-repeat-notice-after="%5$s" class="smarttoolz-notices button-secondary">
										%6$s
										</a>
									</div>
								</div>',
							/* translators: %s usage doc link */
							sprintf( $notice_string . '<span dir="%1s"> <a href="%2s" target="_blank" rel="noreferrer noopener">%3s</a><span><br><br>', $language_dir, esc_url( $usage_doc_link ), __( 'Learn more.', 'smarttoolz' ) ),
							esc_url(
								add_query_arg(
									array(
		...
		public function register_usage_tracking_setting() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$author         = isset( $data['author'] ) ? $data['author'] : 'SmartToolz';
				...
				add_settings_field(
					...
					__( 'Usage Tracking', 'smarttoolz' ),       // Field title.
		...
		public function render_settings_field_html( $args ) {
			...
			<label for="<?php echo esc_attr( $args['label_for'] ); ?>">
				...
				echo esc_html( sprintf( __( 'Help improve %s by sharing non-sensitive usage data — like PHP version and features used.', 'smarttoolz' ), $args['title'] ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

				if ( is_multisite() ) {
					esc_html_e( ' This will be applicable for all sites from the network.', 'smarttoolz' );
				}
				?>
			</label>
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $args['usage_doc_link'] ), __( 'Learn More.', 'smarttoolz' ) ) );
			?>
			</fieldset>
			<?php
		}
