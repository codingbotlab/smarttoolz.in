...
if ( ! class_exists( 'SmartToolz_Nps_Survey' ) ) {

	/**
	 * Admin
	 */
	class SmartToolz_Nps_Survey {
		/**
		 * Instance
		 *
		 * @since 4.8.7
		 * @var (Object) SmartToolz_Nps_Survey
		 */
		...
		private function __construct() {
			...
			if ( apply_filters( 'smarttoolz_nps_survey_disable', false ) ) {
				return;
			}
		...
		/**
		 * Add the SmartToolz theme and SmartToolz Pro versions to the NPS survey data.
		 *
		 * Only appended for SmartToolz's own survey submission, not for other
		 * BSF products that share the NPS survey library. The Pro version is
		 * added only when SmartToolz Pro (Addon) is active.
		 *
		...
		public function add_versions( $post_data, $nps_id = '' ) {
			if (
				is_array( $post_data ) &&
				defined( 'SMARTTOOLZ_THEME_VERSION' ) &&
				isset( $post_data['plugin_slug'] ) &&
				'smarttoolz' === $post_data['plugin_slug']
			) {
				$post_data['theme_version'] = SMARTTOOLZ_THEME_VERSION;

				if ( defined( 'SMARTTOOLZ_EXT_VER' ) ) {
					$post_data['pro_version'] = SMARTTOOLZ_EXT_VER;
				}
			}
	 ...
	 */
	SmartToolz_Nps_Survey::get_instance();
