	...
	public static function get_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to do that.', 'smarttoolz' ),
	...
	public static function submit_rating( $request ) {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( (string) $nonce ), 'wp_rest' ) ) {
			return new \WP_Error(
				...
				__( 'Nonce verification failed.', 'smarttoolz' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				__( 'Remote request failed.', 'smarttoolz' ),
				array( 'status' => 500 )
			);
		}
