...
if ( ! class_exists( 'BSF_Admin_Notices' ) ) :
	 ...
	 * BSF_Admin_Notices
	 *
	 * Renamed from SmartToolz_Notices. All runtime strings (AJAX action, nonce,
	...
	class BSF_Admin_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_smarttoolz-notice-dismiss', array( $this, 'dismiss_notice' ) );
			add_filter( 'wp_kses_allowed_html', array( $this, 'add_data_attributes' ), 10, 2 );
		}

		/**
		 * Migrate allowed_smarttoolz_notices → smarttoolz_notices_allowed (one-time, on first load).
		 *
		 * @since 1.2.2
		 * @return void
		 */
		private function maybe_migrate_notices_option() {
			$old = get_option( 'allowed_smarttoolz_notices', false );
			if ( false !== $old ) {
				update_option( 'smarttoolz_notices_allowed', $old );
				delete_option( 'allowed_smarttoolz_notices' );
			}
		}
		...
		public static function add_notice( $args = array() ) {
			...
			$notices   = get_option( 'smarttoolz_notices_allowed', array() );
			if ( ! in_array( $notice_id, $notices, true ) ) {
				$notices[] = $notice_id; // Add notice id to the array.
				update_option( 'smarttoolz_notices_allowed', $notices ); // Update the option.
			}
		}
		...
		public function dismiss_notice() {
			check_ajax_referer( 'smarttoolz-notices', 'nonce' );
			 ...
			 */
			$cap_check = apply_filters( 'smarttoolz_notices_user_cap_check', $has_cap );
			$cap_check = apply_filters( 'bsf_admin_notices_user_cap_check', $cap_check );

			if ( ! $has_cap || ! $cap_check ) {
				wp_send_json_error( esc_html__( 'Permission denied.', 'smarttoolz' ) );
			}

			$allowed_notices = get_option( 'smarttoolz_notices_allowed', array() ); // Get allowed notices.
			...
			// if $notice_id does not start with smarttoolz-notices-id and notice_id is not from the allowed notices, then return.
			if ( 0 !== strpos( $notice_id, 'smarttoolz-notices-id-' ) && ( ! in_array( $notice_id, $allowed_notices, true ) ) ) {
				wp_send_json_error( esc_html__( 'Invalid notice ID.', 'smarttoolz' ) );
			}
		...
		public function enqueue_scripts() {
			wp_register_style( 'bsf-smarttoolz-notices', self::get_url() . 'notices.css', array(), self::$version );
			wp_register_script( 'bsf-smarttoolz-notices', self::get_url() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				'bsf-smarttoolz-notices',
				'bsfSmartToolzNotices',
				array(
					'_notice_nonce' => wp_create_nonce( 'smarttoolz-notices' ),
				)
			);
		}
		...
		public function show_notices() {
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `smarttoolz-notices-id-<$array-index>`.
		...
		public static function markup( $notice = array() ) {
			wp_enqueue_script( 'bsf-smarttoolz-notices' );
			wp_enqueue_style( 'bsf-smarttoolz-notices' );

			// Dual-emit: legacy (smarttoolz_notice_*) + new (bsf_admin_notice_*) hooks for backward compat.
			// Note: consumers hooking BOTH names for the same event will be called twice.
			do_action( 'smarttoolz_notice_before_markup' );
			do_action( 'bsf_admin_notice_before_markup' );

			do_action( "smarttoolz_notice_before_markup_{$notice['id']}" );
			do_action( "bsf_admin_notice_before_markup_{$notice['id']}" );

			?>
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( 'smarttoolz-notice-wrapper ' . $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="smarttoolz-notice-container">
					<?php do_action( "smarttoolz_notice_inside_markup_{$notice['id']}" ); ?>
			...
			do_action( "smarttoolz_notice_after_markup_{$notice['id']}" );
			do_action( "bsf_admin_notice_after_markup_{$notice['id']}" );

			do_action( 'smarttoolz_notice_after_markup' );
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'smarttoolz-notice', 'notice' );
		...
		private static function get_notice_id( $notice, $key ) {
			...
			return 'smarttoolz-notices-id-' . $key;
		}
...
// are updated.
if ( ! class_exists( 'SmartToolz_Notices' ) ) {
	class_alias( 'BSF_Admin_Notices', 'SmartToolz_Notices' ); // phpcs:ignore PHPCompatibility.FunctionUse.NewFunctions.class_aliasFound
}
