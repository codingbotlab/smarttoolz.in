 ...
 * Download webfonts locally.
 */
class SmartToolz_WebFont_Loader {
	...
	public function __construct( $url = '' ) {
		...
		add_action( 'smarttoolz_delete_fonts_folder', array( $this, 'smarttoolz_delete_fonts_folder' ) );
	}
	...
	public function get_url() {
		...
		$smarttoolz_font_url = file_exists( $this->get_local_stylesheet_path() ) ? $this->get_local_stylesheet_url() : $this->remote_url;

		// If the local file exists, return its URL, with a fallback to the remote URL.
		smarttoolz_update_option( 'smarttoolz_font_url', wp_json_encode( $smarttoolz_font_url ) );

		return $smarttoolz_font_url;
	}
	...
	public function get_styles() {
		...
		$convert_to_url = apply_filters( 'smarttoolz_convert_fonts_to_url', true ); 
		...
		foreach ( $files as $remote => $local ) {
			...
			$files[ $remote ] = apply_filters( 'smarttoolz_webfont_loader_file_url', $url, $remote, $local );
		}
	...
	public function preload_local_fonts() {
		...
		update_site_option( 'smarttoolz_local_font_files', $local_font );
	...
	public function get_local_stylesheet_filename() {
		return apply_filters( 'smarttoolz_local_font_file_name', 'smarttoolz-local-fonts' );
	}
	...
	public function set_font_format( $format = 'woff2' ) {
		$this->font_format = apply_filters( 'smarttoolz_local_google_fonts_format', $format );
	}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			...
			$this->base_path = apply_filters( 'smarttoolz_local_fonts_base_path', $wp_content_dir );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			...
			$this->base_url = apply_filters( 'smarttoolz_local_fonts_base_url', $base_url );
		}
	...
	public function get_subfolder_name() {
		if ( ! $this->subfolder_name ) {
			$this->subfolder_name = apply_filters( 'smarttoolz_local_fonts_directory_name', 'smarttoolz-local-fonts' );
		}
	...
	public function get_current_blog_id() {
		...
		if ( ! $this->current_blog_id ) {
			$this->current_blog_id = apply_filters( 'smarttoolz_local_fonts_current_blog_id', get_current_blog_id() );
		}
	...
	public function schedule_cleanup() {
		if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
			if ( ! wp_next_scheduled( 'smarttoolz_delete_fonts_folder' ) && ! wp_installing() ) {
				wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'smarttoolz_delete_fonts_folder' );  // phpcs:ignore WPThemeReview.PluginTerritory.ForbiddenFunctions.cron_functionality_wp_schedule_event
			}
		}
	}
	 ...
	 * @return bool
	 */
	public function smarttoolz_delete_fonts_folder() {
		// Delete previously created supportive options.
		smarttoolz_delete_option( 'smarttoolz_font_url' );
		delete_site_option( 'smarttoolz_local_font_files' );
		$filesystem = $this->get_filesystem();
		if ( ! $filesystem ) {
			return false;
		}
...
/**
 * Create instance of SmartToolz_WebFont_Loader class.
 *
 * @param string $font_url Google font URL to set data.
 * @return SmartToolz_WebFont_Loader Instance of SmartToolz_WebFont_Loader class.
 * @since 3.6.0
 */
function smarttoolz_webfont_loader_instance( $font_url = '' ) {
	return new SmartToolz_WebFont_Loader( $font_url );
}
