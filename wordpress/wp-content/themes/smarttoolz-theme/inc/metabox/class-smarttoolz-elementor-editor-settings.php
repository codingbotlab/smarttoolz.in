 ...
 * @since 4.11.3
 */
if ( ! class_exists( 'SmartToolz_Elementor_Editor_Settings' ) ) {
	/**
	 * Elementor Editor Settings integration class.
	 *
	 * @since 4.11.3
	 */
	class SmartToolz_Elementor_Editor_Settings {
		/**
		 * Class instance.
		 *
		 * @var self|null
		...
		private function __construct() {
			...
			/**
			 * Filter to disable SmartToolz meta settings on the Elementor editor.
			 *
			 * @since 4.12.4
			 *
			 * @param bool $disable Whether to disable SmartToolz meta settings on the Elementor editor. Default false.
			 */
			if ( apply_filters( 'smarttoolz_disable_elementor_editor_meta_settings', false ) ) {
				return;
			}
		...
		private function init_hooks() {
			...
			// Hook into Elementor's document saving process to persist SmartToolz-specific settings.
		...
		/**
		 * Get array of setting keys for SmartToolz Elementor settings.
		 *
		 * @since 4.11.3
		 *
		 * @return array Array of setting keys.
		 */
		public static function get_smarttoolz_elementor_setting_keys() {
			$settings = array(
				'site-post-title', // SmartToolz Disable Title.
			...
			/**
			 * Filter to modify the SmartToolz Elementor setting keys.
			 *
			 * @since 4.11.3
			 *
			 * @param array $keys Array of SmartToolz Elementor setting keys.
			 */
			return apply_filters( 'smarttoolz_elementor_page_setting_keys', $settings );
		}
		...
		public function enqueue_editor_assets() {
			...
			$handle = 'smarttoolz-elementor-meta-settings';
			$path   = SMARTTOOLZ_THEME_URI . '/inc/metabox/extend-metabox/build/elementor.js';

			// Get the script dependencies from the asset file.
			$dependencies = array();
			if ( file_exists( SMARTTOOLZ_THEME_DIR . 'inc/metabox/extend-metabox/build/elementor.asset.php' ) ) {
				$assets = require SMARTTOOLZ_THEME_DIR . 'inc/metabox/extend-metabox/build/elementor.asset.php';
			...
			/**
			 * Filter to modify the localized data for SmartToolz smarttoolzElementor editor settings.
			 *
			 * @since 4.11.3
			 * @param array $localize_data Localized data for smarttoolzElementor editor settings.
			 */
			$localize_data = apply_filters(
				'smarttoolz_elementor_editor_localize_data',
				array(
					'themeName'    => smarttoolz_get_theme_name(),
					'themeIconUrl' => esc_url( apply_filters( 'smarttoolz_admin_menu_icon', SMARTTOOLZ_THEME_URI . 'inc/assets/images/smarttoolz-logo.svg' ) ),
					'postTitle'    => get_post_meta( intval( get_the_ID() ), 'site-post-title', true ),
				)
			);

			wp_localize_script( $handle, 'smarttoolzElementorEditor', $localize_data );
		}
		...
		public static function get_formatted_container_layout_options() {
			$options = SmartToolz_Meta_Boxes::get_instance()->get_content_layout_options();
		...
		public static function get_formatted_container_style_options() {
			$options = SmartToolz_Meta_Boxes::get_instance()->get_content_style_options();
		...
		public static function get_formatted_sidebar_layout_options() {
			$options = SmartToolz_Meta_Boxes::get_instance()->get_sidebar_options();
		...
		public static function get_formatted_sidebar_style_options() {
			$options = SmartToolz_Meta_Boxes::get_instance()->get_sidebar_style_options();
		...
		public static function get_formatted_header_enabled_options() {
			$options = SmartToolz_Meta_Boxes::get_instance()->get_header_enabled_options();
		 ...
		 * providing a placeholder div for the React-powered SmartToolz Settings UI.
		 *
		...
		public function register_document_controls( $document ) {
			...
			$document->start_controls_section(
				'smarttoolz_section',
				array(
					'label' => esc_html__( 'SmartToolz Settings', 'smarttoolz' ),
			...
			$document->add_control(
				...
				array(
					...
					'label'   => __( 'Container Layout', 'smarttoolz' ),
			...
			$document->add_control(
				...
				array(
					...
					'label'       => __( 'Container Style', 'smarttoolz' ),
					'type'        => \Elementor\Controls_Manager::CHOOSE,
			...
			$document->add_control(
				...
				array(
					'classes'     => 'ast-layout-control',
					'label'       => __( 'Sidebar Layout', 'smarttoolz' ),
					'type'        => \Elementor\Controls_Manager::CHOOSE,
			...
			$document->add_control(
				...
				array(
					'classes' => 'ast-selector-control',
					'label'   => __( 'Sidebar Style', 'smarttoolz' ),
					'type'    => \Elementor\Controls_Manager::CHOOSE,
			...
			$document->add_control(
				...
				array(
					'label'     => __( 'Disable Elements', 'smarttoolz' ),
					'type'      => \Elementor\Controls_Manager::HEADING,
					'separator' => 'before',
				)
			);

			$controls = SmartToolz_Meta_Boxes::get_instance()->get_disable_section_fields();
			...
			$document->add_control(
				...
				array(
					'label'     => __( 'Advanced Settings', 'smarttoolz' ),
					'type'      => \Elementor\Controls_Manager::HEADING,
					'separator' => 'before',
				)
			);
			...
			$document->add_control(
				...
				array(
					...
					'label'   => __( 'Header Rows', 'smarttoolz' ),
				)
			);

			$document->start_popover();

			$controls = SmartToolz_Meta_Boxes::get_instance()->get_header_disable_meta_fields();
			foreach ( $controls as $details ) {
				$meta_key = $details['key'];
				$document->add_control(
					$meta_key,
			...
			$document->add_control(
				...
				array(
					'classes' => 'ast-selector-control',
					'label'   => __( 'Transparent Header', 'smarttoolz' ),
					'type'    => \Elementor\Controls_Manager::CHOOSE,
			...
			if ( defined( 'SMARTTOOLZ_EXT_VER' ) && class_exists( 'SmartToolz_Ext_Extension' ) && SmartToolz_Ext_Extension::is_active( 'sticky-header' ) ) {
				$document->add_control(
					'stick-header-meta',
					array(
						'classes' => 'ast-selector-control',
						'label'   => __( 'Sticky Header', 'smarttoolz' ),
						'type'    => \Elementor\Controls_Manager::CHOOSE,
						'options' => self::get_formatted_header_enabled_options(),
				...
				$document->add_control(
					...
					array(
						'classes'   => 'ast-popover-control',
						'type'      => \Elementor\Controls_Manager::POPOVER_TOGGLE,
						'label'     => __( 'Sticky Header Rows', 'smarttoolz' ),
						'condition' => array(
				...
				$controls = SmartToolz_Meta_Boxes::get_instance()->get_sticky_header_options();
				foreach ( $controls as $details ) {
					$meta_key = $details['key'];
					$document->add_control(
						$meta_key,
			...
			$document->add_control(
				...
				array(
					...
					'notice_type' => 'warning',
					'content'     => __( 'Make sure to update your post for changes to take effect.', 'smarttoolz' ),
				)
			);
			...
			$document->add_control(
				...
				array(
					...
					'button_type' => 'success',
					'text'        => esc_html__( 'Preview Changes', 'smarttoolz' ),
					'event'       => 'namespace:editor:smarttoolzRefresh',
					'disabled'    => true,
				)
			);
				...
				$document->add_control(
					...
					array(
						...
						'raw'  => sprintf(
							...
							esc_html__( 'Create beautiful pages faster with advanced design controls, ready templates, and full flexibility.', 'smarttoolz' ),
							esc_url( smarttoolz_get_pro_url( '/pricing/', 'free-theme', 'elementor-editor', 'upgrade' ) ),
			...
			$document->add_control(
				...
				array(
					'type'        => \Elementor\Controls_Manager::NOTICE,
					'notice_type' => 'info',
					'content'     => __( "Changes to 'Hide Title' will automatically sync with SmartToolz’s 'Disable Title' option.", 'smarttoolz' ),
				)
			);
		...
		/**
		 * Adds inline styles to enhance SmartToolz controls in Elementor Page Settings.
		 *
		...
		public function add_custom_control_style() {
			...
			echo class_exists( 'SmartToolz_Minify' ) ? SmartToolz_Minify::trim_css( $css_rules ) : $css_rules;
		}

		/**
		 * Handle Elementor template redirect for previewing SmartToolz settings changes.
		 *
		 * @since 4.11.3
		 */
		public function handle_preview() {
			...
			$keys     = self::get_smarttoolz_elementor_setting_keys();
			...
			add_action( 'smarttoolz_the_title_enabled', '__return_true', 999 );
		}

		/**
		 * Sync specific SmartToolz settings from Elementor's Page Settings to post meta.
		 *
		...
		public function sync_save_settings( $document ) {
			...
			// Sync smarttoolz site-post-title with elementor hide_title.
		...
		public static function get_svg( $icon = '' ) {
			...
			$svg_icons = SmartToolz_Builder_UI_Controller::$ast_svgs;
			if ( ! SmartToolz_Builder_UI_Controller::$ast_svgs ) {
				SmartToolz_Builder_UI_Controller::fetch_svg_icon();
				$svg_icons = SmartToolz_Builder_UI_Controller::$ast_svgs;
			}
		...
		/**
		 * Get excluded post types for SmartToolz Meta setting in Elementor editor.
		 *
		 * @since 4.11.11
		 * @return array
		 */
		public static function get_excluded_post_types() {
			/**
			 * Filter to exclude post types from rendering SmartToolz Meta setting in Elementor editor.
			 *
			 * @param array $excluded_post_types Array of post types to exclude.
			 */
			return apply_filters(
				'smarttoolz_elementor_editor_excluded_post_types',
				array(
					'smarttoolz-advanced-hook', // SmartToolz Theme Builder Advanced Layout.
 ...
 */
SmartToolz_Elementor_Editor_Settings::get_instance();
